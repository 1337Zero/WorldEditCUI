...
public class RealmsDownloadLatestWorldScreen extends RealmsScreen {
   ...
   private void drawProgressBar() {
      double d = this.downloadStatus.bytesWritten.doubleValue() / this.downloadStatus.totalBytes.doubleValue() * 100.0D;
   ...
   public static String humanReadableSize(long bytes) {
      ...
      if (bytes < 1024L) {
         ...
         int j = (int)(Math.log((double)bytes) / Math.log(1024.0D));
         String string = "KMGTPE".charAt(j - 1) + "";
         return String.format(Locale.ROOT, "%.0f %sB", (double)bytes / Math.pow(1024.0D, (double)j), string);
      }
   }
