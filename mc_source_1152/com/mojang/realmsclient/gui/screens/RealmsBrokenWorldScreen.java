...
public class RealmsBrokenWorldScreen extends RealmsScreen {
   ...
   public void addButtons() {
      ...
      while(var1.hasNext()) {
         ...
         Object realmsButton2;
         if (bl) {
            realmsButton2 = new RealmsBrokenWorldScreen.PlayButton((Integer)playButtonIds.get((Integer)entry.getKey() - 1), this.getFramePositionX((Integer)entry.getKey()), getLocalizedString("mco.brokenworld.play"));
         } else {
            realmsButton2 = new RealmsBrokenWorldScreen.DownloadButton((Integer)downloadButtonIds.get((Integer)entry.getKey() - 1), this.getFramePositionX((Integer)entry.getKey()), getLocalizedString("mco.brokenworld.download"));
         }

         if (this.slotsThatHasBeenDownloaded.contains(entry.getKey())) {
            ((RealmsButton)realmsButton2).active(false);
            ((RealmsButton)realmsButton2).setMessage(getLocalizedString("mco.brokenworld.downloaded"));
         }

         this.buttonsAdd((AbstractRealmsButton)realmsButton2);
         this.buttonsAdd(new RealmsButton((Integer)resetButtonIds.get((Integer)entry.getKey() - 1), this.getFramePositionX((Integer)entry.getKey()), RealmsConstants.row(10), 80, 20, getLocalizedString("mco.brokenworld.reset")) {
            public void onPress() {
               ...
               if (i != RealmsBrokenWorldScreen.this.field_20492.activeSlot || RealmsBrokenWorldScreen.this.field_20492.worldType.equals(RealmsServer.WorldType.MINIGAME)) {
         ...
         while(true) {
            while(var7.hasNext()) {
               ...
               if (((RealmsWorldOptions)entry.getValue()).templateImage != null && ((RealmsWorldOptions)entry.getValue()).templateId != -1L) {
                  this.drawSlotFrame(this.getFramePositionX((Integer)entry.getKey()), RealmsConstants.row(1) + 5, xm, ym, this.field_20492.activeSlot == (Integer)entry.getKey() && !this.isMinigame(), ((RealmsWorldOptions)entry.getValue()).getSlotName((Integer)entry.getKey()), (Integer)entry.getKey(), ((RealmsWorldOptions)entry.getValue()).templateId, ((RealmsWorldOptions)entry.getValue()).templateImage, ((RealmsWorldOptions)entry.getValue()).empty);
               } else {
                  this.drawSlotFrame(this.getFramePositionX((Integer)entry.getKey()), RealmsConstants.row(1) + 5, xm, ym, this.field_20492.activeSlot == (Integer)entry.getKey() && !this.isMinigame(), ((RealmsWorldOptions)entry.getValue()).getSlotName((Integer)entry.getKey()), (Integer)entry.getKey(), -1L, (String)null, ((RealmsWorldOptions)entry.getValue()).empty);
               }
            }
   ...
   private void downloadWorld(int slotId) {
      ...
      try {
         ...
         RealmsDownloadLatestWorldScreen realmsDownloadLatestWorldScreen = new RealmsDownloadLatestWorldScreen(this, worldDownload, this.field_20492.name + " (" + ((RealmsWorldOptions)this.field_20492.slots.get(slotId)).getSlotName(slotId) + ")");
