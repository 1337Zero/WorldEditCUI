...
public class RealmsResetWorldScreen extends RealmsScreenWithCallback<WorldTemplate> {
   ...
   private void drawFrame(int x, int y, String text, long imageId, String image, RealmsResetWorldScreen.ResetType resetType, boolean hoveredOrFocused, boolean hovered) {
      if (imageId == -1L) {
         bind(image);
      } else {
   ...
   abstract class FrameButton extends RealmsButton {
      ...
      public void renderButton(int mouseX, int mouseY, float a) {
         RealmsResetWorldScreen.this.drawFrame(this.x(), this.y(), this.getProxy().getMessage(), this.imageId, this.image, this.resetType, this.getProxy().isHovered(), this.getProxy().isMouseOver((double)mouseX, (double)mouseY));
      }
   }
