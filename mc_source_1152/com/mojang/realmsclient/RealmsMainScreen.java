...
public class RealmsMainScreen extends RealmsScreen {
   ...
   public boolean shouldShowMessageInList() {
      if (this.hasParentalConsent() && this.hasFetchedServers) {
         if (this.trialsAvailable && !this.createdTrial) {
            ...
            } while(!realmsServer.ownerUUID.equals(Realms.getUUID()));
         ...
         if (realmsDataFetcher.isFetchedSinceLastTry(RealmsDataFetcher.Task.SERVER_LIST)) {
            ...
            if (list != null) {
               ...
               while(var4.hasNext()) {
                  ...
                  if (this.isSelfOwnedNonExpiredServer(realmsServer3)) {
                     bl2 = true;
                  }
               }
   ...
   private void checkParentalConsent() {
      (new Thread("MCO Compatability Checker #1") {
         public void run() {
            ...
            try {
               Boolean var2 = realmsClient.mcoEnabled();
               if (var2) {
      ...
      if (!RealmsClient.currentEnvironment.equals(RealmsClient.Environment.STAGE)) {
         (new Thread("MCO Stage Availability Checker #1") {
            public void run() {
               ...
               try {
                  Boolean var2 = realmsClient.stageAvailable();
                  if (var2) {
   ...
   private void configureClicked(RealmsServer realmsServer) {
      if (Realms.getUUID().equals(realmsServer.ownerUUID) || overrideConfigure) {
         ...
         minecraftClient.execute(() -> {
            minecraftClient.openScreen((new RealmsConfigureWorldScreen(this, realmsServer.id)).getProxy());
         });
      }
               ...
               public void run() {
                  try {
                     ...
                     if (realmsServer != null) {
                        ...
                        RealmsMainScreen.this.realmSelectionList.children().removeIf((realmListEntry) -> {
                           return realmListEntry instanceof RealmsMainScreen.RealmSelectionListEntry && ((RealmsMainScreen.RealmSelectionListEntry)realmListEntry).mServerData.id == RealmsMainScreen.this.selectedServerId;
                        });
   ...
   public void render(int xm, int ym, float a) {
      ...
      if (this.trialsAvailable && !this.createdTrial && this.shouldShowPopup()) {
         ...
         RealmsScreen.blit(this.createTrialButton.x() + this.createTrialButton.getWidth() - 8 - 4, this.createTrialButton.y() + this.createTrialButton.getHeight() / 2 - 4, 0.0F, (float)k, 8, 8, 8, 16);
   ...
   private void drawInvitationPendingIcon(int xm, int ym, int x, int y, boolean selectedOrHovered, boolean active) {
      ...
      if (bl4) {
         ...
         RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
         RenderSystem.pushMatrix();
         RealmsScreen.blit(x + 4, y + 4 + l, (float)m, bl ? 8.0F : 0.0F, 8, 8, 48, 16);
         RenderSystem.popMatrix();
      }
      ...
      if (bl5) {
         ...
         int o = this.fontWidth(string);
         this.fillGradient(m - 3, ym - 3, m + o + 3, ym + 8 + 3, -1073741824, -1073741824);
         this.fontDrawShadow(string, m, ym, -1);
      }
   ...
   protected void renderMousehoverTooltip(String msg, int x, int y) {
      if (msg != null) {
         int i = 0;
         int j = 0;
         String[] var6 = msg.split("\n");
         int m = var6.length;

         int k;
         for(int var8 = 0; var8 < m; ++var8) {
            String string = var6[var8];
            k = this.fontWidth(string);
            if (k > j) {
               j = k;
            }
         }
   ...
   public static void method_23765(ResourceManager resourceManager) {
      Collection<Identifier> collection = resourceManager.findResources("textures/gui/images", (string) -> {
         return string.endsWith(".png");
      });
      field_21517 = (List)collection.stream().filter((identifier) -> {
   ...
   class RealmSelectionListEntry extends RealmListEntry {
      ...
      private void renderMcoServerItem(RealmsServer serverData, int x, int y, int mouseX, int mouseY) {
         if (serverData.state == RealmsServer.State.UNINITIALIZED) {
            ...
            int i = -16777216 | (int)(127.0F * f) << 16 | (int)(255.0F * f) << 8 | (int)(127.0F * f);
            ...
            if (RealmsMainScreen.this.isSelfOwnedServer(serverData) && serverData.expired) {
               ...
               string2 = RealmsScreen.getLocalizedString("mco.selectServer.expiredList");
               ...
               int l = RealmsMainScreen.this.fontWidth(string3) + 17;
               int m = true;
               int n = x + RealmsMainScreen.this.fontWidth(string2) + 8;
               int o = y + 13;
               if (mouseX >= n && mouseX < n + l && mouseY > o && mouseY <= o + 16 & mouseY < RealmsMainScreen.this.height() - 40 && mouseY > 32 && !RealmsMainScreen.this.shouldShowPopup()) {
                  bl = true;
                  RealmsMainScreen.this.expiredHover = true;
               }

               int p = bl ? 2 : 1;
               RealmsScreen.blit(n, o, 0.0F, (float)(46 + p * 20), l / 2, 8, 256, 256);
               RealmsScreen.blit(n + l / 2, o, (float)(200 - l / 2), (float)(46 + p * 20), l / 2, 8, 256, 256);
               RealmsScreen.blit(n, o + 8, 0.0F, (float)(46 + p * 20 + 12), l / 2, 8, 256, 256);
               RealmsScreen.blit(n + l / 2, o + 8, (float)(200 - l / 2), (float)(46 + p * 20 + 12), l / 2, 8, 256, 256);
               RenderSystem.popMatrix();
               RenderSystem.disableBlend();
               int q = y + 11 + 5;
               int r = bl ? 16777120 : 16777215;
               RealmsMainScreen.this.drawString(string2, x + 2, q + 1, 15553363);
               RealmsMainScreen.this.drawCenteredString(string3, n + l / 2, q + 1, r);
            } else {
               if (serverData.worldType.equals(RealmsServer.WorldType.MINIGAME)) {
                  int s = 13413468;
                  string2 = RealmsScreen.getLocalizedString("mco.selectServer.minigame") + " ";
                  int t = RealmsMainScreen.this.fontWidth(string2);
                  RealmsMainScreen.this.drawString(string2, x + 2, y + 12, 13413468);
                  RealmsMainScreen.this.drawString(serverData.getMinigameName(), x + 2 + t, y + 12, 8421504);
               } else {
   ...
   class RealmSelectionList extends RealmsObjectSelectionList<RealmListEntry> {
      ...
      public boolean keyPressed(int eventKey, int scancode, int mods) {
         if (eventKey != 257 && eventKey != 32 && eventKey != 335) {
            ...
            return realmListEntry == null ? super.keyPressed(eventKey, scancode, mods) : realmListEntry.mouseClicked(0.0D, 0.0D, 0);
         }
      }
         ...
         if (item != -1) {
            ...
            if (RealmsMainScreen.this.shouldShowMessageInList()) {
               if (item == 0) {
                  ...
                  realmsServer3 = (RealmsServer)RealmsMainScreen.this.realmsServers.get(item - 1);
               }
            } else {
               ...
               realmsServer3 = (RealmsServer)RealmsMainScreen.this.realmsServers.get(item);
            }
