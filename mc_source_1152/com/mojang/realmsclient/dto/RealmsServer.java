...
public class RealmsServer extends ValueObject {
   ...
   public void updateServerPing(RealmsServerPlayerList serverPlayerList) {
      ...
      while(true) {
         ...
         } while(string.equals(Realms.getUUID()));
   ...
   private static void sortInvited(RealmsServer server) {
      server.players.sort((playerInfo, playerInfo2) -> {
   ...
   public static class McoServerComparator implements Comparator<RealmsServer> {
      ...
      public int compare(RealmsServer realmsServer, RealmsServer realmsServer2) {
         return ComparisonChain.start().compareTrueFirst(realmsServer.state.equals(RealmsServer.State.UNINITIALIZED), realmsServer2.state.equals(RealmsServer.State.UNINITIALIZED)).compareTrueFirst(realmsServer.expiredTrial, realmsServer2.expiredTrial).compareTrueFirst(realmsServer.owner.equals(this.refOwner), realmsServer2.owner.equals(this.refOwner)).compareFalseFirst(realmsServer.expired, realmsServer2.expired).compareTrueFirst(realmsServer.state.equals(RealmsServer.State.OPEN), realmsServer2.state.equals(RealmsServer.State.OPEN)).compare(realmsServer.id, realmsServer2.id).result();
      }
   }
}
