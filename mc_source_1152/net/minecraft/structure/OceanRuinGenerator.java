   ...
   public static class Piece extends SimpleStructurePiece {
      ...
      protected void handleMetadata(String metadata, BlockPos pos, IWorld world, Random random, BlockBox boundingBox) {
         if ("chest".equals(metadata)) {
            ...
            DrownedEntity drownedEntity = (DrownedEntity)EntityType.DROWNED.create(world.getWorld());
            drownedEntity.setPersistent();
            drownedEntity.refreshPositionAndAngles(pos, 0.0F, 0.0F);
            drownedEntity.initialize(world, world.getLocalDifficulty(pos), SpawnType.STRUCTURE, (EntityData)null, (CompoundTag)null);
      ...
      private int method_14829(BlockPos blockPos, BlockView blockView, BlockPos blockPos2) {
         ...
         while(var8.hasNext()) {
            ...
            for(FluidState fluidState = blockView.getFluidState(mutable); (blockState.isAir() || fluidState.matches(FluidTags.WATER) || blockState.getBlock().matches(BlockTags.ICE)) && o > 1; fluidState = blockView.getFluidState(mutable)) {
               --o;
               mutable.set(m, o, n);
