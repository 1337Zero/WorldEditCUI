...
public class StructureProcessorRule {
   ...
   public StructureProcessorRule(AbstractRuleTest abstractRuleTest, AbstractRuleTest abstractRuleTest2, BlockState blockState) {
      this(abstractRuleTest, abstractRuleTest2, blockState, (CompoundTag)null);
   }
   ...
   public <T> Dynamic<T> method_16764(DynamicOps<T> dynamicOps) {
      ...
      return this.tag == null ? new Dynamic(dynamicOps, object) : new Dynamic(dynamicOps, dynamicOps.mergeInto(object, dynamicOps.createString("output_nbt"), (new Dynamic(NbtOps.INSTANCE, this.tag)).convert(dynamicOps).getValue()));
   }
   ...
   public static <T> StructureProcessorRule method_16765(Dynamic<T> dynamic) {
      ...
      AbstractRuleTest abstractRuleTest = (AbstractRuleTest)DynamicDeserializer.deserialize(dynamic2, Registry.RULE_TEST, "predicate_type", AlwaysTrueRuleTest.INSTANCE);
      AbstractRuleTest abstractRuleTest2 = (AbstractRuleTest)DynamicDeserializer.deserialize(dynamic3, Registry.RULE_TEST, "predicate_type", AlwaysTrueRuleTest.INSTANCE);
      BlockState blockState = BlockState.deserialize(dynamic.get("output_state").orElseEmptyMap());
      CompoundTag compoundTag = (CompoundTag)dynamic.get("output_nbt").map((dynamicx) -> {
         return (Tag)dynamicx.convert(NbtOps.INSTANCE).getValue();
      }).orElse((Object)null);
