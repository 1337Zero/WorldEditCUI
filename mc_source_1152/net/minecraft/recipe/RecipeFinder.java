   ...
   class Filter {
         ...
         for(int k = 0; k < j; ++k) {
            if (RecipeFinder.this.idToAmountMap.get(this.field_7551[k]) >= i) {
               ...
               while(!this.field_7557.isEmpty()) {
                  ...
                  int p;
                  for(p = 0; p < n; ++p) {
                     if (!this.method_7426(bl, p) && this.method_7418(bl, m, p) && this.method_7425(bl, m, p)) {
                        this.method_7413(bl, p);
                        break;
                     }
                  }

                  p = this.field_7557.size();
                  if (p == l) {
      ...
      private int method_7415() {
         ...
         while(var2.hasNext()) {
            ...
            int k;
            for(IntListIterator var5 = ingredient.getIds().iterator(); var5.hasNext(); j = Math.max(j, RecipeFinder.this.idToAmountMap.get(k))) {
               k = (Integer)var5.next();
            }
