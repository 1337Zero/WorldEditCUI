...
public class InputSlotFiller<C extends Inventory> implements RecipeGridAligner<Integer> {
   ...
   protected void returnInputs() {
      for(int i = 0; i < this.craftingContainer.getCraftingWidth() * this.craftingContainer.getCraftingHeight() + 1; ++i) {
         if (i != this.craftingContainer.getCraftingResultSlotIndex() || !(this.craftingContainer instanceof CraftingTableContainer) && !(this.craftingContainer instanceof PlayerContainer)) {
            this.returnSlot(i);
         }
      }
   ...
   protected void fillInputSlots(Recipe<C> recipe, boolean craftAll) {
      ...
      if (this.recipeFinder.findRecipe(recipe, intList, j)) {
         ...
         if (this.recipeFinder.findRecipe(recipe, intList, l)) {
            this.returnInputs();
            this.alignRecipeToGrid(this.craftingContainer.getCraftingWidth(), this.craftingContainer.getCraftingHeight(), this.craftingContainer.getCraftingResultSlotIndex(), recipe, intList.iterator(), l);
         }
      }
