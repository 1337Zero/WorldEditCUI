...
public class BrewingRecipeRegistry {
   ...
   public static ItemStack craft(ItemStack input, ItemStack ingredient) {
      if (!ingredient.isEmpty()) {
         ...
         int l;
         BrewingRecipeRegistry.Recipe recipe2;
         for(l = ITEM_RECIPES.size(); k < l; ++k) {
            recipe2 = (BrewingRecipeRegistry.Recipe)ITEM_RECIPES.get(k);
            if (recipe2.input == item && recipe2.ingredient.test(input)) {
               return PotionUtil.setPotion(new ItemStack((ItemConvertible)recipe2.output), potion);
            }
         }

         k = 0;

         for(l = POTION_RECIPES.size(); k < l; ++k) {
            recipe2 = (BrewingRecipeRegistry.Recipe)POTION_RECIPES.get(k);
            if (recipe2.input == potion && recipe2.ingredient.test(input)) {
               return PotionUtil.setPotion(new ItemStack(item), (Potion)recipe2.output);
            }
         }
      }
