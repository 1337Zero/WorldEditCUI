...
public class PlayerAdvancementTracker {
   ...
   public void clearCriterions() {
      Iterator var1 = Criterions.getAllCriterions().iterator();
   ...
   private void beginTrackingAllAdvancements() {
      Iterator var1 = this.server.getAdvancementLoader().getAdvancements().iterator();
      ...
      if (this.advancementFile.isFile()) {
         try {
            ...
            try {
               ...
               while(var6.hasNext()) {
                  Entry<Identifier, AdvancementProgress> entry = (Entry)var6.next();
      ...
      if (advancementProgress.obtain(criterion)) {
         ...
         if (!bl2 && advancementProgress.isDone()) {
            ...
            if (advancement.getDisplay() != null && advancement.getDisplay().shouldAnnounceToChat() && this.owner.world.getGameRules().getBoolean(GameRules.ANNOUNCE_ADVANCEMENTS)) {
               this.server.getPlayerManager().sendToAll((Text)(new TranslatableText("chat.type.advancement." + advancement.getDisplay().getFrame().getId(), new Object[]{this.owner.getDisplayName(), advancement.toHoverableText()})));
            }
         }
      }
