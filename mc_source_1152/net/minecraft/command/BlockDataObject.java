...
public class BlockDataObject implements DataCommandObject {
   ...
   public Text feedbackQuery(Tag tag) {
      return new TranslatableText("commands.data.block.query", new Object[]{this.pos.getX(), this.pos.getY(), this.pos.getZ(), tag.toText()});
   }

   public Text feedbackGet(NbtPathArgumentType.NbtPath nbtPath, double scale, int result) {
      return new TranslatableText("commands.data.block.get", new Object[]{nbtPath, this.pos.getX(), this.pos.getY(), this.pos.getZ(), String.format(Locale.ROOT, "%.2f", scale), result});
   }
}
