...
public class FloatRangeArgument {
   ...
   public FloatRangeArgument(@Nullable Float var1, @Nullable Float float2) {
      this.min = var1;
      this.max = float2;
   }
   ...
   public static FloatRangeArgument parse(StringReader reader, boolean allowFloats, Function<Float, Float> transform) throws CommandSyntaxException {
      if (!reader.canRead()) {
         ...
         Float var4 = mapFloat(parseFloat(reader, allowFloats), transform);
         Float float3;
         if (reader.canRead(2) && reader.peek() == '.' && reader.peek(1) == '.') {
            reader.skip();
            reader.skip();
            float3 = mapFloat(parseFloat(reader, allowFloats), transform);
            if (var4 == null && float3 == null) {
               reader.setCursor(i);
               throw NumberRange.EXCEPTION_EMPTY.createWithContext(reader);
            }
         } else {
         ...
         if (var4 == null && float3 == null) {
            ...
            return new FloatRangeArgument(var4, float3);
         }
      }
   }
   ...
   @Nullable
   private static Float mapFloat(@Nullable Float var0, Function<Float, Float> function) {
