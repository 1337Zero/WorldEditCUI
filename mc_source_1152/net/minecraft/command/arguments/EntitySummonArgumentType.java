...
public class EntitySummonArgumentType implements ArgumentType<Identifier> {
   ...
   public static final DynamicCommandExceptionType NOT_FOUND_EXCEPTION = new DynamicCommandExceptionType((object) -> {
      return new TranslatableText("entity.notFound", new Object[]{object});
   });
   ...
   private static Identifier validate(Identifier identifier) throws CommandSyntaxException {
      Registry.ENTITY_TYPE.getOrEmpty(identifier).filter(EntityType::isSummonable).orElseThrow(() -> {
         return NOT_FOUND_EXCEPTION.create(identifier);
      });
      return identifier;
   }
