...
public class TestUtil {
   ...
   public static Collection<GameTestBatch> createBatches(Collection<TestFunction> testFunctions) {
      ...
      testFunctions.forEach((testFunction) -> {
         ...
         Collection<TestFunction> collection = (Collection)map.computeIfAbsent(string, (stringx) -> {
            return Lists.newArrayList();
         });
         collection.add(testFunction);
      });
      return (Collection)map.keySet().stream().flatMap((string) -> {
         ...
         return Streams.stream(Iterables.partition(collection, 100)).map((list) -> {
            return new GameTestBatch(string + ":" + atomicInteger.incrementAndGet(), collection, consumer);
         });
   ...
   private static ItemStack createBook(String structureName, boolean required, String message) {
      ...
      Arrays.stream(structureName.split("\\.")).forEach((string) -> {
   ...
   private static void sendMessage(ServerWorld world, Formatting formatting, String message) {
      world.getPlayers((serverPlayerEntity) -> {
         ...
         serverPlayerEntity.sendMessage((new LiteralText(message)).formatted(formatting));
      });
   }
   ...
   public static void clearTests(ServerWorld world, BlockPos pos, TestManager testManager, int radius) {
      ...
      BlockPos.stream(blockPos, blockPos2).filter((blockPosx) -> {
         return world.getBlockState(blockPosx).getBlock() == Blocks.STRUCTURE_BLOCK;
      }).forEach((blockPosx) -> {
