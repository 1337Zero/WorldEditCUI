...
public class TripwireBlock extends Block {
   ...
   public BlockState getStateForNeighborUpdate(BlockState state, Direction facing, BlockState neighborState, IWorld world, BlockPos pos, BlockPos neighborPos) {
      return facing.getAxis().isHorizontal() ? (BlockState)state.with((Property)FACING_PROPERTIES.get(facing), this.shouldConnectTo(neighborState, facing)) : super.getStateForNeighborUpdate(state, facing, neighborState, world, pos, neighborPos);
   }
   ...
   private void updatePowered(World world, BlockPos pos) {
      ...
      boolean bl2 = false;
      List<? extends Entity> list = world.getEntities((Entity)null, blockState.getOutlineShape(world, pos).getBoundingBox().offset(pos));
      if (!list.isEmpty()) {
