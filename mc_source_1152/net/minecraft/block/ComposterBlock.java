...
public class ComposterBlock extends Block implements InventoryProvider {
   ...
   static {
      ...
      LEVEL_TO_COLLISION_SHAPE = (VoxelShape[])Util.make(new VoxelShape[9], (voxelShapes) -> {
         for(int i = 0; i < 8; ++i) {
            voxelShapes[i] = VoxelShapes.combineAndSimplify(RAY_TRACE_SHAPE, Block.createCuboidShape(2.0D, (double)Math.max(2, 1 + i * 2), 2.0D, 14.0D, 16.0D, 14.0D), BooleanBiFunction.ONLY_FIRST);
         }
