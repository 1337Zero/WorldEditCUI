...
public class ScaffoldingBlock extends Block implements Waterloggable {
   ...
   public VoxelShape getCollisionShape(BlockState state, BlockView view, BlockPos pos, EntityContext ePos) {
      if (ePos.isAbove(VoxelShapes.fullCube(), pos, true) && !ePos.isDescending()) {
         ...
         return (Integer)state.get(DISTANCE) != 0 && (Boolean)state.get(BOTTOM) && ePos.isAbove(OUTLINE_SHAPE, pos, true) ? COLLISION_SHAPE : VoxelShapes.empty();
      }
   }
   ...
   public static int calculateDistance(BlockView world, BlockPos pos) {
      ...
      while(var5.hasNext()) {
         ...
         BlockState blockState2 = world.getBlockState(mutable.set((Vec3i)pos).setOffset(direction));
