...
public class Block implements ItemConvertible {
   ...
   private static final VoxelShape SOLID_MEDIUM_SQUARE_SHAPE;
   private static final VoxelShape SOLID_SMALL_SQUARE_SHAPE;
   ...
   public static VoxelShape createCuboidShape(double xMin, double yMin, double zMin, double xMax, double yMax, double zMax) {
      return VoxelShapes.cuboid(xMin / 16.0D, yMin / 16.0D, zMin / 16.0D, xMax / 16.0D, yMax / 16.0D, zMax / 16.0D);
   }
   ...
   public static void dropStacks(BlockState state, World world, BlockPos pos, @Nullable BlockEntity blockEntity) {
      if (world instanceof ServerWorld) {
         getDroppedStacks(state, (ServerWorld)world, pos, blockEntity).forEach((itemStack) -> {
            dropStack(world, pos, itemStack);
         });
      }
   ...
   public Vec3d getOffsetPos(BlockState state, BlockView view, BlockPos blockPos) {
      ...
      if (offsetType == Block.OffsetType.NONE) {
         ...
         return new Vec3d(((double)((float)(l & 15L) / 15.0F) - 0.5D) * 0.5D, offsetType == Block.OffsetType.XYZ ? ((double)((float)(l >> 4 & 15L) / 15.0F) - 1.0D) * 0.2D : 0.0D, ((double)((float)(l >> 8 & 15L) / 15.0F) - 0.5D) * 0.5D);
      }
   }
   ...
   static {
      ...
      FULL_CUBE_SHAPE_CACHE = CacheBuilder.newBuilder().maximumSize(512L).weakKeys().build(new CacheLoader<VoxelShape, Boolean>() {
         public Boolean load(VoxelShape voxelShape) {
            return !VoxelShapes.matchesAnywhere(VoxelShapes.fullCube(), voxelShape, BooleanBiFunction.NOT_SAME);
         }
      });
      SOLID_MEDIUM_SQUARE_SHAPE = VoxelShapes.combineAndSimplify(VoxelShapes.fullCube(), createCuboidShape(2.0D, 0.0D, 2.0D, 14.0D, 16.0D, 14.0D), BooleanBiFunction.ONLY_FIRST);
      SOLID_SMALL_SQUARE_SHAPE = createCuboidShape(7.0D, 0.0D, 7.0D, 9.0D, 10.0D, 9.0D);
