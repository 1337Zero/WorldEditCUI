...
public class PistonBlockEntity extends BlockEntity implements Tickable {
   ...
   private BlockState getHeadBlockState() {
      return !this.isExtending() && this.isSource() && this.pushedBlock.getBlock() instanceof PistonBlock ? (BlockState)((BlockState)Blocks.PISTON_HEAD.getDefaultState().with(PistonHeadBlock.TYPE, this.pushedBlock.getBlock() == Blocks.STICKY_PISTON ? PistonType.STICKY : PistonType.DEFAULT)).with(PistonHeadBlock.FACING, this.pushedBlock.get(PistonBlock.FACING)) : this.pushedBlock;
   }
   ...
   private void pushEntities(float nextProgress) {
      ...
      if (!voxelShape.isEmpty()) {
         ...
         List<Entity> list2 = this.world.getEntities((Entity)null, Boxes.stretch(box, direction, d).union(box));
         if (!list2.isEmpty()) {
   ...
   private void method_23674(float f) {
      if (this.method_23364()) {
         ...
         if (direction.getAxis().isHorizontal()) {
            ...
            List<Entity> list = this.world.getEntities((Entity)null, box, (entityx) -> {
   ...
   private Box getApproximateHeadBox(List<Box> boxes) {
      ...
      Box box;
      for(Iterator var14 = boxes.iterator(); var14.hasNext(); i = Math.max(box.z2, i)) {
         box = (Box)var14.next();
         d = Math.min(box.x1, d);
         e = Math.min(box.y1, e);
         f = Math.min(box.z1, f);
      ...
      if (this.lastProgress < 1.0F && this.world != null) {
         ...
         if (this.world.getBlockState(this.pos).getBlock() == Blocks.MOVING_PISTON) {
            ...
            if (this.source) {
               ...
               blockState2 = Block.getRenderingState(this.pushedBlock, this.world, this.pos);
            }
   ...
   public VoxelShape getCollisionShape(BlockView blockView, BlockPos blockPos) {
      ...
      if (!this.extending && this.source) {
         voxelShape2 = ((BlockState)this.pushedBlock.with(PistonBlock.EXTENDED, true)).getCollisionShape(blockView, blockPos);
      } else {
      ...
      if ((double)this.progress < 1.0D && direction == this.getMovementDirection()) {
         ...
         BlockState blockState2;
         if (this.isSource()) {
            blockState2 = (BlockState)((BlockState)Blocks.PISTON_HEAD.getDefaultState().with(PistonHeadBlock.FACING, this.facing)).with(PistonHeadBlock.SHORT, this.extending != 1.0F - this.progress < 4.0F);
         } else {
         ...
         return VoxelShapes.union(voxelShape2, blockState2.getCollisionShape(blockView, blockPos).offset(d, e, g));
      }
   }
