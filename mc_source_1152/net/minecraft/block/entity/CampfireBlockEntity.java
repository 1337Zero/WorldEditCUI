...
public class CampfireBlockEntity extends BlockEntity implements Clearable, Tickable {
      ...
      for(int i = 0; i < this.itemsBeingCooked.size(); ++i) {
         ...
         if (!itemStack.isEmpty()) {
            ...
            if (this.cookingTimes[i] >= this.cookingTotalTimes[i]) {
               Inventory inventory = new BasicInventory(new ItemStack[]{itemStack});
   ...
   public Optional<CampfireCookingRecipe> getRecipeFor(ItemStack item) {
      return this.itemsBeingCooked.stream().noneMatch(ItemStack::isEmpty) ? Optional.empty() : this.world.getRecipeManager().getFirstMatch(RecipeType.CAMPFIRE_COOKING, new BasicInventory(new ItemStack[]{item}), this.world);
   }
