...
public abstract class BlockEntity {
   ...
   public double getSquaredDistance(double x, double y, double z) {
      double d = (double)this.pos.getX() + 0.5D - x;
      double e = (double)this.pos.getY() + 0.5D - y;
      double f = (double)this.pos.getZ() + 0.5D - z;
      return d * d + e * e + f * f;
   }
   ...
   public void populateCrashReport(CrashReportSection crashReportSection) {
      crashReportSection.add("Name", () -> {
         return Registry.BLOCK_ENTITY_TYPE.getId(this.getType()) + " // " + this.getClass().getCanonicalName();
      });
      if (this.world != null) {
   ...
   public void markInvalid() {
      if (!this.invalid) {
         ...
         LOGGER.warn("Block entity invalid: {} @ {}", new Supplier[]{() -> {
            return Registry.BLOCK_ENTITY_TYPE.getId(this.getType());
         }, this::getPos});
      }
   }
}
