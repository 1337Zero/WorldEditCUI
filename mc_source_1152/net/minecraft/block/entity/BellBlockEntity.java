...
public class BellBlockEntity extends BlockEntity implements Tickable {
   ...
   private void method_20219() {
      ...
      if (!this.world.isClient) {
         Iterator var4 = this.field_19156.iterator();
   ...
   private boolean method_20523() {
      BlockPos blockPos = this.getPos();
      Iterator var2 = this.field_19156.iterator();
      ...
      } while(!livingEntity.isAlive() || livingEntity.removed || !blockPos.isWithinDistance(livingEntity.getPos(), 32.0D) || !livingEntity.getType().isTaggedWith(EntityTypeTags.RAIDERS));
   ...
   private void method_20218(World world) {
      if (world.isClient) {
         ...
         int i = (int)this.field_19156.stream().filter((livingEntity) -> {
         ...
         this.field_19156.stream().filter(this::method_20518).forEach((livingEntity) -> {
            ...
            for(int k = 0; k < j; ++k) {
               ...
               double h = (double)(atomicInteger.get() >> 16 & 255) / 255.0D;
               double l = (double)(atomicInteger.get() >> 8 & 255) / 255.0D;
               double m = (double)(atomicInteger.get() & 255) / 255.0D;
   ...
   private boolean method_20518(LivingEntity livingEntity) {
      return livingEntity.isAlive() && !livingEntity.removed && this.getPos().isWithinDistance(livingEntity.getPos(), 48.0D) && livingEntity.getType().isTaggedWith(EntityTypeTags.RAIDERS);
   }
