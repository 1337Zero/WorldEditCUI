...
public class BeehiveBlockEntity extends BlockEntity implements Tickable {
   ...
   private List<Entity> tryReleaseBee(BlockState state, BeehiveBlockEntity.BeeState beeState) {
      ...
      this.bees.removeIf((bee) -> {
         return this.releaseBee(state, bee.entityData, list, beeState);
      });
      return list;
   }
         ...
         if (bl && beeState != BeehiveBlockEntity.BeeState.EMERGENCY) {
            ...
            if (entity != null) {
               ...
               if (!entity.getType().isTaggedWith(EntityTypeTags.BEEHIVE_INHABITORS)) {
                  ...
                  this.world.playSound((PlayerEntity)null, (double)blockPos3.getX(), (double)blockPos3.getY(), (double)blockPos3.getZ(), SoundEvents.BLOCK_BEEHIVE_EXIT, SoundCategory.BLOCKS, 1.0F, 1.0F);
   ...
   private void tickBees() {
      ...
      while(iterator.hasNext()) {
         ...
         if (bee.ticksInHive > bee.minOccupationTIcks) {
            ...
            BeehiveBlockEntity.BeeState beeState = compoundTag.getBoolean("HasNectar") ? BeehiveBlockEntity.BeeState.HONEY_DELIVERED : BeehiveBlockEntity.BeeState.BEE_RELEASED;
            if (this.releaseBee(blockState, compoundTag, (List)null, beeState)) {
               iterator.remove();
            }
         } else {
            bee.ticksInHive++;
   ...
   public void fromTag(CompoundTag tag) {
      ...
      for(int i = 0; i < listTag.size(); ++i) {
         ...
         BeehiveBlockEntity.Bee bee = new BeehiveBlockEntity.Bee(compoundTag.getCompound("EntityData"), compoundTag.getInt("TicksInHive"), compoundTag.getInt("MinOccupationTicks"));
         this.bees.add(bee);
      }
