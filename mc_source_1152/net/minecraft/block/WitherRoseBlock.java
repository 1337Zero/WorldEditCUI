   ...
   public void randomDisplayTick(BlockState state, World world, BlockPos pos, Random random) {
      ...
      for(int i = 0; i < 3; ++i) {
         if (random.nextBoolean()) {
            world.addParticle(ParticleTypes.SMOKE, d + (double)(random.nextFloat() / 5.0F), (double)pos.getY() + (0.5D - (double)random.nextFloat()), e + (double)(random.nextFloat() / 5.0F), 0.0D, 0.0D, 0.0D);
         }
      }
