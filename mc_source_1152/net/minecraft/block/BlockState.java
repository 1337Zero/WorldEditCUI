...
public class BlockState extends AbstractState<Block, BlockState> implements State<BlockState> {
   ...
   public VoxelShape getCullingFace(BlockView view, BlockPos pos, Direction facing) {
      return this.shapeCache != null && this.shapeCache.extrudedFaces != null ? this.shapeCache.extrudedFaces[facing.ordinal()] : VoxelShapes.extrudeFace(this.getCullingShape(view, pos), facing);
   }
   ...
   public static <T> Dynamic<T> serialize(DynamicOps<T> ops, BlockState state) {
      ...
      if (immutableMap.isEmpty()) {
         object2 = ops.createMap(ImmutableMap.of(ops.createString("Name"), ops.createString(Registry.BLOCK.getId(state.getBlock()).toString())));
      } else {
         object2 = ops.createMap(ImmutableMap.of(ops.createString("Name"), ops.createString(Registry.BLOCK.getId(state.getBlock()).toString()), ops.createString("Properties"), ops.createMap((Map)immutableMap.entrySet().stream().map((entry) -> {
   ...
   public static <T> BlockState deserialize(Dynamic<T> dynamic) {
      ...
      var10004.getClass();
      Block block = (Block)var10000.get(new Identifier((String)var10003.flatMap(var10004::getStringValue).orElse("minecraft:air")));
      ...
      while(var5.hasNext()) {
         ...
         if (property != null) {
            blockState = (BlockState)State.tryRead(blockState, property, string, dynamic.toString(), (String)entry.getValue());
         }
      }
   ...
   static final class ShapeCache {
      ...
      private ShapeCache(BlockState state) {
         ...
         this.exceedsCube = Arrays.stream(Direction.Axis.values()).anyMatch((axis) -> {
            return this.collisionShape.getMinimum(axis) < 0.0D || this.collisionShape.getMaximum(axis) > 1.0D;
         });
         ...
         for(var5 = 0; var5 < var9; ++var5) {
            Direction direction2 = var8[var5];
