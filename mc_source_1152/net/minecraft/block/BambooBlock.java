...
public class BambooBlock extends Block implements Fertilizable {
   protected static final VoxelShape SMALL_LEAVES_SHAPE = Block.createCuboidShape(5.0D, 0.0D, 5.0D, 11.0D, 16.0D, 11.0D);
   protected static final VoxelShape LARGE_LEAVES_SHAPE = Block.createCuboidShape(3.0D, 0.0D, 3.0D, 13.0D, 16.0D, 13.0D);
   ...
   protected void updateLeaves(BlockState state, World world, BlockPos pos, Random random, int height) {
      ...
      int i = (Integer)state.get(AGE) != 1 && blockState2.getBlock() != Blocks.BAMBOO ? 0 : 1;
