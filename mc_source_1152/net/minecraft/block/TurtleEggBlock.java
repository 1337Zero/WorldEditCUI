...
public class TurtleEggBlock extends Block {
   ...
   public boolean canReplace(BlockState state, ItemPlacementContext ctx) {
      return ctx.getStack().getItem() == this.asItem() && (Integer)state.get(EGGS) < 4 ? true : super.canReplace(state, ctx);
   }

   @Nullable
   public BlockState getPlacementState(ItemPlacementContext ctx) {
      BlockState blockState = ctx.getWorld().getBlockState(ctx.getBlockPos());
      return blockState.getBlock() == this ? (BlockState)blockState.with(EGGS, Math.min(4, (Integer)blockState.get(EGGS) + 1)) : super.getPlacementState(ctx);
   }
