...
public class EnchantmentHelper {
      ...
      if (!stack.isEmpty()) {
         ...
         for(int i = 0; i < listTag.size(); ++i) {
            ...
            Registry.ENCHANTMENT.getOrEmpty(Identifier.tryParse(string)).ifPresent((enchantment) -> {
               enchantmentHandler.accept(enchantment, j);
            });
         }
   ...
   public static int getProtectionAmount(Iterable<ItemStack> equipment, DamageSource source) {
      ...
      accept((enchantment, i) -> {
         mutableInt.add(enchantment.getProtectionAmount(i, source));
      }, equipment);
   ...
   public static float getAttackDamage(ItemStack stack, EntityGroup group) {
      ...
      accept((enchantment, i) -> {
         mutableFloat.add(enchantment.getAttackDamage(i, group));
      }, stack);
   ...
   public static void onUserDamaged(LivingEntity user, Entity attacker) {
      EnchantmentHelper.Consumer consumer = (enchantment, i) -> {
   ...
   public static void onTargetDamaged(LivingEntity user, Entity target) {
      EnchantmentHelper.Consumer consumer = (enchantment, i) -> {
         enchantment.onTargetDamaged(user, target, i);
      };
      if (user != null) {
         accept(consumer, user.getItemsEquipped());
      }
   ...
   public static Entry<EquipmentSlot, ItemStack> getRandomEnchantedEquipment(Enchantment enchantment, LivingEntity livingEntity) {
      ...
      if (map.isEmpty()) {
         ...
         while(var4.hasNext()) {
            Entry<EquipmentSlot, ItemStack> entry = (Entry)var4.next();
   ...
   public static ItemStack enchant(Random random, ItemStack target, int level, boolean hasTreasure) {
      ...
      while(var6.hasNext()) {
         InfoEnchantment infoEnchantment = (InfoEnchantment)var6.next();
         if (bl) {
      ...
      if (i <= 0) {
         ...
         if (!list2.isEmpty()) {
            ...
            while(random.nextInt(50) <= level) {
               remove(list2, (InfoEnchantment)Util.getLast(list));
               if (list2.isEmpty()) {
                  break;
               }
   ...
   public static boolean contains(Collection<Enchantment> collection, Enchantment enchantment) {
      ...
      do {
         ...
         enchantment2 = (Enchantment)var2.next();
      } while(enchantment2.isDifferent(enchantment));
   ...
   public static List<InfoEnchantment> getHighestApplicableEnchantmentsAtPower(int power, ItemStack stack, boolean bl) {
      ...
      while(true) {
         while(true) {
            ...
            } while(!enchantment.type.isAcceptableItem(item) && !bl2);
