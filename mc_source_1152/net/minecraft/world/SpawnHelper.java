...
public final class SpawnHelper {
      ...
      if (k >= 1) {
         ...
         if (!blockState.isSimpleFullBlock(chunk, blockPos)) {
            ...
            while(m < 3) {
               ...
               int q = MathHelper.ceil(Math.random() * 4.0D);
               int r = 0;
               int s = 0;
                  ...
                  label115: {
                     if (s < q) {
                        label123: {
                           ...
                           PlayerEntity playerEntity = serverWorld.getClosestPlayer((double)f, (double)g, -1.0D);
                           if (playerEntity == null) {
                              break label115;
                           }

                           double d = playerEntity.squaredDistanceTo((double)f, (double)k, (double)g);
                           if (d <= 576.0D || spawnPos.isWithinDistance(new Vec3d((double)f, (double)k, (double)g), 24.0D)) {
                              break label115;
                           }
                           ...
                           if (spawnEntry.type.getCategory() == EntityCategory.MISC || !spawnEntry.type.isSpawnableFarFromPlayer() && d > 16384.0D) {
                              break label115;
                           }
                           ...
                           if (!canSpawn(location, serverWorld, mutable, entityType) || !SpawnRestriction.canSpawn(entityType, serverWorld, SpawnType.NATURAL, mutable, serverWorld.random) || !serverWorld.doesNotCollide(entityType.createSimpleBoundingBox((double)f, (double)k, (double)g))) {
                              break label115;
                           }
                           ...
                           try {
                              ...
                              mobEntity2 = (MobEntity)entity;
                           } catch (Exception var31) {
                              LOGGER.warn("Failed to create mob", var31);
                              return;
                           }

                           mobEntity2.refreshPositionAndAngles((double)f, (double)k, (double)g, serverWorld.random.nextFloat() * 360.0F, 0.0F);
                           if (d > 16384.0D && mobEntity2.canImmediatelyDespawn(d) || !mobEntity2.canSpawn(serverWorld, SpawnType.NATURAL) || !mobEntity2.canSpawn(serverWorld)) {
                              break label115;
                           }

                           entityData = mobEntity2.initialize(serverWorld, serverWorld.getLocalDifficulty(new BlockPos(mobEntity2)), SpawnType.NATURAL, entityData, (CompoundTag)null);
                           ++i;
                           ++r;
                           serverWorld.spawnEntity(mobEntity2);
                           if (i >= mobEntity2.getLimitPerChunk()) {
                              return;
                           }

                           if (!mobEntity2.spawnsTooManyForEachTry(r)) {
                              break label115;
                           }
                        }
                     }
   ...
   private static Biome.SpawnEntry pickRandomSpawnEntry(ChunkGenerator<?> chunkGenerator, EntityCategory entityCategory, Random random, BlockPos pos) {
      ...
      return list.isEmpty() ? null : (Biome.SpawnEntry)WeightedPicker.getRandom(random, list);
   }
      ...
      if (location == SpawnRestriction.Location.NO_RESTRICTIONS) {
         ...
         default:
            ...
            if (!blockState2.allowsSpawning(worldView, blockPos2, entityType)) {
               ...
               return isClearForSpawn(worldView, pos, blockState, fluidState) && isClearForSpawn(worldView, blockPos, worldView.getBlockState(blockPos), worldView.getFluidState(blockPos));
            }
         }
      } else {
         return false;
            ...
            for(int p = 0; p < k; ++p) {
               ...
               for(int q = 0; !bl && q < 4; ++q) {
                  ...
                  if (spawnEntry.type.isSummonable() && canSpawn(SpawnRestriction.Location.ON_GROUND, world, blockPos, spawnEntry.type)) {
                     ...
                     if (!world.doesNotCollide(spawnEntry.type.createSimpleBoundingBox(d, (double)blockPos.getY(), e)) || !SpawnRestriction.canSpawn(spawnEntry.type, world, SpawnType.CHUNK_GENERATION, new BlockPos(d, (double)blockPos.getY(), e), world.getRandom())) {
                        continue;
                     }
                     ...
                     if (entity2 instanceof MobEntity) {
                        ...
                        if (mobEntity.canSpawn(world, SpawnType.CHUNK_GENERATION) && mobEntity.canSpawn(world)) {
                           entityData = mobEntity.initialize(world, world.getLocalDifficulty(new BlockPos(mobEntity)), SpawnType.CHUNK_GENERATION, entityData, (CompoundTag)null);
