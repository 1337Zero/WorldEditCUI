...
public class SerializingRegionBasedStorage<R extends DynamicSerializable> implements AutoCloseable {
   ...
   protected R getOrCreate(long pos) {
      ...
      if (optional.isPresent()) {
         return (DynamicSerializable)optional.get();
      } else {
         R dynamicSerializable = (DynamicSerializable)this.factory.apply(() -> {
            this.onUpdate(pos);
         });
   ...
   private <T> void method_20368(ChunkPos chunkPos, DynamicOps<T> dynamicOps, @Nullable T object) {
      if (object == null) {
         ...
         for(int l = 0; l < 16; ++l) {
            ...
            Optional<R> optional = optionalDynamic.get(Integer.toString(l)).get().map((dynamicx) -> {
               return (DynamicSerializable)this.deserializer.apply(() -> {
                  this.onUpdate(m);
               }, dynamicx);
            });
