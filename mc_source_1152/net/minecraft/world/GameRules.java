...
public class GameRules {
   ...
   private static final Map<GameRules.RuleKey<?>, GameRules.RuleType<?>> RULE_TYPES = Maps.newTreeMap(Comparator.comparing((key) -> {
      return key.name;
   }));
   ...
   public static final GameRules.RuleKey<GameRules.BooleanRule> REDUCED_DEBUG_INFO = register("reducedDebugInfo", GameRules.BooleanRule.create(false, (server, rule) -> {
   ...
   public static final GameRules.RuleKey<GameRules.BooleanRule> SPECTATORS_GENERATE_CHUNKS = register("spectatorsGenerateChunks", GameRules.BooleanRule.create(true));
   public static final GameRules.RuleKey<GameRules.IntRule> SPAWN_RADIUS = register("spawnRadius", GameRules.IntRule.create(10));
   public static final GameRules.RuleKey<GameRules.BooleanRule> DISABLE_ELYTRA_MOVEMENT_CHECK = register("disableElytraMovementCheck", GameRules.BooleanRule.create(false));
   ...
   public static final GameRules.RuleKey<GameRules.BooleanRule> DO_IMMEDIATE_RESPAWN = register("doImmediateRespawn", GameRules.BooleanRule.create(false, (server, rule) -> {
   ...
   public GameRules() {
      this.rules = (Map)RULE_TYPES.entrySet().stream().collect(ImmutableMap.toImmutableMap(Entry::getKey, (e) -> {
         return ((GameRules.RuleType)e.getValue()).createRule();
      }));
   }
   ...
   public CompoundTag toNbt() {
      ...
      this.rules.forEach((key, rule) -> {
         compoundTag.putString(key.name, rule.serialize());
      });
      return compoundTag;
   }
