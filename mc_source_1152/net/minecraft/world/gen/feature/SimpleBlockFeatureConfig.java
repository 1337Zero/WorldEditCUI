...
public class SimpleBlockFeatureConfig implements FeatureConfig {
   ...
   public SimpleBlockFeatureConfig(BlockState toPlace, BlockState[] placeOn, BlockState[] placeIn, BlockState[] placeUnder) {
      this(toPlace, (List)Lists.newArrayList(placeOn), (List)Lists.newArrayList(placeIn), (List)Lists.newArrayList(placeUnder));
   }

   public <T> Dynamic<T> serialize(DynamicOps<T> ops) {
      T object = BlockState.serialize(ops, this.toPlace).getValue();
      T object2 = ops.createList(this.placeOn.stream().map((blockState) -> {
         return BlockState.serialize(ops, blockState).getValue();
      }));
      T object3 = ops.createList(this.placeIn.stream().map((blockState) -> {
         return BlockState.serialize(ops, blockState).getValue();
      }));
      T object4 = ops.createList(this.placeUnder.stream().map((blockState) -> {
         return BlockState.serialize(ops, blockState).getValue();
      }));
      return new Dynamic(ops, ops.createMap(ImmutableMap.of(ops.createString("to_place"), object, ops.createString("place_on"), object2, ops.createString("place_in"), object3, ops.createString("place_under"), object4)));
   }
