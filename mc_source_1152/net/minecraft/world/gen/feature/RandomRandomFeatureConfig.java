...
public class RandomRandomFeatureConfig implements FeatureConfig {
   ...
   public <T> Dynamic<T> serialize(DynamicOps<T> ops) {
      return new Dynamic(ops, ops.createMap(ImmutableMap.of(ops.createString("features"), ops.createList(this.features.stream().map((configuredFeature) -> {
         return configuredFeature.serialize(ops).getValue();
