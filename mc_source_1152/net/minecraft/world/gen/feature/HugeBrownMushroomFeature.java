   ...
   protected void generate(IWorld world, Random random, BlockPos blockPos, int i, BlockPos.Mutable pos, HugeMushroomFeatureConfig config) {
      ...
      for(int k = -j; k <= j; ++k) {
         for(int l = -j; l <= j; ++l) {
            ...
            boolean bl6 = bl3 || bl4;
            if (!bl5 || !bl6) {
               pos.set((Vec3i)blockPos).setOffset(k, i, l);
               if (!world.getBlockState(pos).isFullOpaque(world, pos)) {
                  ...
                  this.setBlockState(world, pos, (BlockState)((BlockState)((BlockState)((BlockState)config.capProvider.getBlockState(random, blockPos).with(MushroomBlock.WEST, bl7)).with(MushroomBlock.EAST, bl8)).with(MushroomBlock.NORTH, bl9)).with(MushroomBlock.SOUTH, bl10));
               }
            }
         }
      }
