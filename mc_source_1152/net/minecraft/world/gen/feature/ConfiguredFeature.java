...
public class ConfiguredFeature<FC extends FeatureConfig, F extends Feature<FC>> {
   ...
   public <T> Dynamic<T> serialize(DynamicOps<T> ops) {
      return new Dynamic(ops, ops.createMap(ImmutableMap.of(ops.createString("name"), ops.createString(Registry.FEATURE.getId(this.feature).toString()), ops.createString("config"), this.config.serialize(ops).getValue())));
   }
   ...
   public static <T> ConfiguredFeature<?, ?> deserialize(Dynamic<T> dynamic) {
      ...
      Feature feature = (Feature)Registry.FEATURE.get(new Identifier(string));
