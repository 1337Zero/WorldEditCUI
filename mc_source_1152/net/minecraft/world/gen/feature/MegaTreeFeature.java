...
public abstract class MegaTreeFeature<T extends TreeFeatureConfig> extends AbstractTreeFeature<T> {
   ...
   protected void method_23400(ModifiableTestableWorld modifiableTestableWorld, Random random, BlockPos blockPos, int i, Set<BlockPos> set, BlockBox blockBox, MegaTreeFeatureConfig megaTreeFeatureConfig) {
      ...
      for(int j = 0; j < i; ++j) {
         mutable.set((Vec3i)blockPos).setOffset(0, j, 0);
         ...
         if (j < i - 1) {
            mutable.set((Vec3i)blockPos).setOffset(1, j, 0);
            if (canTreeReplace(modifiableTestableWorld, mutable)) {
               this.setLogBlockState(modifiableTestableWorld, random, mutable, set, blockBox, megaTreeFeatureConfig);
            }

            mutable.set((Vec3i)blockPos).setOffset(1, j, 1);
            if (canTreeReplace(modifiableTestableWorld, mutable)) {
               this.setLogBlockState(modifiableTestableWorld, random, mutable, set, blockBox, megaTreeFeatureConfig);
            }

            mutable.set((Vec3i)blockPos).setOffset(0, j, 1);
            if (canTreeReplace(modifiableTestableWorld, mutable)) {
               this.setLogBlockState(modifiableTestableWorld, random, mutable, set, blockBox, megaTreeFeatureConfig);
            }
         }
