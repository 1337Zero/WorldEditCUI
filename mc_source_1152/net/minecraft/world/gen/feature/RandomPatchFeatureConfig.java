...
public class RandomPatchFeatureConfig implements FeatureConfig {
   ...
   public <T> Dynamic<T> serialize(DynamicOps<T> ops) {
      ...
      builder.put(ops.createString("state_provider"), this.stateProvider.serialize(ops)).put(ops.createString("block_placer"), this.blockPlacer.serialize(ops)).put(ops.createString("whitelist"), ops.createList(this.whitelist.stream().map((block) -> {
         return BlockState.serialize(ops, block.getDefaultState()).getValue();
      }))).put(ops.createString("blacklist"), ops.createList(this.blacklist.stream().map((blockState) -> {
         return BlockState.serialize(ops, blockState).getValue();
   ...
   public static <T> RandomPatchFeatureConfig deserialize(Dynamic<T> dynamic) {
      StateProviderType<?> stateProviderType = (StateProviderType)Registry.BLOCK_STATE_PROVIDER_TYPE.get(new Identifier((String)dynamic.get("state_provider").get("type").asString().orElseThrow(RuntimeException::new)));
      BlockPlacerType<?> blockPlacerType = (BlockPlacerType)Registry.BLOCK_PLACER_TYPE.get(new Identifier((String)dynamic.get("block_placer").get("type").asString().orElseThrow(RuntimeException::new)));
      return new RandomPatchFeatureConfig(stateProviderType.deserialize(dynamic.get("state_provider").orElseEmptyMap()), blockPlacerType.deserialize(dynamic.get("block_placer").orElseEmptyMap()), (Set)dynamic.get("whitelist").asList(BlockState::deserialize).stream().map(BlockState::getBlock).collect(Collectors.toSet()), Sets.newHashSet(dynamic.get("blacklist").asList(BlockState::deserialize)), dynamic.get("tries").asInt(128), dynamic.get("xspread").asInt(7), dynamic.get("yspread").asInt(3), dynamic.get("zspread").asInt(7), dynamic.get("can_replace").asBoolean(false), dynamic.get("project").asBoolean(true), dynamic.get("need_water").asBoolean(false));
   }
