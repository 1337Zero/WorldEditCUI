...
public class BeehiveTreeDecorator extends TreeDecorator {
   ...
   public void generate(IWorld world, Random random, List<BlockPos> list, List<BlockPos> list2, Set<BlockPos> set, BlockBox box) {
      if (random.nextFloat() < this.chance) {
         ...
         int i = !list2.isEmpty() ? Math.max(((BlockPos)list2.get(0)).getY() - 1, ((BlockPos)list.get(0)).getY()) : Math.min(((BlockPos)list.get(0)).getY() + 1 + random.nextInt(3), ((BlockPos)list.get(list.size() - 1)).getY());
         List<BlockPos> list3 = (List)list.stream().filter((blockPosx) -> {
            return blockPosx.getY() == i;
   ...
   public <T> T serialize(DynamicOps<T> ops) {
      return (new Dynamic(ops, ops.createMap(ImmutableMap.of(ops.createString("type"), ops.createString(Registry.TREE_DECORATOR_TYPE.getId(this.field_21319).toString()), ops.createString("probability"), ops.createFloat(this.chance))))).getValue();
   }
}
