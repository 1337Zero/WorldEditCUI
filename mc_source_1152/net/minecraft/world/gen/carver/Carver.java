...
public abstract class Carver<C extends CarverConfig> {
   ...
   private static <C extends CarverConfig, F extends Carver<C>> F register(String string, F carver) {
      return (Carver)Registry.register(Registry.CARVER, (String)string, carver);
   }
   ...
   protected boolean carveRegion(Chunk chunk, Function<BlockPos, Biome> function, long l, int i, int j, int k, double d, double e, double f, double g, double h, BitSet bitSet) {
      ...
      if (d >= m - 16.0D - g * 2.0D && f >= n - 16.0D - g * 2.0D && d <= m + 16.0D + g * 2.0D && f <= n + 16.0D + g * 2.0D) {
   ...
   protected boolean carveAtPoint(Chunk chunk, Function<BlockPos, Biome> function, BitSet bitSet, Random random, BlockPos.Mutable mutable, BlockPos.Mutable mutable2, BlockPos.Mutable mutable3, int mainChunkX, int mainChunkZ, int i, int j, int k, int l, int m, int n, AtomicBoolean atomicBoolean) {
      ...
      if (bitSet.get(o)) {
         ...
         BlockState blockState2 = chunk.getBlockState(mutable2.set((Vec3i)mutable).setOffset(Direction.UP));
         ...
         if (!this.canCarveBlock(blockState, blockState2)) {
            ...
            if (m < 11) {
               ...
               if (atomicBoolean.get()) {
                  mutable3.set((Vec3i)mutable).setOffset(Direction.DOWN);
   ...
   public abstract boolean carve(Chunk chunk, Function<BlockPos, Biome> function, Random random, int chunkX, int chunkZ, int mainChunkX, int mainChunkZ, int i, BitSet bitSet, C carverConfig);
