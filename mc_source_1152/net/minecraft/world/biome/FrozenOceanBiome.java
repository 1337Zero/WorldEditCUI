...
public final class FrozenOceanBiome extends Biome {
   ...
   public FrozenOceanBiome() {
      super((new Biome.Settings()).configureSurfaceBuilder(SurfaceBuilder.FROZEN_OCEAN, SurfaceBuilder.GRASS_CONFIG).precipitation(Biome.Precipitation.SNOW).category(Biome.Category.OCEAN).depth(-1.0F).scale(0.1F).temperature(0.0F).downfall(0.5F).waterColor(3750089).waterFogColor(329011).parent((String)null));
   ...
   protected float computeTemperature(BlockPos blockPos) {
      ...
      double d = field_9487.sample((double)blockPos.getX() * 0.05D, (double)blockPos.getZ() * 0.05D, false) * 7.0D;
