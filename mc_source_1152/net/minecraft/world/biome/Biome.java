...
public abstract class Biome {
   ...
   protected Biome(Biome.Settings settings) {
      if (settings.surfaceBuilder != null && settings.precipitation != null && settings.category != null && settings.depth != null && settings.scale != null && settings.temperature != null && settings.downfall != null && settings.waterColor != null && settings.waterFogColor != null) {
         ...
         GenerationStep.Feature[] var2 = GenerationStep.Feature.values();
         int var3 = var2.length;

         int var4;
         for(var4 = 0; var4 < var3; ++var4) {
            GenerationStep.Feature feature = var2[var4];
            this.features.put(feature, Lists.newArrayList());
         }

         EntityCategory[] var6 = EntityCategory.values();
         var3 = var6.length;

         for(var4 = 0; var4 < var3; ++var4) {
            EntityCategory entityCategory = var6[var4];
   ...
   public List<Biome.SpawnEntry> getEntitySpawnList(EntityCategory category) {
      return (List)this.spawns.get(category);
   }
   ...
   protected float computeTemperature(BlockPos blockPos) {
      if (blockPos.getY() > 64) {
         float f = (float)(TEMPERATURE_NOISE.sample((double)((float)blockPos.getX() / 8.0F), (double)((float)blockPos.getZ() / 8.0F), false) * 4.0D);
   ...
   public <C extends CarverConfig> void addCarver(GenerationStep.Carver step, ConfiguredCarver<C> configuredCarver) {
      ((List)this.carvers.computeIfAbsent(step, (carver) -> {
         return Lists.newArrayList();
      })).add(configuredCarver);
   }

   public List<ConfiguredCarver<?>> getCarversForStep(GenerationStep.Carver carver) {
      return (List)this.carvers.computeIfAbsent(carver, (carverx) -> {
         return Lists.newArrayList();
      });
   }
   ...
   public <C extends FeatureConfig> C getStructureFeatureConfig(StructureFeature<C> structureFeature) {
      return (FeatureConfig)this.structureFeatures.get(structureFeature);
   }
   ...
   public List<ConfiguredFeature<?, ?>> getFeaturesForStep(GenerationStep.Feature feature) {
      return (List)this.features.get(feature);
   }
   ...
   public void generateFeatureStep(GenerationStep.Feature step, ChunkGenerator<? extends ChunkGeneratorConfig> chunkGenerator, IWorld world, long seed, ChunkRandom random, BlockPos pos) {
      ...
      for(Iterator var9 = ((List)this.features.get(step)).iterator(); var9.hasNext(); ++i) {
         ...
         try {
            ...
            crashReport.addElement("Feature").add("Id", (Object)Registry.FEATURE.getId(configuredFeature.feature)).add("Description", () -> {
   ...
   public Biome.TemperatureGroup getTemperatureGroup() {
      if (this.category == Biome.Category.OCEAN) {
         ...
         return (double)this.getTemperature() < 1.0D ? Biome.TemperatureGroup.MEDIUM : Biome.TemperatureGroup.WARM;
      }
   }
   ...
   public static enum Precipitation {
      ...
      private static final Map<String, Biome.Precipitation> NAME_MAP = (Map)Arrays.stream(values()).collect(Collectors.toMap(Biome.Precipitation::getName, (precipitation) -> {
   ...
   public static enum Category {
      ...
      private static final Map<String, Biome.Category> NAME_MAP = (Map)Arrays.stream(values()).collect(Collectors.toMap(Biome.Category::getName, (category) -> {
         return category;
      }));
      private final String name;
   ...
   public static enum TemperatureGroup {
      ...
      private static final Map<String, Biome.TemperatureGroup> NAME_MAP = (Map)Arrays.stream(values()).collect(Collectors.toMap(Biome.TemperatureGroup::getName, (temperatureGroup) -> {
