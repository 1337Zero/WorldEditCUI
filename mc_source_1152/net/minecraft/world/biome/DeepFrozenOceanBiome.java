...
public class DeepFrozenOceanBiome extends Biome {
   ...
   public DeepFrozenOceanBiome() {
      super((new Biome.Settings()).configureSurfaceBuilder(SurfaceBuilder.FROZEN_OCEAN, SurfaceBuilder.GRASS_CONFIG).precipitation(Biome.Precipitation.RAIN).category(Biome.Category.OCEAN).depth(-1.8F).scale(0.1F).temperature(0.5F).downfall(0.5F).waterColor(3750089).waterFogColor(329011).parent((String)null));
   ...
   protected float computeTemperature(BlockPos blockPos) {
      ...
      double d = noiseSampler.sample((double)blockPos.getX() * 0.05D, (double)blockPos.getZ() * 0.05D, false) * 7.0D;
      double e = FOLIAGE_NOISE.sample((double)blockPos.getX() * 0.2D, (double)blockPos.getZ() * 0.2D, false);
      double g = d + e;
      if (g < 0.3D) {
         double h = FOLIAGE_NOISE.sample((double)blockPos.getX() * 0.09D, (double)blockPos.getZ() * 0.09D, false);
         if (h < 0.8D) {
            f = 0.2F;
         }
      }
      ...
      if (blockPos.getY() > 64) {
         float i = (float)(TEMPERATURE_NOISE.sample((double)((float)blockPos.getX() / 8.0F), (double)((float)blockPos.getZ() / 8.0F), false) * 4.0D);
