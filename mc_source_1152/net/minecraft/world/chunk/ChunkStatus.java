...
public class ChunkStatus {
   ...
   private static final ChunkStatus.NoGenTask STATUS_BUMP_NO_GEN_TASK;
   ...
   private static ChunkStatus register(String id, @Nullable ChunkStatus previous, int taskMargin, EnumSet<Heightmap.Type> heightMapTypes, ChunkStatus.ChunkType chunkType, ChunkStatus.Task task, ChunkStatus.NoGenTask noGenTask) {
      return (ChunkStatus)Registry.register(Registry.CHUNK_STATUS, (String)id, new ChunkStatus(id, previous, taskMargin, heightMapTypes, chunkType, task, noGenTask));
   }
   ...
   static {
      ...
      STATUS_BUMP_NO_GEN_TASK = (chunkStatus, serverWorld, structureManager, serverLightingProvider, function, chunk) -> {
      ...
      EMPTY = register("empty", (ChunkStatus)null, -1, PRE_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.PROTOCHUNK, (ChunkStatus.SimpleTask)((serverWorld, chunkGenerator, list, chunk) -> {
      }));
      STRUCTURE_STARTS = register("structure_starts", EMPTY, 0, PRE_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.PROTOCHUNK, (ChunkStatus.Task)((chunkStatus, serverWorld, chunkGenerator, structureManager, serverLightingProvider, function, list, chunk) -> {
         if (!chunk.getStatus().isAtLeast(chunkStatus)) {
      ...
      STRUCTURE_REFERENCES = register("structure_references", STRUCTURE_STARTS, 8, PRE_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.PROTOCHUNK, (ChunkStatus.SimpleTask)((serverWorld, chunkGenerator, list, chunk) -> {
         chunkGenerator.addStructureReferences(new ChunkRegion(serverWorld, list), chunk);
      }));
      BIOMES = register("biomes", STRUCTURE_REFERENCES, 0, PRE_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.PROTOCHUNK, (ChunkStatus.SimpleTask)((serverWorld, chunkGenerator, list, chunk) -> {
         chunkGenerator.populateBiomes(chunk);
      }));
      NOISE = register("noise", BIOMES, 8, PRE_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.PROTOCHUNK, (ChunkStatus.SimpleTask)((serverWorld, chunkGenerator, list, chunk) -> {
         chunkGenerator.populateNoise(new ChunkRegion(serverWorld, list), chunk);
      }));
      SURFACE = register("surface", NOISE, 0, PRE_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.PROTOCHUNK, (ChunkStatus.SimpleTask)((serverWorld, chunkGenerator, list, chunk) -> {
         chunkGenerator.buildSurface(new ChunkRegion(serverWorld, list), chunk);
      }));
      CARVERS = register("carvers", SURFACE, 0, PRE_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.PROTOCHUNK, (ChunkStatus.SimpleTask)((serverWorld, chunkGenerator, list, chunk) -> {
         chunkGenerator.carve(serverWorld.getBiomeAccess().withSource(chunkGenerator.getBiomeSource()), chunk, GenerationStep.Carver.AIR);
      }));
      LIQUID_CARVERS = register("liquid_carvers", CARVERS, 0, POST_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.PROTOCHUNK, (ChunkStatus.SimpleTask)((serverWorld, chunkGenerator, list, chunk) -> {
         chunkGenerator.carve(serverWorld.getBiomeAccess().withSource(chunkGenerator.getBiomeSource()), chunk, GenerationStep.Carver.LIQUID);
      }));
      FEATURES = register("features", LIQUID_CARVERS, 8, POST_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.PROTOCHUNK, (ChunkStatus.Task)((chunkStatus, serverWorld, chunkGenerator, structureManager, serverLightingProvider, function, list, chunk) -> {
      ...
      LIGHT = register("light", FEATURES, 1, POST_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.PROTOCHUNK, (chunkStatus, serverWorld, chunkGenerator, structureManager, serverLightingProvider, function, list, chunk) -> {
         return method_20610(chunkStatus, serverLightingProvider, chunk);
      }, (chunkStatus, serverWorld, structureManager, serverLightingProvider, function, chunk) -> {
         return method_20610(chunkStatus, serverLightingProvider, chunk);
      });
      SPAWN = register("spawn", LIGHT, 0, POST_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.PROTOCHUNK, (ChunkStatus.SimpleTask)((serverWorld, chunkGenerator, list, chunk) -> {
         chunkGenerator.populateEntities(new ChunkRegion(serverWorld, list));
      }));
      HEIGHTMAPS = register("heightmaps", SPAWN, 0, POST_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.PROTOCHUNK, (ChunkStatus.SimpleTask)((serverWorld, chunkGenerator, list, chunk) -> {
      }));
      FULL = register("full", HEIGHTMAPS, 0, POST_CARVER_HEIGHTMAPS, ChunkStatus.ChunkType.LEVELCHUNK, (chunkStatus, serverWorld, chunkGenerator, structureManager, serverLightingProvider, function, list, chunk) -> {
         return (CompletableFuture)function.apply(chunk);
      }, (chunkStatus, serverWorld, structureManager, serverLightingProvider, function, chunk) -> {
         return (CompletableFuture)function.apply(chunk);
      });
      DISTANCE_TO_TARGET_GENERATION_STATUS = ImmutableList.of(FULL, FEATURES, LIQUID_CARVERS, STRUCTURE_STARTS, STRUCTURE_STARTS, STRUCTURE_STARTS, STRUCTURE_STARTS, STRUCTURE_STARTS, STRUCTURE_STARTS, STRUCTURE_STARTS, STRUCTURE_STARTS);
      STATUS_TO_TARGET_GENERATION_RADIUS = (IntList)Util.make(new IntArrayList(createOrderedList().size()), (intArrayList) -> {
         int i = 0;
   ...
   interface SimpleTask extends ChunkStatus.Task {
      default CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> doWork(ChunkStatus targetStatus, ServerWorld serverWorld, ChunkGenerator<?> generator, StructureManager structureManager, ServerLightingProvider serverLightingProvider, Function<Chunk, CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>>> function, List<Chunk> list, Chunk chunk) {
   ...
   interface NoGenTask {
      CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> doWork(ChunkStatus chunkStatus, ServerWorld serverWorld, StructureManager structureManager, ServerLightingProvider serverLightingProvider, Function<Chunk, CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>>> function, Chunk chunk);
   }

   interface Task {
      CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>> doWork(ChunkStatus targetStatus, ServerWorld serverWorld, ChunkGenerator<?> generator, StructureManager structureManager, ServerLightingProvider serverLightingProvider, Function<Chunk, CompletableFuture<Either<Chunk, ChunkHolder.Unloaded>>> function, List<Chunk> list, Chunk chunk);
   }
}
