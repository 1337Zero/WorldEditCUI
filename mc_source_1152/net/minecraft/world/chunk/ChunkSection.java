...
public class ChunkSection {
   ...
   public BlockState getBlockState(int x, int y, int z) {
      return (BlockState)this.container.get(x, y, z);
   }
   ...
   public BlockState setBlockState(int x, int y, int z, BlockState state, boolean lock) {
      ...
      if (lock) {
         blockState2 = (BlockState)this.container.setSync(x, y, z, state);
      } else {
         blockState2 = (BlockState)this.container.set(x, y, z, state);
      }
   ...
   static {
      palette = new IdListPalette(Block.STATE_IDS, Blocks.AIR.getDefaultState());
   }
}
