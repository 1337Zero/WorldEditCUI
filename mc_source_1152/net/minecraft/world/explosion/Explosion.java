...
public class Explosion {
   ...
   public Explosion(World world, @Nullable Entity entity, double x, double y, double z, float power, boolean createFire, Explosion.DestructionType blockDestructionType) {
   ...
   public void collectBlocksAndDamageEntities() {
      ...
      k = MathHelper.floor(this.x - (double)r - 1.0D);
      l = MathHelper.floor(this.x + (double)r + 1.0D);
      int u = MathHelper.floor(this.y - (double)r - 1.0D);
      int v = MathHelper.floor(this.y + (double)r + 1.0D);
      int w = MathHelper.floor(this.z - (double)r - 1.0D);
      int x = MathHelper.floor(this.z + (double)r + 1.0D);
      List<Entity> list = this.world.getEntities(this.entity, new Box((double)k, (double)u, (double)w, (double)l, (double)v, (double)x));
         ...
         if (!entity.isImmuneToExplosion()) {
            ...
            if (z <= 1.0D) {
               ...
               if (ad != 0.0D) {
                  ...
                  double af = (1.0D - z) * ae;
                  entity.damage(this.getDamageSource(), (float)((int)((af * af + af) / 2.0D * 7.0D * (double)r + 1.0D)));
                  double ag = af;
