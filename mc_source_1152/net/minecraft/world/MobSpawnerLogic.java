...
public abstract class MobSpawnerLogic {
   ...
   private Identifier getEntityId() {
      ...
      try {
         ...
         LOGGER.warn("Invalid entity id '{}' at spawner {}:[{},{},{}]", string, this.getWorld().dimension.getType(), blockPos.getX(), blockPos.getY(), blockPos.getZ());
         return null;
      }
   }
      ...
      if (!this.isPlayerInRange()) {
         ...
         if (world.isClient) {
            ...
            while(true) {
               ...
               if (world.doesNotCollide(((EntityType)optional.get()).createSimpleBoundingBox(g, h, k)) && SpawnRestriction.canSpawn((EntityType)optional.get(), world.getWorld(), SpawnType.SPAWNER, new BlockPos(g, h, k), world.getRandom())) {
                  label97: {
   ...
   public Entity getRenderedEntity() {
      if (this.renderedEntity == null) {
         ...
         if (this.spawnEntry.getEntityTag().getSize() == 1 && this.spawnEntry.getEntityTag().contains("id", 8) && this.renderedEntity instanceof MobEntity) {
            ((MobEntity)this.renderedEntity).initialize(this.getWorld(), this.getWorld().getLocalDifficulty(new BlockPos(this.renderedEntity)), SpawnType.SPAWNER, (EntityData)null, (CompoundTag)null);
         }
      }
