...
public class PointOfInterestSet implements DynamicSerializable {
   ...
   public <T> PointOfInterestSet(Runnable updateListener, Dynamic<T> dynamic) {
      ...
      try {
         ...
         dynamic.get("Records").asStream().forEach((dynamicx) -> {
            this.add(new PointOfInterest(dynamicx, updateListener));
         });
   ...
   public <T> T serialize(DynamicOps<T> ops) {
      T object = ops.createList(this.pointsOfInterestByPos.values().stream().map((pointOfInterest) -> {
         return pointOfInterest.serialize(ops);
      }));
