...
public class LocationPredicate {
   ...
   public static LocationPredicate biome(Biome biome) {
      return new LocationPredicate(NumberRange.FloatRange.ANY, NumberRange.FloatRange.ANY, NumberRange.FloatRange.ANY, biome, (StructureFeature)null, (DimensionType)null, LightPredicate.ANY, BlockPredicate.ANY, FluidPredicate.ANY);
   }

   public static LocationPredicate dimension(DimensionType dimension) {
      return new LocationPredicate(NumberRange.FloatRange.ANY, NumberRange.FloatRange.ANY, NumberRange.FloatRange.ANY, (Biome)null, (StructureFeature)null, dimension, LightPredicate.ANY, BlockPredicate.ANY, FluidPredicate.ANY);
   }

   public static LocationPredicate feature(StructureFeature<?> feature) {
      return new LocationPredicate(NumberRange.FloatRange.ANY, NumberRange.FloatRange.ANY, NumberRange.FloatRange.ANY, (Biome)null, feature, (DimensionType)null, LightPredicate.ANY, BlockPredicate.ANY, FluidPredicate.ANY);
   }
   ...
   public boolean test(ServerWorld world, float x, float y, float z) {
      if (!this.x.test(x)) {
         ...
         if (this.biome != null && (!bl || this.biome != world.getBiome(blockPos))) {
            return false;
         } else if (this.feature != null && (!bl || !this.feature.isInsideStructure(world, blockPos))) {
            return false;
   ...
   public static LocationPredicate fromJson(@Nullable JsonElement element) {
      if (element != null && !element.isJsonNull()) {
         ...
         StructureFeature<?> structureFeature = jsonObject.has("feature") ? (StructureFeature)Feature.STRUCTURES.get(JsonHelper.getString(jsonObject, "feature")) : null;
         Biome biome = null;
         if (jsonObject.has("biome")) {
            ...
            biome = (Biome)Registry.BIOME.getOrEmpty(identifier).orElseThrow(() -> {
               return new JsonSyntaxException("Unknown biome '" + identifier + "'");
            });
         }
   ...
   static {
      ANY = new LocationPredicate(NumberRange.FloatRange.ANY, NumberRange.FloatRange.ANY, NumberRange.FloatRange.ANY, (Biome)null, (StructureFeature)null, (DimensionType)null, LightPredicate.ANY, BlockPredicate.ANY, FluidPredicate.ANY);
   }
