...
public class EntityEffectPredicate {
   ...
   public boolean test(Map<StatusEffect, StatusEffectInstance> map) {
      if (this == EMPTY) {
         ...
         } while(((EntityEffectPredicate.EffectData)entry.getValue()).test(statusEffectInstance));
   ...
   public static EntityEffectPredicate deserialize(@Nullable JsonElement jsonElement) {
      if (jsonElement != null && !jsonElement.isJsonNull()) {
         ...
         while(var3.hasNext()) {
            ...
            StatusEffect statusEffect = (StatusEffect)Registry.STATUS_EFFECT.getOrEmpty(identifier).orElseThrow(() -> {
               return new JsonSyntaxException("Unknown effect '" + identifier + "'");
            });
            EntityEffectPredicate.EffectData effectData = EntityEffectPredicate.EffectData.deserialize(JsonHelper.asObject((JsonElement)entry.getValue(), (String)entry.getKey()));
   ...
   public JsonElement serialize() {
      if (this == EMPTY) {
         ...
         while(var2.hasNext()) {
            Entry<StatusEffect, EntityEffectPredicate.EffectData> entry = (Entry)var2.next();
            jsonObject.add(Registry.STATUS_EFFECT.getId(entry.getKey()).toString(), ((EntityEffectPredicate.EffectData)entry.getValue()).serialize());
         }
   ...
   public static class EffectData {
      ...
      public EffectData(NumberRange.IntRange amplifier, NumberRange.IntRange duration, @Nullable Boolean ambient, @Nullable Boolean var4) {
      ...
      public static EntityEffectPredicate.EffectData deserialize(JsonObject json) {
         ...
         Boolean var3 = json.has("ambient") ? JsonHelper.getBoolean(json, "ambient") : null;
         Boolean boolean2 = json.has("visible") ? JsonHelper.getBoolean(json, "visible") : null;
         return new EntityEffectPredicate.EffectData(intRange, intRange2, var3, boolean2);
      }
   }
}
