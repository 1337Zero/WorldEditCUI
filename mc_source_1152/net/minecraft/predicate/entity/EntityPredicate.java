...
public class EntityPredicate {
   ...
   public static EntityPredicate fromJson(@Nullable JsonElement el) {
      if (el != null && !el.isJsonNull()) {
         ...
         return (new EntityPredicate.Builder()).type(entityTypePredicate).distance(distancePredicate).location(locationPredicate).effects(entityEffectPredicate).nbt(nbtPredicate).flags(entityFlagsPredicate).equipment(entityEquipmentPredicate).player(playerPredicate).team(string).catType(identifier).build();
      } else {
         return ANY;
      }
   }
   ...
   public static JsonElement serializeAll(EntityPredicate[] predicates) {
      if (predicates == EMPTY) {
         ...
         EntityPredicate[] var2 = predicates;
         int var3 = predicates.length;

         for(int var4 = 0; var4 < var3; ++var4) {
            EntityPredicate entityPredicate = var2[var4];
   ...
   static {
      ANY = new EntityPredicate(EntityTypePredicate.ANY, DistancePredicate.ANY, LocationPredicate.ANY, EntityEffectPredicate.EMPTY, NbtPredicate.ANY, EntityFlagsPredicate.ANY, EntityEquipmentPredicate.ANY, PlayerPredicate.ANY, (String)null, (Identifier)null);
