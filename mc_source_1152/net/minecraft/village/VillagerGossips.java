...
public class VillagerGossips {
   ...
   private Stream<VillagerGossips.GossipEntry> entries() {
      return this.entityReputation.entrySet().stream().flatMap((entry) -> {
         return ((VillagerGossips.Reputation)entry.getValue()).entriesFor((UUID)entry.getKey());
      });
   }
   ...
   private VillagerGossips.Reputation getReputationFor(UUID target) {
      return (VillagerGossips.Reputation)this.entityReputation.computeIfAbsent(target, (uUID) -> {
         return new VillagerGossips.Reputation();
      });
   }
   ...
   public void startGossip(UUID target, VillageGossipType type, int value) {
      ...
      reputation.associatedGossip.mergeInt(type, value, (integer, integer2) -> {
         return this.mergeReputation(type, integer, integer2);
      });
   ...
   public <T> Dynamic<T> serialize(DynamicOps<T> ops) {
      return new Dynamic(ops, ops.createList(this.entries().map((gossipEntry) -> {
         return gossipEntry.serialize(ops);
      }).map(Dynamic::getValue)));
   }

   public void deserialize(Dynamic<?> dynamic) {
      dynamic.asStream().map(VillagerGossips.GossipEntry::deserialize).flatMap(Util::stream).forEach((gossipEntry) -> {
         this.getReputationFor(gossipEntry.target).associatedGossip.put(gossipEntry.type, gossipEntry.value);
      });
   }
   ...
   static class Reputation {
      ...
      public Stream<VillagerGossips.GossipEntry> entriesFor(UUID target) {
         return this.associatedGossip.object2IntEntrySet().stream().map((entry) -> {
            return new VillagerGossips.GossipEntry(target, (VillageGossipType)entry.getKey(), entry.getIntValue());
         });
      }
   ...
   static class GossipEntry {
      ...
      public <T> Dynamic<T> serialize(DynamicOps<T> ops) {
         return Util.writeUuid("Target", this.target, new Dynamic(ops, ops.createMap(ImmutableMap.of(ops.createString("Type"), ops.createString(this.type.key), ops.createString("Value"), ops.createInt(this.value)))));
      }

      public static Optional<VillagerGossips.GossipEntry> deserialize(Dynamic<?> dynamic) {
         return dynamic.get("Type").asString().map(VillageGossipType::byKey).flatMap((villageGossipType) -> {
            return Util.readUuid("Target", dynamic).flatMap((uUID) -> {
               return dynamic.get("Value").asNumber().map((number) -> {
                  return new VillagerGossips.GossipEntry(uUID, villageGossipType, number.intValue());
               });
            });
         });
      }
