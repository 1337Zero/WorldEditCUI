...
public class ZombieSiegeManager {
   ...
   private boolean spawn(ServerWorld serverWorld) {
      Iterator var2 = serverWorld.getPlayers().iterator();

      while(var2.hasNext()) {
         PlayerEntity playerEntity = (PlayerEntity)var2.next();
         if (!playerEntity.isSpectator()) {
            BlockPos blockPos = playerEntity.getBlockPos();
            if (serverWorld.isNearOccupiedPointOfInterest(blockPos) && serverWorld.getBiome(blockPos).getCategory() != Biome.Category.MUSHROOM) {
               for(int i = 0; i < 10; ++i) {
                  float f = serverWorld.random.nextFloat() * 6.2831855F;
   ...
   private void trySpawnZombie(ServerWorld serverWorld) {
      ...
      if (vec3d != null) {
         ...
         try {
            zombieEntity2 = new ZombieEntity(serverWorld);
            zombieEntity2.initialize(serverWorld, serverWorld.getLocalDifficulty(new BlockPos(zombieEntity2)), SpawnType.EVENT, (EntityData)null, (CompoundTag)null);
         } catch (Exception var5) {
            var5.printStackTrace();
            return;
         }

         zombieEntity2.refreshPositionAndAngles(vec3d.x, vec3d.y, vec3d.z, serverWorld.random.nextFloat() * 360.0F, 0.0F);
         serverWorld.spawnEntity(zombieEntity2);
      }
   }
   ...
   private Vec3d getSpawnVector(ServerWorld serverWorld, BlockPos blockPos) {
      for(int i = 0; i < 10; ++i) {
         ...
         if (serverWorld.isNearOccupiedPointOfInterest(blockPos2) && HostileEntity.canSpawnInDark(EntityType.ZOMBIE, serverWorld, SpawnType.EVENT, blockPos2, serverWorld.random)) {
            return new Vec3d((double)blockPos2.getX() + 0.5D, (double)blockPos2.getY(), (double)blockPos2.getZ() + 0.5D);
         }
      }
