...
public class RegistryTagManager implements ResourceReloadListener {
   ...
   private final RegistryTagContainer<EntityType<?>> entityTypes;

   public RegistryTagManager() {
      this.blocks = new RegistryTagContainer(Registry.BLOCK, "tags/blocks", "block");
      this.items = new RegistryTagContainer(Registry.ITEM, "tags/items", "item");
      this.fluids = new RegistryTagContainer(Registry.FLUID, "tags/fluids", "fluid");
      this.entityTypes = new RegistryTagContainer(Registry.ENTITY_TYPE, "tags/entity_types", "entity_type");
   }
   ...
   public CompletableFuture<Void> reload(ResourceReloadListener.Synchronizer synchronizer, ResourceManager manager, Profiler prepareProfiler, Profiler applyProfiler, Executor prepareExecutor, Executor applyExecutor) {
      ...
      CompletableFuture var10000 = completableFuture.thenCombine(completableFuture2, Pair::of).thenCombine(completableFuture3.thenCombine(completableFuture4, Pair::of), (pair, pair2) -> {
         return new RegistryTagManager.BuilderHolder((Map)pair.getFirst(), (Map)pair.getSecond(), (Map)pair2.getFirst(), (Map)pair2.getSecond());
      });
      synchronizer.getClass();
      return var10000.thenCompose(synchronizer::whenPrepared).thenAcceptAsync((builderHolder) -> {
