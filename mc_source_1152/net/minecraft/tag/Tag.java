...
public class Tag<T> {
   ...
   public Tag(Identifier id, Collection<Tag.Entry<T>> entries, boolean ordered) {
      this.id = id;
      this.values = (Set)(ordered ? Sets.newLinkedHashSet() : Sets.newHashSet());
      this.entries = entries;
      Iterator var4 = entries.iterator();

      while(var4.hasNext()) {
         Tag.Entry<T> entry = (Tag.Entry)var4.next();
   ...
   public static class TagEntry<T> implements Tag.Entry<T> {
      ...
      public void build(Collection<T> collection) {
         if (this.tag == null) {
            throw (IllegalStateException)Util.throwOrPause(new IllegalStateException("Cannot build unresolved tag entry"));
         } else {
   ...
   public static class Builder<T> {
      ...
      public boolean applyTagGetter(Function<Identifier, Tag<T>> function) {
         ...
         do {
            ...
            entry = (Tag.Entry)var2.next();
         } while(entry.applyTagGetter(function));
      ...
      public Tag.Builder<T> fromJson(Function<Identifier, Optional<T>> function, JsonObject jsonObject) {
         ...
         while(var5.hasNext()) {
            ...
            if (string.startsWith("#")) {
               ...
               list.add(new Tag.CollectionEntry(Collections.singleton(((Optional)function.apply(identifier)).orElseThrow(() -> {
                  return new JsonParseException("Unknown value '" + identifier + "'");
               }))));
            }
         }
