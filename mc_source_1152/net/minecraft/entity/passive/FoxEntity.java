...
public class FoxEntity extends AnimalEntity {
   ...
   protected void initGoals() {
      ...
      this.goalSelector.add(2, new FoxEntity.EscapeWhenNotAggresiveGoal(2.2D));
   ...
   private void spit(ItemStack itemStack) {
      if (!itemStack.isEmpty() && !this.world.isClient) {
         ItemEntity itemEntity = new ItemEntity(this.world, this.getX() + this.getRotationVector().x, this.getY() + 1.0D, this.getZ() + this.getRotationVector().z, itemStack);
   ...
   protected SoundEvent getAmbientSound() {
      if (this.isSleeping()) {
         ...
         if (!this.world.isDay() && this.random.nextFloat() < 0.1F) {
            List<PlayerEntity> list = this.world.getEntities(PlayerEntity.class, this.getBoundingBox().expand(16.0D, 16.0D, 16.0D), EntityPredicates.EXCEPT_SPECTATOR);
            if (list.isEmpty()) {
   ...
   class FoxSwimGoal extends SwimGoal {
      ...
      public boolean canStart() {
         return FoxEntity.this.isTouchingWater() && FoxEntity.this.getWaterHeight() > 0.25D || FoxEntity.this.isInLava();
      }
   }
      ...
      public boolean canStart() {
         if (!FoxEntity.this.getEquippedStack(EquipmentSlot.MAINHAND).isEmpty()) {
            ...
            if (!FoxEntity.this.wantsToPickupItem()) {
               ...
               List<ItemEntity> list = FoxEntity.this.world.getEntities(ItemEntity.class, FoxEntity.this.getBoundingBox().expand(8.0D, 8.0D, 8.0D), FoxEntity.PICKABLE_DROP_FILTER);
