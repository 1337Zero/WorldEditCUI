...
public abstract class HorseBaseEntity extends AnimalEntity implements InventoryListener, JumpingMount {
   ...
   private void playEatingAnimation() {
      this.setEating();
      if (!this.isSilent()) {
         this.world.playSound((PlayerEntity)null, this.getX(), this.getY(), this.getZ(), SoundEvents.ENTITY_HORSE_EAT, this.getSoundCategory(), 1.0F, 1.0F + (this.random.nextFloat() - this.random.nextFloat()) * 0.2F);
      }
   ...
   protected void initAttributes() {
      ...
      this.getAttributeInstance(EntityAttributes.MAX_HEALTH).setBaseValue(53.0D);
      this.getAttributeInstance(EntityAttributes.MOVEMENT_SPEED).setBaseValue(0.22499999403953552D);
   }
      ...
      if (this.isAlive()) {
         if (this.hasPassengers() && this.canBeControlledByRider() && this.isSaddled()) {
            ...
            if (this.jumpStrength > 0.0F && !this.isInAir() && this.onGround) {
               k = this.getJumpStrength() * (double)this.jumpStrength * (double)this.getJumpVelocityMultiplier();
               if (this.hasStatusEffect(StatusEffects.JUMP_BOOST)) {
                  l = k + (double)((float)(this.getStatusEffect(StatusEffects.JUMP_BOOST).getAmplifier() + 1) * 0.1F);
               } else {
                  l = k;
               }

               Vec3d vec3d = this.getVelocity();
               this.setVelocity(vec3d.x, l, vec3d.z);
               this.setInAir(true);
            ...
            k = this.getX() - this.prevX;
            l = this.getZ() - this.prevZ;
   ...
   public void readCustomDataFromTag(CompoundTag tag) {
      ...
      if (entityAttributeInstance != null) {
         this.getAttributeInstance(EntityAttributes.MOVEMENT_SPEED).setBaseValue(entityAttributeInstance.getBaseValue() * 0.25D);
      }
   ...
   protected void setChildAttributes(PassiveEntity mate, HorseBaseEntity child) {
      ...
      child.getAttributeInstance(EntityAttributes.MAX_HEALTH).setBaseValue(d / 3.0D);
      ...
      child.getAttributeInstance(JUMP_STRENGTH).setBaseValue(e / 3.0D);
      ...
      child.getAttributeInstance(EntityAttributes.MOVEMENT_SPEED).setBaseValue(f / 3.0D);
   }
   ...
   static {
      ...
      JUMP_STRENGTH = (new ClampedEntityAttribute((EntityAttribute)null, "horse.jumpStrength", 0.7D, 0.0D, 2.0D)).setName("Jump Strength").setTracked(true);
