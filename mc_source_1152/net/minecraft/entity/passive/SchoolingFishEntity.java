...
public abstract class SchoolingFishEntity extends FishEntity {
   ...
   public void tick() {
      ...
      if (this.hasOtherFishInGroup() && this.world.random.nextInt(200) == 1) {
         List<FishEntity> list = this.world.getNonSpectatingEntities(this.getClass(), this.getBoundingBox().expand(8.0D, 8.0D, 8.0D));
         if (list.size() <= 1) {
            this.groupSize = 1;
         }
      }
   ...
   public void pullInOtherFish(Stream<SchoolingFishEntity> fish) {
      fish.limit((long)(this.getMaxGroupSize() - this.groupSize)).filter((schoolingFishEntity) -> {
         return schoolingFishEntity != this;
      }).forEach((schoolingFishEntity) -> {
         schoolingFishEntity.joinGroupOf(this);
      });
   }

   @Nullable
   public EntityData initialize(IWorld world, LocalDifficulty difficulty, SpawnType spawnType, @Nullable EntityData entityData, @Nullable CompoundTag entityTag) {
      super.initialize(world, difficulty, spawnType, (EntityData)entityData, entityTag);
      if (entityData == null) {
