...
public class ParrotEntity extends TameableShoulderEntity implements Flutterer {
   ...
   public void tickMovement() {
      ...
      if (this.songSource == null || !this.songSource.isWithinDistance(this.getPos(), 3.46D) || this.world.getBlockState(this.songSource).getBlock() != Blocks.JUKEBOX) {
   ...
   private static boolean imitateNearbyMob(World world, Entity parrot) {
      if (parrot.isAlive() && !parrot.isSilent() && world.random.nextInt(50) == 0) {
         List<MobEntity> list = world.getEntities(MobEntity.class, parrot.getBoundingBox().expand(20.0D), CAN_IMITATE);
         if (!list.isEmpty()) {
            ...
            if (!mobEntity.isSilent()) {
               ...
               world.playSound((PlayerEntity)null, parrot.getX(), parrot.getY(), parrot.getZ(), soundEvent, parrot.getSoundCategory(), 0.7F, getSoundPitch(world.random));
               return true;
            }
         }
   ...
   public static void playMobSound(World world, Entity parrot) {
      if (!parrot.isSilent() && !imitateNearbyMob(world, parrot) && world.random.nextInt(200) == 0) {
         world.playSound((PlayerEntity)null, parrot.getX(), parrot.getY(), parrot.getZ(), getRandomSound(world.random), parrot.getSoundCategory(), 1.0F, getSoundPitch(world.random));
      }
   ...
   public int getVariant() {
      return MathHelper.clamp((Integer)this.dataTracker.get(ATTR_VARIANT), 0, 4);
   }
   ...
   static {
      ATTR_VARIANT = DataTracker.registerData(ParrotEntity.class, TrackedDataHandlerRegistry.INTEGER);
      ...
      MOB_SOUNDS = (Map)Util.make(Maps.newHashMap(), (hashMap) -> {
