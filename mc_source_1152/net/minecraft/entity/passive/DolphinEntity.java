...
public class DolphinEntity extends WaterCreatureEntity {
   ...
   protected void initAttributes() {
      ...
      this.getAttributeInstance(EntityAttributes.MOVEMENT_SPEED).setBaseValue(1.2000000476837158D);
   ...
   static class LeadToNearbyTreasureGoal extends Goal {
      ...
      public boolean shouldContinue() {
         ...
         return !(new BlockPos((double)blockPos.getX(), this.dolphin.getY(), (double)blockPos.getZ())).isWithinDistance(this.dolphin.getPos(), 4.0D) && !this.field_6753 && this.dolphin.getAir() >= 100;
      }
      ...
      public void start() {
         if (this.dolphin.world instanceof ServerWorld) {
            ...
            String string = (double)serverWorld.random.nextFloat() >= 0.5D ? "Ocean_Ruin" : "Shipwreck";
      ...
      public void stop() {
         BlockPos blockPos = this.dolphin.getTreasurePos();
         if ((new BlockPos((double)blockPos.getX(), this.dolphin.getY(), (double)blockPos.getZ())).isWithinDistance(this.dolphin.getPos(), 4.0D) || this.field_6753) {
      ...
      public void tick() {
         ...
         if (this.dolphin.isCloseToTarget() || this.dolphin.getNavigation().isIdle()) {
            ...
            if (vec3d2 != null) {
               ...
               if (!world.getFluidState(blockPos).matches(FluidTags.WATER) || !world.getBlockState(blockPos).canPlaceAtSide(world, blockPos, BlockPlacementEnvironment.WATER)) {
   ...
   class PlayWithItemsGoal extends Goal {
      ...
      public boolean canStart() {
         if (this.field_6758 > DolphinEntity.this.age) {
            ...
            List<ItemEntity> list = DolphinEntity.this.world.getEntities(ItemEntity.class, DolphinEntity.this.getBoundingBox().expand(8.0D, 8.0D, 8.0D), DolphinEntity.CAN_TAKE);
      ...
      private void spitOutItem(ItemStack stack) {
         if (!stack.isEmpty()) {
            ...
            itemEntity.setVelocity((double)(0.3F * -MathHelper.sin(DolphinEntity.this.yaw * 0.017453292F) * MathHelper.cos(DolphinEntity.this.pitch * 0.017453292F) + MathHelper.cos(g) * h), (double)(0.3F * MathHelper.sin(DolphinEntity.this.pitch * 0.017453292F) * 1.5F), (double)(0.3F * MathHelper.cos(DolphinEntity.this.yaw * 0.017453292F) * MathHelper.cos(DolphinEntity.this.pitch * 0.017453292F) + MathHelper.sin(g) * h));
