...
public class BeeEntity extends AnimalEntity implements Flutterer {
   ...
   public void tickMovement() {
      ...
      if (!this.world.isClient) {
         ...
         boolean bl = this.isAngry() && !this.hasStung() && this.getTarget() != null && this.getTarget().squaredDistanceTo(this) < 4.0D;
   ...
   public BeeEntity createChild(PassiveEntity passiveEntity) {
      return (BeeEntity)EntityType.BEE.create(this.world);
   }
   ...
   static {
      multipleByteTracker = DataTracker.registerData(BeeEntity.class, TrackedDataHandlerRegistry.BYTE);
   ...
   class FindHiveGoal extends BeeEntity.NotAngryGoal {
      ...
      private List<BlockPos> getNearbyFreeHives() {
         ...
         Stream<PointOfInterest> stream = pointOfInterestStorage.get((pointOfInterestType) -> {
   ...
   class PollinateGoal extends BeeEntity.NotAngryGoal {
      ...
      public void tick() {
         ...
         if (this.ticks > 600) {
            ...
            Vec3d vec3d = (new Vec3d(BeeEntity.this.flowerPos)).add(0.5D, 0.6000000238418579D, 0.5D);
            ...
            for(int j = 0; (double)j < searchDistance; ++j) {
               for(int k = 0; k <= j; k = k > 0 ? -k : 1 - k) {
                  for(int l = k < j && k > -j ? j : 0; l <= j; l = l > 0 ? -l : 1 - l) {
                     mutable.set((Vec3i)blockPos).setOffset(k, i - 1, l);
