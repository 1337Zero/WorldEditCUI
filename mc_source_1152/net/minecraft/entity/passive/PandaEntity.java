...
public class PandaEntity extends AnimalEntity {
   ...
   private static final TargetPredicate field_21803;
   ...
   public PassiveEntity createChild(PassiveEntity mate) {
      PandaEntity pandaEntity = (PandaEntity)EntityType.PANDA.create(this.world);
   ...
   protected void initAttributes() {
      ...
      this.getAttributeInstance(EntityAttributes.MOVEMENT_SPEED).setBaseValue(0.15000000596046448D);
   ...
   static {
      ...
      field_21803 = (new TargetPredicate()).setBaseMaxDistance(8.0D).includeTeammates().includeInvulnerable();
      IS_FOOD = (itemEntity) -> {
   ...
   class PickUpFoodGoal extends Goal {
      ...
      public boolean canStart() {
         if (this.startAge <= PandaEntity.this.age && !PandaEntity.this.isBaby() && !PandaEntity.this.isTouchingWater() && PandaEntity.this.method_18442() && PandaEntity.this.getAskForBambooTicks() <= 0) {
            List<ItemEntity> list = PandaEntity.this.world.getEntities(ItemEntity.class, PandaEntity.this.getBoundingBox().expand(6.0D, 6.0D, 6.0D), PandaEntity.IS_FOOD);
      ...
      public void start() {
         List<ItemEntity> list = PandaEntity.this.world.getEntities(ItemEntity.class, PandaEntity.this.getBoundingBox().expand(8.0D, 8.0D, 8.0D), PandaEntity.IS_FOOD);
            ...
            for(int j = 0; j < 8; ++j) {
               for(int k = 0; k <= j; k = k > 0 ? -k : 1 - k) {
                  for(int l = k < j && k > -j ? j : 0; l <= j; l = l > 0 ? -l : 1 - l) {
                     mutable.set((Vec3i)blockPos).setOffset(k, i, l);
