...
public class LlamaEntity extends AbstractDonkeyEntity implements RangedAttackMob {
   ...
   public int getStrength() {
      return (Integer)this.dataTracker.get(ATTR_STRENGTH);
   }
   ...
   public int getVariant() {
      return MathHelper.clamp((Integer)this.dataTracker.get(ATTR_VARIANT), 0, 3);
   }
   ...
   public net.minecraft.entity.EntityData initialize(IWorld world, LocalDifficulty difficulty, SpawnType spawnType, @Nullable net.minecraft.entity.EntityData entityData, @Nullable CompoundTag entityTag) {
      ...
      if (entityData instanceof LlamaEntity.EntityData) {
         j = ((LlamaEntity.EntityData)entityData).variant;
      } else {
         j = this.random.nextInt(4);
         entityData = new LlamaEntity.EntityData(j);
      }
   ...
   static {
      ATTR_STRENGTH = DataTracker.registerData(LlamaEntity.class, TrackedDataHandlerRegistry.INTEGER);
      CARPET_COLOR = DataTracker.registerData(LlamaEntity.class, TrackedDataHandlerRegistry.INTEGER);
      ATTR_VARIANT = DataTracker.registerData(LlamaEntity.class, TrackedDataHandlerRegistry.INTEGER);
   }

   static class ChaseWolvesGoal extends FollowTargetGoal<WolfEntity> {
      public ChaseWolvesGoal(LlamaEntity llama) {
         super(llama, WolfEntity.class, 16, false, true, (livingEntity) -> {
