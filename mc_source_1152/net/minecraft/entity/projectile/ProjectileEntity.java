...
public abstract class ProjectileEntity extends Entity implements Projectile {
   ...
   protected static final TrackedData<Optional<UUID>> OPTIONAL_UUID;
   private static final TrackedData<Byte> PIERCE_LEVEL;
   @Nullable
   private BlockState inBlockState;
   protected boolean inGround;
   protected int inGroundTime;
   public ProjectileEntity.PickupPermission pickupType;
   public int shake;
   ...
   protected ProjectileEntity(EntityType<? extends ProjectileEntity> type, World world) {
      super(type, world);
      this.pickupType = ProjectileEntity.PickupPermission.DISALLOWED;
      this.damage = 2.0D;
   ...
   public void setProperties(Entity user, float pitch, float yaw, float roll, float modifierZ, float modifierXYZ) {
      ...
      this.setVelocity(this.getVelocity().add(user.getVelocity().x, user.onGround ? 0.0D : user.getVelocity().y, user.getVelocity().z));
   }
   ...
   public void tick() {
      ...
      if (this.inGround && !bl) {
         if (this.inBlockState != blockState && this.world.doesNotCollide(this.getBoundingBox().expand(0.06D))) {
            this.inGround = false;
   ...
   protected EntityHitResult getEntityCollision(Vec3d currentPosition, Vec3d nextPosition) {
      return ProjectileUtil.getEntityCollision(this.world, this, currentPosition, nextPosition, this.getBoundingBox().stretch(this.getVelocity()).expand(1.0D), (entity) -> {
         return !entity.isSpectator() && entity.isAlive() && entity.collides() && (entity != this.getOwner() || this.flyingTick >= 5) && (this.piercedEntities == null || !this.piercedEntities.contains(entity.getEntityId()));
      });
   }
   ...
   public void setOwner(@Nullable Entity entity) {
      ...
      if (entity instanceof PlayerEntity) {
         this.pickupType = ((PlayerEntity)entity).abilities.creativeMode ? ProjectileEntity.PickupPermission.CREATIVE_ONLY : ProjectileEntity.PickupPermission.ALLOWED;
      }
   ...
   public void onPlayerCollision(PlayerEntity player) {
      if (!this.world.isClient && (this.inGround || this.isNoClip()) && this.shake <= 0) {
         boolean bl = this.pickupType == ProjectileEntity.PickupPermission.ALLOWED || this.pickupType == ProjectileEntity.PickupPermission.CREATIVE_ONLY && player.abilities.creativeMode || this.isNoClip() && this.getOwner().getUuid() == player.getUuid();
   ...
   static {
      PROJECTILE_FLAGS = DataTracker.registerData(ProjectileEntity.class, TrackedDataHandlerRegistry.BYTE);
      OPTIONAL_UUID = DataTracker.registerData(ProjectileEntity.class, TrackedDataHandlerRegistry.OPTIONAL_UUID);
      PIERCE_LEVEL = DataTracker.registerData(ProjectileEntity.class, TrackedDataHandlerRegistry.BYTE);
   }
