...
public abstract class AbstractDecorationEntity extends Entity {
   ...
   protected void method_6895() {
      if (this.facing != null) {
         double d = (double)this.blockPos.getX() + 0.5D;
         double e = (double)this.blockPos.getY() + 0.5D;
         double f = (double)this.blockPos.getZ() + 0.5D;
         double g = 0.46875D;
      ...
      if (!this.world.doesNotCollide(this)) {
         ...
         for(int k = 0; k < i; ++k) {
            for(int l = 0; l < j; ++l) {
               int m = (i - 1) / -2;
               int n = (j - 1) / -2;
               mutable.set((Vec3i)blockPos).setOffset(direction, k + m).setOffset(Direction.UP, l + n);
         ...
         return this.world.getEntities((Entity)this, this.getBoundingBox(), PREDICATE).isEmpty();
      }
   }
