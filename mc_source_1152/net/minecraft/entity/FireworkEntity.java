...
public class FireworkEntity extends Entity implements FlyingItemEntity, Projectile {
   ...
   public void tick() {
      ...
      if (this.wasShotByEntity()) {
         ...
         if (!this.wasShotAtAngle()) {
            this.setVelocity(this.getVelocity().multiply(1.15D, 1.0D, 1.15D).add(0.0D, 0.04D, 0.0D));
         }
      ...
      HitResult hitResult = ProjectileUtil.getCollision(this, this.getBoundingBox().stretch(vec3d).expand(1.0D), (entity) -> {
      ...
      if (f > 0.0F) {
         ...
         while(true) {
            ...
            for(int i = 0; i < 2; ++i) {
               ...
               HitResult hitResult = this.world.rayTrace(new RayTraceContext(vec3d, vec3d2, RayTraceContext.ShapeType.COLLIDER, RayTraceContext.FluidHandling.NONE, this));
   ...
   public void setVelocity(double x, double y, double z, float speed, float divergence) {
      ...
      y /= (double)f;
      z /= (double)f;
      x += this.random.nextGaussian() * 0.007499999832361937D * (double)divergence;
      y += this.random.nextGaussian() * 0.007499999832361937D * (double)divergence;
      z += this.random.nextGaussian() * 0.007499999832361937D * (double)divergence;
      x *= (double)speed;
      y *= (double)speed;
      z *= (double)speed;
      this.setVelocity(x, y, z);
   ...
   static {
      ITEM = DataTracker.registerData(FireworkEntity.class, TrackedDataHandlerRegistry.ITEM_STACK);
      SHOOTER_ENTITY_ID = DataTracker.registerData(FireworkEntity.class, TrackedDataHandlerRegistry.field_17910);
      SHOT_AT_ANGLE = DataTracker.registerData(FireworkEntity.class, TrackedDataHandlerRegistry.BOOLEAN);
   }
}
