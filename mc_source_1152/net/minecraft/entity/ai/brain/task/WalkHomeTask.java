...
public class WalkHomeTask extends Task<LivingEntity> {
   ...
   protected boolean shouldRun(ServerWorld world, LivingEntity entity) {
      if (world.getTime() - this.expiryTimeLimit < 20L) {
         ...
         Optional<BlockPos> optional = pointOfInterestStorage.getNearestPosition(PointOfInterestType.HOME.getCompletionCondition(), (blockPos) -> {
            return true;
         }, new BlockPos(entity), 48, PointOfInterestStorage.OccupationStatus.ANY);
         return optional.isPresent() && ((BlockPos)optional.get()).getSquaredDistance(new BlockPos(mobEntityWithAi)) > 4.0D;
      }
   }
   ...
   protected void run(ServerWorld world, LivingEntity entity, long time) {
      ...
      if (path != null && path.reachesTarget()) {
         ...
         this.positionToExpiry.long2LongEntrySet().removeIf((entry) -> {
            return entry.getLongValue() < this.expiryTimeLimit;
         });
      }
