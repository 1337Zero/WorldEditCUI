...
public class GoToPointOfInterestTask extends Task<VillagerEntity> {
   ...
   protected void run(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
      ...
      for(int j = 0; j < 5; ++j) {
         Vec3d vec3d2 = TargetFinder.findGroundTarget(villagerEntity, 15, 7, (blockPos) -> {
            return (double)(-serverWorld.getOccupiedPointOfInterestDistance(ChunkSectionPos.from(blockPos)));
         });
         if (vec3d2 != null) {
