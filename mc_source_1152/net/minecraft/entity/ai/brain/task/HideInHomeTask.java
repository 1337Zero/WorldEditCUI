...
public class HideInHomeTask extends Task<LivingEntity> {
   ...
   protected boolean shouldRun(ServerWorld world, LivingEntity entity) {
      Optional<BlockPos> optional = world.getPointOfInterestStorage().getPosition((pointOfInterestType) -> {
         return pointOfInterestType == PointOfInterestType.HOME;
      }, (blockPos) -> {
         return true;
      }, new BlockPos(entity), this.preferredDistance + 1, PointOfInterestStorage.OccupationStatus.ANY);
   ...
   protected void run(ServerWorld world, LivingEntity entity, long time) {
      ...
      if (!optional.isPresent()) {
         optional = world.getPointOfInterestStorage().getPosition((pointOfInterestType) -> {
            return pointOfInterestType == PointOfInterestType.HOME;
         }, (blockPos) -> {
            return true;
         }, PointOfInterestStorage.OccupationStatus.ANY, new BlockPos(entity), this.maxDistance, entity.getRandom());
         if (!optional.isPresent()) {
