...
public class VillagerWorkTask extends Task<VillagerEntity> {
   ...
   protected boolean shouldRun(ServerWorld serverWorld, VillagerEntity villagerEntity) {
      if (serverWorld.getTime() - this.lastCheckedTime < 300L) {
         ...
         return Objects.equals(globalPos.getDimension(), serverWorld.getDimension().getType()) && globalPos.getPos().isWithinDistance(villagerEntity.getPos(), 1.73D);
      }
   }
   ...
   protected void run(ServerWorld serverWorld, VillagerEntity villagerEntity, long l) {
      ...
      brain.getOptionalMemory(MemoryModuleType.JOB_SITE).ifPresent((globalPos) -> {
         brain.putMemory(MemoryModuleType.LOOK_TARGET, new BlockPosLookTarget(globalPos.getPos()));
      });
