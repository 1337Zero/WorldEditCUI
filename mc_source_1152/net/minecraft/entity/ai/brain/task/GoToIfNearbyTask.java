...
public class GoToIfNearbyTask extends Task<MobEntityWithAi> {
   ...
   protected boolean shouldRun(ServerWorld serverWorld, MobEntityWithAi mobEntityWithAi) {
      ...
      return optional.isPresent() && Objects.equals(serverWorld.getDimension().getType(), ((GlobalPos)optional.get()).getDimension()) && ((GlobalPos)optional.get()).getPos().isWithinDistance(mobEntityWithAi.getPos(), (double)this.maxDistance);
   }
   ...
   protected void run(ServerWorld serverWorld, MobEntityWithAi mobEntityWithAi, long l) {
      if (l > this.nextUpdateTime) {
         ...
         mobEntityWithAi.getBrain().setMemory(MemoryModuleType.WALK_TARGET, optional.map((vec3d) -> {
            return new WalkTarget(vec3d, 0.4F, 1);
         }));
