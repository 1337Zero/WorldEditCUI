...
public class Brain<E extends LivingEntity> implements DynamicSerializable {
   ...
   public <T> Brain(Collection<MemoryModuleType<?>> collection, Collection<SensorType<? extends Sensor<? super E>>> sensors, Dynamic<T> dynamic) {
      ...
      this.sensors.values().forEach((sensor) -> {
         Iterator var2 = sensor.getOutputMemoryModules().iterator();

         while(var2.hasNext()) {
            MemoryModuleType<?> memoryModuleType = (MemoryModuleType)var2.next();
            this.memories.put(memoryModuleType, Optional.empty());
         }

      });
      Iterator var4 = dynamic.get("memories").asMap(Function.identity(), Function.identity()).entrySet().iterator();

      while(var4.hasNext()) {
         Entry<Dynamic<T>, Dynamic<T>> entry = (Entry)var4.next();
         this.readMemory((MemoryModuleType)Registry.MEMORY_MODULE_TYPE.get(new Identifier(((Dynamic)entry.getKey()).asString(""))), (Dynamic)entry.getValue());
      }
   ...
   private <T, U> void readMemory(MemoryModuleType<U> memoryModuleType, Dynamic<T> dynamic) {
      this.putMemory(memoryModuleType, ((Function)memoryModuleType.getFactory().orElseThrow(RuntimeException::new)).apply(dynamic));
   }
   ...
   public Stream<Task<? super E>> streamRunningTasks() {
      return this.tasks.values().stream().flatMap((map) -> {
   ...
   public void stopAllTasks(ServerWorld serverWorld, E livingEntity) {
      ...
      this.streamRunningTasks().forEach((task) -> {
         task.stop(serverWorld, livingEntity, l);
      });
   }

   public <T> T serialize(DynamicOps<T> ops) {
      T object = ops.createMap((Map)this.memories.entrySet().stream().filter((entry) -> {
         return ((MemoryModuleType)entry.getKey()).getFactory().isPresent() && ((Optional)entry.getValue()).isPresent();
      }).map((entry) -> {
         return Pair.of(ops.createString(Registry.MEMORY_MODULE_TYPE.getId(entry.getKey()).toString()), ((DynamicSerializable)((Optional)entry.getValue()).get()).serialize(ops));
   ...
   private void startTasks(ServerWorld serverWorld, E livingEntity) {
      ...
      this.tasks.values().stream().flatMap((map) -> {
         return map.entrySet().stream();
      }).filter((entry) -> {
         return this.possibleActivities.contains(entry.getKey());
