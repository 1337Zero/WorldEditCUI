...
public class GoToVillageGoal extends Goal {
   ...
   public boolean canStart() {
      if (this.mob.hasPassengers()) {
         ...
         if (!serverWorld.isNearOccupiedPointOfInterest(blockPos, 6)) {
            ...
            Vec3d vec3d = TargetFinder.findGroundTarget(this.mob, 15, 7, (blockPosx) -> {
   ...
   public void tick() {
      if (this.targetPosition != null) {
         ...
         if (entityNavigation.isIdle() && !this.targetPosition.isWithinDistance(this.mob.getPos(), 10.0D)) {
            Vec3d vec3d = new Vec3d(this.targetPosition);
            Vec3d vec3d2 = this.mob.getPos();
            Vec3d vec3d3 = vec3d2.subtract(vec3d);
            vec3d = vec3d3.multiply(0.4D).add(vec3d);
            ...
            if (!entityNavigation.startMovingTo((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ(), 1.0D)) {
