   ...
   public boolean canStart() {
      ...
      if (this.requiresNighttime && this.mob.world.isDay()) {
         ...
         if (!serverWorld.isNearOccupiedPointOfInterest(blockPos, 6)) {
            ...
            Vec3d vec3d = TargetFinder.findGroundTarget(this.mob, 15, 7, (blockPos2x) -> {
               if (!serverWorld.isNearOccupiedPointOfInterest(blockPos2x)) {
                  return Double.NEGATIVE_INFINITY;
               } else {
                  Optional<BlockPos> optional = serverWorld.getPointOfInterestStorage().getPosition(PointOfInterestType.ALWAYS_TRUE, this::shouldVisit, blockPos2x, 10, PointOfInterestStorage.OccupationStatus.IS_OCCUPIED);
                  return !optional.isPresent() ? Double.NEGATIVE_INFINITY : -((BlockPos)optional.get()).getSquaredDistance(blockPos);
               }
            });
            if (vec3d == null) {
               return false;
               ...
               if (!optional.isPresent()) {
                  ...
                  this.targetPath = mobNavigation.findPathTo((BlockPos)this.target, 0);
