...
public class MoveToRaidCenterGoal<T extends RaiderEntity> extends Goal {
   ...
   public boolean canStart() {
      return this.actor.getTarget() == null && !this.actor.hasPassengers() && this.actor.hasActiveRaid() && !this.actor.getRaid().isFinished() && !((ServerWorld)this.actor.world).isNearOccupiedPointOfInterest(new BlockPos(this.actor));
   }

   public boolean shouldContinue() {
      return this.actor.hasActiveRaid() && !this.actor.getRaid().isFinished() && this.actor.world instanceof ServerWorld && !((ServerWorld)this.actor.world).isNearOccupiedPointOfInterest(new BlockPos(this.actor));
   }
   ...
   private void includeFreeRaiders(Raid raid) {
      if (raid.isActive()) {
         ...
         List<RaiderEntity> list = this.actor.world.getEntities(RaiderEntity.class, this.actor.getBoundingBox().expand(16.0D), (raiderEntityx) -> {
