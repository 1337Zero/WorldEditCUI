...
public class LandPathNodeMaker extends PathNodeMaker {
   ...
   public PathNode getStart() {
      ...
      if (this.canSwim() && this.entity.isTouchingWater()) {
         k = MathHelper.floor(this.entity.getY());
         BlockPos.Mutable mutable = new BlockPos.Mutable(this.entity.getX(), (double)k, this.entity.getZ());

         for(BlockState blockState = this.field_20622.getBlockState(mutable); blockState.getBlock() == Blocks.WATER || blockState.getFluidState() == Fluids.WATER.getStill(false); blockState = this.field_20622.getBlockState(mutable)) {
            ++k;
            mutable.set(this.entity.getX(), (double)k, this.entity.getZ());
         }

         --k;
      } else if (this.entity.onGround) {
         k = MathHelper.floor(this.entity.getY() + 0.5D);
      } else {
         for(blockPos2 = new BlockPos(this.entity); (this.field_20622.getBlockState(blockPos2).isAir() || this.field_20622.getBlockState(blockPos2).canPlaceAtSide(this.field_20622, blockPos2, BlockPlacementEnvironment.LAND)) && blockPos2.getY() > 0; blockPos2 = blockPos2.down()) {
         }
      ...
      PathNodeType pathNodeType = this.getNodeType(this.entity, blockPos2.getX(), k, blockPos2.getZ());
      if (this.entity.getPathfindingPenalty(pathNodeType) < 0.0F) {
         Set<BlockPos> set = Sets.newHashSet();
         set.add(new BlockPos(this.entity.getBoundingBox().x1, (double)k, this.entity.getBoundingBox().z1));
         set.add(new BlockPos(this.entity.getBoundingBox().x1, (double)k, this.entity.getBoundingBox().z2));
         set.add(new BlockPos(this.entity.getBoundingBox().x2, (double)k, this.entity.getBoundingBox().z1));
         set.add(new BlockPos(this.entity.getBoundingBox().x2, (double)k, this.entity.getBoundingBox().z2));
         Iterator var5 = set.iterator();

         while(var5.hasNext()) {
            BlockPos blockPos3 = (BlockPos)var5.next();
      ...
      return this.getNode(blockPos2.getX(), k, blockPos2.getZ());
   }
   ...
   private boolean isValidDiagonalSuccessor(PathNode node, @Nullable PathNode successor1, @Nullable PathNode successor2, @Nullable PathNode diagonalSuccessor) {
      if (diagonalSuccessor != null && successor2 != null && successor1 != null) {
         if (diagonalSuccessor.visited) {
            return false;
         } else if (successor2.y <= node.y && successor1.y <= node.y) {
   ...
   public static double getHeight(BlockView world, BlockPos pos) {
      ...
      return (double)blockPos.getY() + (voxelShape.isEmpty() ? 0.0D : voxelShape.getMaximum(Direction.Axis.Y));
   }
         ...
         if (pathNodeType == PathNodeType.WALKABLE) {
            ...
            if ((pathNode == null || pathNode.penalty < 0.0F) && maxYStep > 0 && pathNodeType != PathNodeType.FENCE && pathNodeType != PathNodeType.TRAPDOOR) {
               ...
               if (pathNode != null && (pathNode.type == PathNodeType.OPEN || pathNode.type == PathNodeType.WALKABLE) && this.entity.getWidth() < 1.0F) {
                  ...
                  Box box = new Box(g - e, getHeight(this.field_20622, new BlockPos(g, (double)(y + 1), h)) + 0.001D, h - e, g + e, (double)this.entity.getHeight() + getHeight(this.field_20622, new BlockPos(pathNode.x, pathNode.y, pathNode.z)) - 0.002D, h + e);
            ...
            if (pathNodeType == PathNodeType.OPEN) {
               Box box2 = new Box((double)x - e + 0.5D, (double)y + 0.001D, (double)z - e + 0.5D, (double)x + e + 0.5D, (double)((float)y + this.entity.getHeight()), (double)z + e + 0.5D);
   ...
   public PathNodeType getNodeType(BlockView world, int x, int y, int z, MobEntity mob, int sizeX, int sizeY, int sizeZ, boolean canOpenDoors, boolean canEnterOpenDoors) {
      ...
      double d = (double)mob.getWidth() / 2.0D;
      ...
      if (enumSet.contains(PathNodeType.FENCE)) {
         ...
         while(var17.hasNext()) {
            PathNodeType pathNodeType3 = (PathNodeType)var17.next();
   ...
   protected static PathNodeType getBasicPathNodeType(BlockView blockView, int i, int j, int k) {
      ...
      if (blockState.isAir()) {
         ...
         } else if (!block.matches(BlockTags.FENCES) && !block.matches(BlockTags.WALLS) && (!(block instanceof FenceGateBlock) || (Boolean)blockState.get(FenceGateBlock.OPEN))) {
