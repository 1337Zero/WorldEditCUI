...
public abstract class EntityNavigation {
   ...
   protected Vec3d field_6672;
   protected Vec3d field_6680;
   ...
   public EntityNavigation(MobEntity mobEntity, World world) {
      this.field_6672 = Vec3d.ZERO;
      this.field_6680 = Vec3d.ZERO;
      this.field_6683 = 0.5F;
   ...
   public Path findPathToAny(Stream<BlockPos> positions, int distance) {
      return this.findPathToAny((Set)positions.collect(Collectors.toSet()), 8, false, distance);
   }
   ...
   public void tick() {
      ...
      if (!this.isIdle()) {
         ...
         if (this.isAtValidPosition()) {
            ...
            if (vec3d3.y > vec3d2.y && !this.entity.onGround && MathHelper.floor(vec3d3.x) == MathHelper.floor(vec3d2.x) && MathHelper.floor(vec3d3.z) == MathHelper.floor(vec3d2.z)) {
         ...
         if (!this.isIdle()) {
            ...
            this.entity.getMoveControl().moveTo(vec3d3.x, this.world.getBlockState(blockPos.down()).isAir() ? vec3d3.y : LandPathNodeMaker.getHeight(this.world, blockPos), vec3d3.z, this.speed);
         }
      }
   }
   ...
   protected void method_6339() {
      ...
      if (Math.abs(this.entity.getX() - (vec3d2.x + 0.5D)) < (double)this.field_6683 && Math.abs(this.entity.getZ() - (vec3d2.z + 0.5D)) < (double)this.field_6683 && Math.abs(this.entity.getY() - vec3d2.y) < 1.0D) {
         this.currentPath.setCurrentNodeIndex(this.currentPath.getCurrentNodeIndex() + 1);
      }
   ...
   protected void method_6346(Vec3d vec3d) {
      ...
      if (this.currentPath != null && !this.currentPath.isFinished()) {
         ...
         if (vec3d2.equals(this.field_6680)) {
            ...
            this.field_6682 = this.entity.getMovementSpeed() > 0.0F ? d / (double)this.entity.getMovementSpeed() * 1000.0D : 0.0D;
         }

         if (this.field_6682 > 0.0D && (double)this.field_6670 > this.field_6682 * 3.0D) {
            this.field_6680 = Vec3d.ZERO;
            this.field_6670 = 0L;
