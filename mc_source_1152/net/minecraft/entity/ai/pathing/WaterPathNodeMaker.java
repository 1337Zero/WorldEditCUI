...
public class WaterPathNodeMaker extends PathNodeMaker {
   ...
   public PathNode getStart() {
      return super.getNode(MathHelper.floor(this.entity.getBoundingBox().x1), MathHelper.floor(this.entity.getBoundingBox().y1 + 0.5D), MathHelper.floor(this.entity.getBoundingBox().z1));
   }
   ...
   public PathNodeType getNodeType(BlockView world, int x, int y, int z) {
      ...
      if (fluidState.isEmpty() && blockState.canPlaceAtSide(world, blockPos.down(), BlockPlacementEnvironment.WATER) && blockState.isAir()) {
         ...
         return fluidState.matches(FluidTags.WATER) && blockState.canPlaceAtSide(world, blockPos, BlockPlacementEnvironment.WATER) ? PathNodeType.WATER : PathNodeType.BLOCKED;
      }
   }
