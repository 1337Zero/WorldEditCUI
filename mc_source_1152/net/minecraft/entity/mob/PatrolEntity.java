   ...
   public static class PatrolGoal<T extends PatrolEntity> extends Goal {
      ...
      public void tick() {
         ...
         if (entityNavigation.isIdle()) {
            ...
            } else if (bl && this.actor.getPatrolTarget().isWithinDistance(this.actor.getPos(), 10.0D)) {
      ...
      private List<PatrolEntity> method_22333() {
         return this.actor.world.getEntities(PatrolEntity.class, this.actor.getBoundingBox().expand(16.0D), (patrolEntity) -> {
            return patrolEntity.hasNoRaid() && !patrolEntity.isPartOf(this.actor);
         });
      }

      private boolean wander() {
         Random random = this.actor.getRandom();
         BlockPos blockPos = this.actor.world.getTopPosition(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, (new BlockPos(this.actor)).add(-8 + random.nextInt(16), 0, -8 + random.nextInt(16)));
