...
public abstract class MobEntity extends LivingEntity {
   ...
   protected void initAttributes() {
      ...
      this.getAttributes().register(EntityAttributes.FOLLOW_RANGE).setBaseValue(16.0D);
   ...
   public float getPathfindingPenalty(PathNodeType nodeType) {
      Float var2 = (Float)this.pathfindingPenalties.get(nodeType);
      return var2 == null ? nodeType.getDefaultPenalty() : var2;
   }
   ...
   public void writeCustomDataToTag(CompoundTag tag) {
      ...
      for(Iterator var11 = this.handItems.iterator(); var11.hasNext(); listTag2.add(compoundTag3)) {
   ...
   public void readCustomDataFromTag(CompoundTag tag) {
      ...
      if (tag.contains("ArmorItems", 9)) {
         listTag4 = tag.getList("ArmorItems", 10);

         for(l = 0; l < this.armorItems.size(); ++l) {
            this.armorItems.set(l, ItemStack.fromTag(listTag4.getCompound(l)));
         }
      }

      if (tag.contains("HandItems", 9)) {
         listTag4 = tag.getList("HandItems", 10);

         for(l = 0; l < this.handItems.size(); ++l) {
            this.handItems.set(l, ItemStack.fromTag(listTag4.getCompound(l)));
         }
      }

      if (tag.contains("ArmorDropChances", 9)) {
         listTag4 = tag.getList("ArmorDropChances", 5);

         for(l = 0; l < listTag4.size(); ++l) {
            this.armorDropChances[l] = listTag4.getFloat(l);
         }
      }

      if (tag.contains("HandDropChances", 9)) {
         listTag4 = tag.getList("HandDropChances", 5);
   ...
   public void checkDespawn() {
      if (this.world.getDifficulty() == Difficulty.PEACEFUL && this.method_23734()) {
         ...
         if (entity != null) {
            ...
            if (d > 16384.0D && this.canImmediatelyDespawn(d)) {
               this.remove();
            }

            if (this.despawnCounter > 600 && this.random.nextInt(800) == 0 && d > 1024.0D && this.canImmediatelyDespawn(d)) {
               this.remove();
            } else if (d < 1024.0D) {
   ...
   public void lookAtEntity(Entity targetEntity, float maxYawChange, float maxPitchChange) {
      ...
      if (targetEntity instanceof LivingEntity) {
         ...
         g = (targetEntity.getBoundingBox().y1 + targetEntity.getBoundingBox().y2) / 2.0D - this.getEyeY();
      }
   ...
   protected float getDropChance(EquipmentSlot equipmentSlot) {
      ...
      switch(equipmentSlot.getType()) {
      case HAND:
         h = this.handDropChances[equipmentSlot.getEntitySlotId()];
         break;
      case ARMOR:
         h = this.armorDropChances[equipmentSlot.getEntitySlotId()];
         break;
      default:
         h = 0.0F;
      }
   ...
   public EntityData initialize(IWorld world, LocalDifficulty difficulty, SpawnType spawnType, @Nullable EntityData entityData, @Nullable CompoundTag entityTag) {
      this.getAttributeInstance(EntityAttributes.FOLLOW_RANGE).addModifier(new EntityAttributeModifier("Random spawn bonus", this.random.nextGaussian() * 0.05D, EntityAttributeModifier.Operation.MULTIPLY_BASE));
   ...
   public boolean tryAttack(Entity target) {
      ...
      if (bl) {
         if (g > 0.0F && target instanceof LivingEntity) {
            ((LivingEntity)target).takeKnockback(this, g * 0.5F, (double)MathHelper.sin(this.yaw * 0.017453292F), (double)(-MathHelper.cos(this.yaw * 0.017453292F)));
