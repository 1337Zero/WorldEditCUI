...
public class WitchEntity extends RaiderEntity implements RangedAttackMob {
   ...
   public void handleStatus(byte status) {
      if (status == 15) {
         for(int i = 0; i < this.random.nextInt(35) + 10; ++i) {
            this.world.addParticle(ParticleTypes.WITCH, this.getX() + this.random.nextGaussian() * 0.12999999523162842D, this.getBoundingBox().y2 + 0.5D + this.random.nextGaussian() * 0.12999999523162842D, this.getZ() + this.random.nextGaussian() * 0.12999999523162842D, 0.0D, 0.0D, 0.0D);
         }
      } else {
   ...
   static {
      DRINKING_SPEED_PENALTY_MODIFIER = (new EntityAttributeModifier(DRINKING_SPEED_PENALTY_MODIFIER_ID, "Drinking speed penalty", -0.25D, EntityAttributeModifier.Operation.ADDITION)).setSerialize(false);
