...
public class TntMinecartEntity extends AbstractMinecartEntity {
   ...
   protected void explode(double d) {
      if (!this.world.isClient) {
         ...
         this.world.createExplosion(this, this.getX(), this.getY(), this.getZ(), (float)(4.0D + this.random.nextDouble() * 1.5D * e), Explosion.DestructionType.BREAK);
         this.remove();
      }
   ...
   public float getEffectiveExplosionResistance(Explosion explosion, BlockView world, BlockPos pos, BlockState blockState, FluidState fluidState, float max) {
      return !this.isPrimed() || !blockState.matches(BlockTags.RAILS) && !world.getBlockState(pos.up()).matches(BlockTags.RAILS) ? super.getEffectiveExplosionResistance(explosion, world, pos, blockState, fluidState, max) : 0.0F;
   }

   public boolean canExplosionDestroyBlock(Explosion explosion, BlockView world, BlockPos pos, BlockState state, float explosionPower) {
      return !this.isPrimed() || !state.matches(BlockTags.RAILS) && !world.getBlockState(pos.up()).matches(BlockTags.RAILS) ? super.canExplosionDestroyBlock(explosion, world, pos, state, explosionPower) : false;
   }
