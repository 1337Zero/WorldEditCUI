...
public abstract class RaiderEntity extends PatrolEntity {
   ...
   static {
      ...
      OBTAINABLE_OMINOUS_BANNER_PREDICATE = (itemEntity) -> {
         return !itemEntity.cannotPickup() && itemEntity.isAlive() && ItemStack.areEqualIgnoreDamage(itemEntity.getStack(), Raid.getOminousBanner());
      };
   }
   ...
   public class PickupBannerAsLeaderGoal<T extends RaiderEntity> extends Goal {
      ...
      public boolean canStart() {
         ...
         if (this.actor.hasActiveRaid() && !this.actor.getRaid().isFinished() && this.actor.canLead() && !ItemStack.areEqualIgnoreDamage(this.actor.getEquippedStack(EquipmentSlot.HEAD), Raid.getOminousBanner())) {
            RaiderEntity raiderEntity = raid.getCaptain(this.actor.getWave());
            if (raiderEntity == null || !raiderEntity.isAlive()) {
               List<ItemEntity> list = this.actor.world.getEntities(ItemEntity.class, this.actor.getBoundingBox().expand(16.0D, 8.0D, 16.0D), RaiderEntity.OBTAINABLE_OMINOUS_BANNER_PREDICATE);
               if (!list.isEmpty()) {
      ...
      public void tick() {
         if (this.actor.getNavigation().getTargetPos().isWithinDistance(this.actor.getPos(), 1.414D)) {
            List<ItemEntity> list = this.actor.world.getEntities(ItemEntity.class, this.actor.getBoundingBox().expand(4.0D, 4.0D, 4.0D), RaiderEntity.OBTAINABLE_OMINOUS_BANNER_PREDICATE);
            if (!list.isEmpty()) {
