...
public class EntityAttributeModifier {
   ...
   public EntityAttributeModifier(String name, double amount, EntityAttributeModifier.Operation operation) {
      this(MathHelper.randomUuid(ThreadLocalRandom.current()), () -> {
         return name;
      }, amount, operation);
   }
   ...
   public String toString() {
      return "AttributeModifier{amount=" + this.amount + ", operation=" + this.operation + ", name='" + (String)this.nameGetter.get() + '\'' + ", id=" + this.uuid + ", serialize=" + this.serialize + '}';
   }
