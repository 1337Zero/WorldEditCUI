...
public class EnderDragonFight {
   ...
   public EnderDragonFight(ServerWorld world, CompoundTag compoundTag) {
      this.bossBar = (ServerBossBar)(new ServerBossBar(new TranslatableText("entity.minecraft.ender_dragon", new Object[0]), BossBar.Color.PINK, BossBar.Style.PROGRESS)).setDragonMusic(true).setThickenFog(true);
   ...
   private void countAliveCrystals() {
      ...
      for(Iterator var1 = EndSpikeFeature.getSpikes(this.world).iterator(); var1.hasNext(); this.endCrystalsAlive += this.world.getNonSpectatingEntities(EnderCrystalEntity.class, spike.getBoundingBox()).size()) {
   ...
   private void generateEndPortal(boolean previouslyKilled) {
      ...
      if (this.exitPortalLocation == null) {
         for(this.exitPortalLocation = this.world.getTopPosition(Heightmap.Type.MOTION_BLOCKING_NO_LEAVES, EndPortalFeature.ORIGIN).down(); this.world.getBlockState(this.exitPortalLocation).getBlock() == Blocks.BEDROCK && this.exitPortalLocation.getY() > this.world.getSeaLevel(); this.exitPortalLocation = this.exitPortalLocation.down()) {
         }
      }
   ...
   public void resetEndCrystals() {
      ...
      while(var1.hasNext()) {
         ...
         while(var4.hasNext()) {
            EnderCrystalEntity enderCrystalEntity = (EnderCrystalEntity)var4.next();
   ...
   static {
      VALID_ENTITY = EntityPredicates.VALID_ENTITY.and(EntityPredicates.maximumDistance(0.0D, 128.0D, 0.0D, 192.0D));
   }
}
