   ...
   private <IS> TypeRewriteRule fixEquipment(Type<IS> type) {
      ...
      return this.fixTypeEverywhereTyped("EntityEquipmentToArmorAndHandFix", this.getInputSchema().getType(TypeReferences.ENTITY), this.getOutputSchema().getType(TypeReferences.ENTITY), (typed) -> {
         ...
         if (optional.isPresent()) {
            ...
            IS object = ((Optional)type.read(dynamic.emptyMap()).getSecond()).orElseThrow(() -> {
         ...
         if (optional2.isPresent()) {
            ...
            if (!dynamic.get("HandDropChances").get().isPresent()) {
               dynamic4 = dynamic.emptyMap().merge(dynamic.createFloat(f)).merge(dynamic.createFloat(0.0F));
               dynamic = dynamic.set("HandDropChances", dynamic4);
            }

            if (!dynamic.get("ArmorDropChances").get().isPresent()) {
               dynamic4 = dynamic.emptyMap().merge(dynamic.createFloat(((Dynamic)iterator.next()).asFloat(0.0F))).merge(dynamic.createFloat(((Dynamic)iterator.next()).asFloat(0.0F))).merge(dynamic.createFloat(((Dynamic)iterator.next()).asFloat(0.0F))).merge(dynamic.createFloat(((Dynamic)iterator.next()).asFloat(0.0F)));
