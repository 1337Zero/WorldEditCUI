...
public class EntityRidingToPassengerFix extends DataFix {
   ...
   private <OldEntityTree, NewEntityTree, Entity> TypeRewriteRule method_4978(Schema schema, Schema schema2, Type<OldEntityTree> type, Type<NewEntityTree> type2, Type<Entity> type3) {
      ...
      Type<Pair<String, Pair<Either<List<NewEntityTree>, Unit>, Entity>>> type5 = DSL.named(TypeReferences.ENTITY_TREE.typeName(), DSL.and(DSL.optional(DSL.field("Passengers", DSL.list(type2))), type3));
      ...
      if (!Objects.equals(type6, type4)) {
         ...
         return TypeRewriteRule.seq(this.fixTypeEverywhere("EntityRidingToPassengerFix", type4, type5, (dynamicOps) -> {
            return (pair) -> {
               ...
               while(true) {
                  Either<List<NewEntityTree>, Unit> either = (Either)DataFixUtils.orElse(optional.map((pairx) -> {
                     Typed<NewEntityTree> typed = (Typed)type2.pointTyped(dynamicOps).orElseThrow(() -> {
                  ...
                  pair2 = (Pair)(new Typed(type, dynamicOps, optional2.get())).getOptional(opticFinder).orElseThrow(() -> {
