...
public class VillagerTradeFix extends ChoiceFix {
   ...
   protected Typed<?> transform(Typed<?> typed) {
      ...
      if (!(type instanceof ListType)) {
         ...
         Function<Typed<?>, Typed<?>> function = (typedx) -> {
            return this.fixPumpkinTrade(opticFinder7, typedx);
         };
         return typed.updateTyped(opticFinder, (typedx) -> {
            return typedx.updateTyped(opticFinder2, (typed) -> {
               return typed.updateTyped(opticFinder3, (typedx) -> {
                  return typedx.updateTyped(opticFinder4, function).updateTyped(opticFinder5, function).updateTyped(opticFinder6, function);
               });
            });
         });
      }
