...
public class LevelFlatGeneratorInfoFix extends DataFix {
   ...
   private Dynamic<?> fixGeneratorOptions(Dynamic<?> tag) {
      return tag.get("generatorName").asString("").equalsIgnoreCase("flat") ? tag.update("generatorOptions", (dynamic) -> {
         Optional var10000 = dynamic.asString().map(this::fixFlatGeneratorOptions);
         dynamic.getClass();
