...
public class Schema704 extends Schema {
   protected static final Map<String, String> field_5744 = (Map)DataFixUtils.make(Maps.newHashMap(), (hashMap) -> {
   ...
   public Type<?> getChoiceType(TypeReference typeReference, String string) {
      return Objects.equals(typeReference.typeName(), TypeReferences.BLOCK_ENTITY.typeName()) ? super.getChoiceType(typeReference, SchemaIdentifierNormalize.normalize(string)) : super.getChoiceType(typeReference, string);
   }
   ...
   public void registerTypes(Schema schema, Map<String, Supplier<TypeTemplate>> map, Map<String, Supplier<TypeTemplate>> map2) {
      ...
      schema.registerType(true, TypeReferences.ITEM_STACK, () -> {
         return DSL.hook(DSL.optionalFields("id", TypeReferences.ITEM_NAME.in(schema), "tag", DSL.optionalFields("EntityTag", TypeReferences.ENTITY_TREE.in(schema), "BlockEntityTag", TypeReferences.BLOCK_ENTITY.in(schema), "CanDestroy", DSL.list(TypeReferences.BLOCK_NAME.in(schema)), "CanPlaceOn", DSL.list(TypeReferences.BLOCK_NAME.in(schema)))), field_5745, HookFunction.IDENTITY);
      });
   }
}
