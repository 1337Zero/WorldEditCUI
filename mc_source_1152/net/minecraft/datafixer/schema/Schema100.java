...
public class Schema100 extends Schema {
   ...
   public Map<String, Supplier<TypeTemplate>> registerEntities(Schema schema) {
      ...
      schema.register(map, "Enderman", (string) -> {
         return DSL.optionalFields("carried", TypeReferences.BLOCK_NAME.in(schema), method_5196(schema));
      });
      ...
      schema.register(map, "EntityHorse", (string) -> {
         return DSL.optionalFields("Items", DSL.list(TypeReferences.ITEM_STACK.in(schema)), "ArmorItem", TypeReferences.ITEM_STACK.in(schema), "SaddleItem", TypeReferences.ITEM_STACK.in(schema), method_5196(schema));
      });
      method_5195(schema, map, "Rabbit");
      schema.register(map, "Villager", (string) -> {
         return DSL.optionalFields("Inventory", DSL.list(TypeReferences.ITEM_STACK.in(schema)), "Offers", DSL.optionalFields("Recipes", DSL.list(DSL.optionalFields("buy", TypeReferences.ITEM_STACK.in(schema), "buyB", TypeReferences.ITEM_STACK.in(schema), "sell", TypeReferences.ITEM_STACK.in(schema)))), method_5196(schema));
      });
