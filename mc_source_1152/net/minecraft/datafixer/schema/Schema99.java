...
public class Schema99 extends Schema {
   ...
   private static final Map<String, String> field_5748 = (Map)DataFixUtils.make(Maps.newHashMap(), (hashMap) -> {
   ...
   public Map<String, Supplier<TypeTemplate>> registerEntities(Schema schema) {
      ...
      schema.register(map, "Enderman", (string) -> {
         return DSL.optionalFields("carried", TypeReferences.BLOCK_NAME.in(schema), method_5353(schema));
      });
      ...
      schema.register(map, "EntityHorse", (string) -> {
         return DSL.optionalFields("Items", DSL.list(TypeReferences.ITEM_STACK.in(schema)), "ArmorItem", TypeReferences.ITEM_STACK.in(schema), "SaddleItem", TypeReferences.ITEM_STACK.in(schema), method_5353(schema));
      });
      method_5339(schema, map, "Rabbit");
      schema.register(map, "Villager", (string) -> {
         return DSL.optionalFields("Inventory", DSL.list(TypeReferences.ITEM_STACK.in(schema)), "Offers", DSL.optionalFields("Recipes", DSL.list(DSL.optionalFields("buy", TypeReferences.ITEM_STACK.in(schema), "buyB", TypeReferences.ITEM_STACK.in(schema), "sell", TypeReferences.ITEM_STACK.in(schema)))), method_5353(schema));
      });
   ...
   public void registerTypes(Schema schema, Map<String, Supplier<TypeTemplate>> map, Map<String, Supplier<TypeTemplate>> map2) {
      ...
      schema.registerType(true, TypeReferences.ITEM_STACK, () -> {
         return DSL.hook(DSL.optionalFields("id", DSL.or(DSL.constType(DSL.intType()), TypeReferences.ITEM_NAME.in(schema)), "tag", DSL.optionalFields("EntityTag", TypeReferences.ENTITY_TREE.in(schema), "BlockEntityTag", TypeReferences.BLOCK_ENTITY.in(schema), "CanDestroy", DSL.list(TypeReferences.BLOCK_NAME.in(schema)), "CanPlaceOn", DSL.list(TypeReferences.BLOCK_NAME.in(schema)))), field_5747, HookFunction.IDENTITY);
      });
   ...
   protected static <T> T method_5359(Dynamic<T> dynamic, Map<String, String> map, String string) {
      return dynamic.update("tag", (dynamic2) -> {
         return dynamic2.update("BlockEntityTag", (dynamic2x) -> {
            ...
            if (string2 == null) {
               field_5749.warn("Unable to resolve BlockEntity for ItemStack: {}", string);
               return dynamic2x;
            } else {
            ...
            return Objects.equals(SchemaIdentifierNormalize.normalize(string2), "minecraft:armor_stand") ? dynamic2x.set("id", dynamic.createString(string)) : dynamic2x;
         });
      }).getValue();
   }
}
