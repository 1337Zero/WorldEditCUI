...
public class StructureReferenceFixer extends DataFix {
   ...
   protected TypeRewriteRule makeRule() {
      ...
      return this.fixTypeEverywhereTyped("Structure Reference Fix", type, (typed) -> {
         return typed.update(DSL.remainderFinder(), StructureReferenceFixer::updateReferences);
      });
   }

   private static <T> Dynamic<T> updateReferences(Dynamic<T> dynamic) {
      return dynamic.update("references", (dynamicx) -> {
         return dynamicx.createInt((Integer)dynamicx.asNumber().map(Number::intValue).filter((integer) -> {
            return integer > 0;
         }).orElse(1));
      });
   }
