...
public abstract class AbstractTagProvider<T> implements DataProvider {
   ...
   public void run(DataCache dataCache) {
      ...
      this.configure();
      TagContainer<T> tagContainer = new TagContainer((identifier) -> {
         return Optional.empty();
      }, "", false, "generated");
      Map<Identifier, Tag.Builder<T>> map = (Map)this.tagBuilders.entrySet().stream().collect(Collectors.toMap((entry) -> {
         return ((Tag)entry.getKey()).getId();
      }, Entry::getValue));
      tagContainer.applyReload(map);
      tagContainer.getEntries().forEach((identifier, tag) -> {
         Registry var10001 = this.registry;
         var10001.getClass();
         JsonObject jsonObject = tag.toJson(var10001::getId);
   ...
   protected Tag.Builder<T> getOrCreateTagBuilder(Tag<T> tag) {
      return (Tag.Builder)this.tagBuilders.computeIfAbsent(tag, (tagx) -> {
