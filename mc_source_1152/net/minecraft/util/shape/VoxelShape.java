   ...
   public BlockHitResult rayTrace(Vec3d start, Vec3d end, BlockPos pos) {
      if (this.isEmpty()) {
         ...
         if (vec3d.lengthSquared() < 1.0E-7D) {
            ...
            return this.contains(vec3d2.x - (double)pos.getX(), vec3d2.y - (double)pos.getY(), vec3d2.z - (double)pos.getZ()) ? new BlockHitResult(vec3d2, Direction.getFacing(vec3d.x, vec3d.y, vec3d.z).getOpposite(), pos, true) : Box.rayTrace(this.getBoundingBoxes(), start, end, pos);
         }
      }
   }
