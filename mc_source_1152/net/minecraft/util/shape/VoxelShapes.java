...
public final class VoxelShapes {
   ...
   private static final VoxelShape EMPTY = new ArrayVoxelShape(new BitSetVoxelSet(0, 0, 0), new DoubleArrayList(new double[]{0.0D}), new DoubleArrayList(new double[]{0.0D}), new DoubleArrayList(new double[]{0.0D}));
   ...
   public static VoxelShape cuboid(Box box) {
      ...
      if (i >= 0 && j >= 0 && k >= 0) {
         if (i == 0 && j == 0 && k == 0) {
            return box.contains(0.5D, 0.5D, 0.5D) ? fullCube() : empty();
         } else {
            int l = 1 << i;
            int m = 1 << j;
            int n = 1 << k;
   ...
   private static int findRequiredBitResolution(double min, double max) {
      if (min >= -1.0E-7D && max <= 1.0000001D) {
         for(int i = 0; i <= 3; ++i) {
            ...
            boolean bl = Math.abs(d - Math.floor(d)) < 1.0E-7D;
            boolean bl2 = Math.abs(e - Math.floor(e)) < 1.0E-7D;
            if (bl && bl2) {
               return i;
            }
         }
   ...
   public static VoxelShape combine(VoxelShape one, VoxelShape two, BooleanBiFunction function) {
      if (function.apply(false, false)) {
         ...
         if (one.isEmpty()) {
            ...
            PairList pairList2 = createListPair(pairList.getPairs().size() - 1, one.getPointPositions(Direction.Axis.Y), two.getPointPositions(Direction.Axis.Y), bl, bl2);
            PairList pairList3 = createListPair((pairList.getPairs().size() - 1) * (pairList2.getPairs().size() - 1), one.getPointPositions(Direction.Axis.Z), two.getPointPositions(Direction.Axis.Z), bl, bl2);
   ...
   public static boolean matchesAnywhere(VoxelShape shape1, VoxelShape shape2, BooleanBiFunction predicate) {
      if (predicate.apply(false, false)) {
         ...
         for(int var7 = 0; var7 < var6; ++var7) {
            ...
            if (shape1.getMaximum(axis) < shape2.getMinimum(axis) - 1.0E-7D) {
               return bl || bl2;
            }

            if (shape2.getMaximum(axis) < shape1.getMinimum(axis) - 1.0E-7D) {
               return bl || bl2;
            }
         }
         ...
         PairList pairList2 = createListPair(pairList.getPairs().size() - 1, shape1.getPointPositions(Direction.Axis.Y), shape2.getPointPositions(Direction.Axis.Y), bl, bl2);
         PairList pairList3 = createListPair((pairList.getPairs().size() - 1) * (pairList2.getPairs().size() - 1), shape1.getPointPositions(Direction.Axis.Z), shape2.getPointPositions(Direction.Axis.Z), bl, bl2);
   ...
   public static double calculateMaxOffset(Direction.Axis axis, Box box, Stream<VoxelShape> shapes, double maxDist) {
      for(Iterator iterator = shapes.iterator(); iterator.hasNext(); maxDist = ((VoxelShape)iterator.next()).calculateMaxDistance(axis, box, maxDist)) {
   ...
   private static double calculatePushVelocity(Box box, WorldView world, double initial, EntityContext context, AxisCycleDirection direction, Stream<VoxelShape> shapes) {
      if (box.getXLength() >= 1.0E-6D && box.getYLength() >= 1.0E-6D && box.getZLength() >= 1.0E-6D) {
         if (Math.abs(initial) < 1.0E-7D) {
            ...
            int i = MathHelper.floor(box.getMin(axis) - 1.0E-7D) - 1;
            int j = MathHelper.floor(box.getMax(axis) + 1.0E-7D) + 1;
            int k = MathHelper.floor(box.getMin(axis2) - 1.0E-7D) - 1;
            int l = MathHelper.floor(box.getMax(axis2) + 1.0E-7D) + 1;
            double d = box.getMin(axis3) - 1.0E-7D;
            double e = box.getMax(axis3) + 1.0E-7D;
            boolean bl = initial > 0.0D;
            int m = bl ? MathHelper.floor(box.getMax(axis3) - 1.0E-7D) - 1 : MathHelper.floor(box.getMin(axis3) + 1.0E-7D) + 1;
                  ...
                  for(int r = k; r <= l; ++r) {
                     ...
                     if (s < 3) {
                        ...
                        if ((s != 1 || blockState.exceedsCube()) && (s != 2 || blockState.getBlock() == Blocks.MOVING_PISTON)) {
                           initial = blockState.getCollisionShape(world, mutable, context).calculateMaxDistance(axis3, box.offset((double)(-mutable.getX()), (double)(-mutable.getY()), (double)(-mutable.getZ())), initial);
                           if (Math.abs(initial) < 1.0E-7D) {
                              return 0.0D;
                           }
   ...
   public static boolean isSideCovered(VoxelShape shape, VoxelShape neighbor, Direction direction) {
      if (shape == fullCube() && neighbor == fullCube()) {
         ...
         return DoubleMath.fuzzyEquals(voxelShape.getMaximum(axis), 1.0D, 1.0E-7D) && DoubleMath.fuzzyEquals(voxelShape2.getMinimum(axis), 0.0D, 1.0E-7D) && !matchesAnywhere(new SlicedVoxelShape(voxelShape, axis, voxelShape.voxels.getSize(axis) - 1), new SlicedVoxelShape(voxelShape2, axis, 0), booleanBiFunction);
      }
   }
   ...
   public static VoxelShape extrudeFace(VoxelShape shape, Direction direction) {
      if (shape == fullCube()) {
         ...
         if (direction.getDirection() == Direction.AxisDirection.POSITIVE) {
            bl2 = DoubleMath.fuzzyEquals(shape.getMaximum(axis), 1.0D, 1.0E-7D);
            j = shape.voxels.getSize(axis) - 1;
         } else {
            bl2 = DoubleMath.fuzzyEquals(shape.getMinimum(axis), 0.0D, 1.0E-7D);
            j = 0;
         }

         return (VoxelShape)(!bl2 ? empty() : new SlicedVoxelShape(shape, axis, j));
      }
   }
   ...
   public static boolean adjacentSidesCoverSquare(VoxelShape one, VoxelShape two, Direction direction) {
      if (one != fullCube() && two != fullCube()) {
         ...
         if (!DoubleMath.fuzzyEquals(voxelShape.getMaximum(axis), 1.0D, 1.0E-7D)) {
            voxelShape = empty();
         }

         if (!DoubleMath.fuzzyEquals(voxelShape2.getMinimum(axis), 0.0D, 1.0E-7D)) {
            voxelShape2 = empty();
         }
