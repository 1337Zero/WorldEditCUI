...
public abstract class Registry<T> implements IndexedIterable<T> {
   ...
   public static final Registry<StateProviderType<?>> BLOCK_STATE_PROVIDER_TYPE = create("block_state_provider_type", () -> {
      return StateProviderType.SIMPLE_STATE_PROVIDER;
   });
   ...
   public static final Registry<RuleTest> RULE_TEST = create("rule_test", () -> {
      return RuleTest.ALWAYS_TRUE;
   });
   public static final Registry<StructureProcessorType> STRUCTURE_PROCESSOR = create("structure_processor", () -> {
   ...
   private static <T, R extends MutableRegistry<T>> R putDefaultEntry(String id, R mutableRegistry, Supplier<T> supplier) {
      ...
      return (MutableRegistry)REGISTRIES.add(identifier, mutableRegistry);
   }
