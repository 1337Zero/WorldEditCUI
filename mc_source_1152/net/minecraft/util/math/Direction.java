...
public enum Direction implements StringIdentifiable {
   ...
   private static final Map<String, Direction> NAME_MAP = (Map)Arrays.stream(ALL).collect(Collectors.toMap(Direction::getName, (direction) -> {
      return direction;
   }));
   ...
   private static final Long2ObjectMap<Direction> VECTOR_TO_DIRECTION = (Long2ObjectMap)Arrays.stream(ALL).collect(Collectors.toMap((direction) -> {
      return (new BlockPos(direction.getVector())).asLong();
   }, (direction) -> {
      return direction;
   ...
   public static enum Axis implements StringIdentifiable, Predicate<Direction> {
      ...
      private static final Map<String, Direction.Axis> BY_NAME = (Map)Arrays.stream(values()).collect(Collectors.toMap(Direction.Axis::getName, (axis) -> {
         return axis;
      }));
      private final String name;
