...
public class BlockPos extends Vec3i implements DynamicSerializable {
   ...
   public static Stream<BlockPos> method_23627(BlockBox blockBox) {
      return stream(Math.min(blockBox.minX, blockBox.maxX), Math.min(blockBox.minY, blockBox.maxY), Math.min(blockBox.minZ, blockBox.maxZ), Math.max(blockBox.minX, blockBox.maxX), Math.max(blockBox.minY, blockBox.maxY), Math.max(blockBox.minZ, blockBox.maxZ));
   }
