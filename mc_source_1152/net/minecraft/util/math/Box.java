...
public class Box {
   ...
   public static Box from(BlockBox mutable) {
      return new Box((double)mutable.minX, (double)mutable.minY, (double)mutable.minZ, (double)(mutable.maxX + 1), (double)(mutable.maxY + 1), (double)(mutable.maxZ + 1));
   }
   ...
   public boolean equals(Object o) {
      if (this == o) {
         ...
         if (Double.compare(box.x1, this.x1) != 0) {
            ...
            return false;
         } else if (Double.compare(box.y2, this.y2) != 0) {
            return false;
         } else {
   ...
   private static Direction traceCollisionSide(Box box, Vec3d intersectingVector, double[] traceDistanceResult, @Nullable Direction approachDirection, double xDelta, double yDelta, double zDelta) {
      if (xDelta > 1.0E-7D) {
         approachDirection = traceCollisionSide(traceDistanceResult, approachDirection, xDelta, yDelta, zDelta, box.x1, box.y1, box.y2, box.z1, box.z2, Direction.WEST, intersectingVector.x, intersectingVector.y, intersectingVector.z);
      } else if (xDelta < -1.0E-7D) {
         approachDirection = traceCollisionSide(traceDistanceResult, approachDirection, xDelta, yDelta, zDelta, box.x2, box.y1, box.y2, box.z1, box.z2, Direction.EAST, intersectingVector.x, intersectingVector.y, intersectingVector.z);
      }

      if (yDelta > 1.0E-7D) {
         approachDirection = traceCollisionSide(traceDistanceResult, approachDirection, yDelta, zDelta, xDelta, box.y1, box.z1, box.z2, box.x1, box.x2, Direction.DOWN, intersectingVector.y, intersectingVector.z, intersectingVector.x);
      } else if (yDelta < -1.0E-7D) {
         approachDirection = traceCollisionSide(traceDistanceResult, approachDirection, yDelta, zDelta, xDelta, box.y2, box.z1, box.z2, box.x1, box.x2, Direction.UP, intersectingVector.y, intersectingVector.z, intersectingVector.x);
      }

      if (zDelta > 1.0E-7D) {
         approachDirection = traceCollisionSide(traceDistanceResult, approachDirection, zDelta, xDelta, yDelta, box.z1, box.x1, box.x2, box.y1, box.y2, Direction.NORTH, intersectingVector.z, intersectingVector.x, intersectingVector.y);
      } else if (zDelta < -1.0E-7D) {
         approachDirection = traceCollisionSide(traceDistanceResult, approachDirection, zDelta, xDelta, yDelta, box.z2, box.x1, box.x2, box.y1, box.y2, Direction.SOUTH, intersectingVector.z, intersectingVector.x, intersectingVector.y);
      }
   ...
   public Vec3d getCenter() {
      return new Vec3d(MathHelper.lerp(0.5D, this.x1, this.x2), MathHelper.lerp(0.5D, this.y1, this.y2), MathHelper.lerp(0.5D, this.z1, this.z2));
   }
}
