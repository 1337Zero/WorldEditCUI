...
public class WeightedList<U> {
   protected final List<WeightedList<U>.Entry<? extends U>> entries;
   ...
   public <T> WeightedList(Dynamic<T> dynamic, Function<Dynamic<T>, U> function) {
      ...
      dynamic.asStream().forEach((dynamicx) -> {
         dynamicx.get("data").map((dynamic2) -> {
            U object = function.apply(dynamic2);
   ...
   public <T> T serialize(DynamicOps<T> ops, Function<U, Dynamic<T>> entrySerializer) {
      return ops.createList(this.streamEntries().map((entry) -> {
         return ops.createMap(ImmutableMap.builder().put(ops.createString("data"), ((Dynamic)entrySerializer.apply(entry.getElement())).getValue()).put(ops.createString("weight"), ops.createInt(entry.getWeight())).build());
      }));
   }
