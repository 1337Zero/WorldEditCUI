...
public enum NetworkState {
   ...
   static {
      ...
      for(int var2 = 0; var2 < var1; ++var2) {
         NetworkState networkState = var0[var2];
         ...
         networkState.packetHandlers.forEach((networkSide, packetHandler) -> {
            packetHandler.getPacketTypes().forEach((var1) -> {
               if (HANDLER_STATE_MAP.containsKey(var1) && HANDLER_STATE_MAP.get(var1) != networkState) {
                  throw new IllegalStateException("Packet " + var1 + " is already assigned to protocol " + HANDLER_STATE_MAP.get(var1) + " - can't reassign to " + networkState);
               } else {
                  HANDLER_STATE_MAP.put(var1, networkState);
               }
            });
         });
      }
   ...
   static class PacketHandlerInitializer {
      ...
      private PacketHandlerInitializer() {
         this.packetHandlers = Maps.newEnumMap(NetworkSide.class);
      }
   ...
   static class PacketHandler<T extends PacketListener> {
      ...
      private final List<Supplier<? extends Packet<T>>> packetFactories;

      private PacketHandler() {
         this.packetIds = (Object2IntMap)Util.make(new Object2IntOpenHashMap(), (object2IntOpenHashMap) -> {
