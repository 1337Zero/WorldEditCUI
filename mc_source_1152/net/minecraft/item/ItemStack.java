...
public final class ItemStack {
   ...
   public List<Text> getTooltip(@Nullable PlayerEntity player, TooltipContext context) {
         ...
         if (this.tag.contains("display", 10)) {
            ...
            if (compoundTag.contains("color", 3)) {
               if (context.isAdvanced()) {
                  ...
                  list.add((new TranslatableText("item.dyed", new Object[0])).formatted(new Formatting[]{Formatting.GRAY, Formatting.ITALIC}));
               }
            }
      ...
      for(j = 0; j < l; ++j) {
         ...
         if (!multimap.isEmpty() && (i & 2) == 0) {
            ...
            while(var11.hasNext()) {
               Entry<String, EntityAttributeModifier> entry = (Entry)var11.next();
               ...
               if (bl) {
                  list.add((new LiteralText(" ")).append(new TranslatableText("attribute.modifier.equals." + entityAttributeModifier.getOperation().getId(), new Object[]{MODIFIER_FORMAT.format(f), new TranslatableText("attribute.name." + (String)entry.getKey(), new Object[0])})).formatted(Formatting.DARK_GREEN));
               } else if (d > 0.0D) {
                  list.add((new TranslatableText("attribute.modifier.plus." + entityAttributeModifier.getOperation().getId(), new Object[]{MODIFIER_FORMAT.format(f), new TranslatableText("attribute.name." + (String)entry.getKey(), new Object[0])})).formatted(Formatting.BLUE));
               } else if (d < 0.0D) {
                  f *= -1.0D;
                  list.add((new TranslatableText("attribute.modifier.take." + entityAttributeModifier.getOperation().getId(), new Object[]{MODIFIER_FORMAT.format(f), new TranslatableText("attribute.name." + (String)entry.getKey(), new Object[0])})).formatted(Formatting.RED));
               }
            }
         }
      }
      ...
      if (context.isAdvanced()) {
         ...
         if (this.hasTag()) {
            list.add((new TranslatableText("item.nbt_tags", new Object[]{this.getTag().getKeys().size()})).formatted(Formatting.DARK_GRAY));
         }
      }
   ...
   public Multimap<String, EntityAttributeModifier> getAttributeModifiers(EquipmentSlot slot) {
      ...
      if (this.hasTag() && this.tag.contains("AttributeModifiers", 9)) {
         ...
         for(int i = 0; i < listTag.size(); ++i) {
            ...
            if (entityAttributeModifier != null && (!compoundTag.contains("Slot", 8) || compoundTag.getString("Slot").equals(slot.getName())) && entityAttributeModifier.getId().getLeastSignificantBits() != 0L && entityAttributeModifier.getId().getMostSignificantBits() != 0L) {
               ((Multimap)multimap).put(compoundTag.getString("AttributeName"), entityAttributeModifier);
            }
         }
      } else {
      ...
      ((Multimap)multimap).values().forEach((entityAttributeModifierx) -> {
         entityAttributeModifierx.setSerialize(false);
      });
   ...
   public Text toHoverableText() {
      ...
      if (!this.empty) {
         ...
         text2.formatted(this.getRarity().formatting).styled((style) -> {
            style.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_ITEM, new LiteralText(compoundTag.toString())));
         });
      }
