...
public class CrossbowItem extends RangedWeaponItem {
   ...
   public CrossbowItem(Item.Settings settings) {
      ...
      this.addPropertyGetter(new Identifier("pulling"), (stack, world, entity) -> {
         return entity != null && entity.isUsingItem() && entity.getActiveItem() == stack && !isCharged(stack) ? 1.0F : 0.0F;
      });
      this.addPropertyGetter(new Identifier("charged"), (stack, world, entity) -> {
         return entity != null && isCharged(stack) ? 1.0F : 0.0F;
      });
      this.addPropertyGetter(new Identifier("firework"), (stack, world, entity) -> {
   ...
   private static void shoot(World world, LivingEntity shooter, Hand hand, ItemStack crossbow, ItemStack projectile, float soundPitch, boolean creative, float speed, float divergence, float simulated) {
      if (!world.isClient) {
         ...
         Object projectile3;
         if (bl) {
            projectile3 = new FireworkEntity(world, projectile, shooter.getX(), shooter.getEyeY() - 0.15000000596046448D, shooter.getZ(), true);
         } else {
            projectile3 = createArrow(world, shooter, crossbow, projectile);
            if (creative || simulated != 0.0F) {
               ((ProjectileEntity)projectile3).pickupType = ProjectileEntity.PickupPermission.CREATIVE_ONLY;
            }
         }

         if (shooter instanceof CrossbowUser) {
            CrossbowUser crossbowUser = (CrossbowUser)shooter;
            crossbowUser.shoot(crossbowUser.getTarget(), crossbow, (Projectile)projectile3, simulated);
         } else {
            ...
            ((Projectile)projectile3).setVelocity((double)vector3f.getX(), (double)vector3f.getY(), (double)vector3f.getZ(), speed, divergence);
         }

         crossbow.damage(bl ? 3 : 1, shooter, (e) -> {
            e.sendToolBreakStatus(hand);
         });
         world.spawnEntity((Entity)projectile3);
