...
public class FireworkItem extends Item {
   ...
   public ActionResult useOnBlock(ItemUsageContext context) {
      ...
      if (!world.isClient) {
         ...
         FireworkEntity fireworkEntity = new FireworkEntity(world, vec3d.x + (double)direction.getOffsetX() * 0.15D, vec3d.y + (double)direction.getOffsetY() * 0.15D, vec3d.z + (double)direction.getOffsetZ() * 0.15D, itemStack);
   ...
   public static enum Type {
      ...
      private static final FireworkItem.Type[] TYPES = (FireworkItem.Type[])Arrays.stream(values()).sorted(Comparator.comparingInt((type) -> {
         return type.id;
      })).toArray((i) -> {
         return new FireworkItem.Type[i];
      });
