...
public class ProfilingResourceReloader extends ResourceReloader<ProfilingResourceReloader.Summary> {
   ...
   public ProfilingResourceReloader(ResourceManager manager, List<ResourceReloadListener> listeners, Executor prepareExecutor, Executor applyExecutor, CompletableFuture<Unit> completableFuture) {
      super(prepareExecutor, applyExecutor, manager, listeners, (synchronizer, resourceManager, resourceReloadListener, prepareExecutorx, applyExecutorx) -> {
         AtomicLong atomicLong = new AtomicLong();
         AtomicLong atomicLong2 = new AtomicLong();
         ProfilerSystem profilerSystem = new ProfilerSystem(Util.getMeasuringTimeNano(), () -> {
            return 0;
         }, false);
         ProfilerSystem profilerSystem2 = new ProfilerSystem(Util.getMeasuringTimeNano(), () -> {
            return 0;
         }, false);
         CompletableFuture<Void> completableFuture = resourceReloadListener.reload(synchronizer, resourceManager, profilerSystem, profilerSystem2, (runnable) -> {
            prepareExecutorx.execute(() -> {
               long l = Util.getMeasuringTimeNano();
               runnable.run();
               atomicLong.addAndGet(Util.getMeasuringTimeNano() - l);
            });
         }, (runnable) -> {
            applyExecutorx.execute(() -> {
               long l = Util.getMeasuringTimeNano();
               runnable.run();
               atomicLong2.addAndGet(Util.getMeasuringTimeNano() - l);
            });
         });
         return completableFuture.thenApplyAsync((var5) -> {
            return new ProfilingResourceReloader.Summary(resourceReloadListener.getName(), profilerSystem.getResults(), profilerSystem2.getResults(), atomicLong, atomicLong2);
         }, applyExecutor);
      }, completableFuture);
   ...
   private void finish(List<ProfilingResourceReloader.Summary> summaries) {
      ...
      for(Iterator var3 = summaries.iterator(); var3.hasNext(); i += k) {
         ProfilingResourceReloader.Summary summary = (ProfilingResourceReloader.Summary)var3.next();
