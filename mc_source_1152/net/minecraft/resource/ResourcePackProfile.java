...
public class ResourcePackProfile implements AutoCloseable {
   ...
   @Nullable
   public static <T extends ResourcePackProfile> T of(String name, boolean alwaysEnabled, Supplier<ResourcePack> packFactory, ResourcePackProfile.Factory<T> containerFactory, ResourcePackProfile.InsertionPosition insertionPosition) {
      try {
   ...
   public ResourcePackProfile(String name, boolean alwaysEnabled, Supplier<ResourcePack> packFactory, Text displayName, Text description, ResourcePackCompatibility compatibility, ResourcePackProfile.InsertionPosition direction, boolean pinned) {
      this.name = name;
   ...
   public ResourcePackProfile(String name, boolean alwaysEnabled, Supplier<ResourcePack> packFactory, ResourcePack pack, PackResourceMetadata metadata, ResourcePackProfile.InsertionPosition direction) {
   ...
   public Text getInformationText(boolean enabled) {
      return Texts.bracketed(new LiteralText(this.name)).styled((style) -> {
         style.setColor(enabled ? Formatting.GREEN : Formatting.RED).setInsertion(StringArgumentType.escapeIfRequired(this.name)).setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, (new LiteralText("")).append(this.displayName).append("\n").append(this.description)));
      });
   }
   ...
   static {
      BROKEN_PACK_META = new PackResourceMetadata((new TranslatableText("resourcePack.broken_assets", new Object[0])).formatted(new Formatting[]{Formatting.RED, Formatting.ITALIC}), SharedConstants.getGameVersion().getPackVersion());
   }
   ...
   public interface Factory<T extends ResourcePackProfile> {
      @Nullable
      T create(String name, boolean alwaysEnabled, Supplier<ResourcePack> packFactory, ResourcePack pack, PackResourceMetadata meta, ResourcePackProfile.InsertionPosition insertionPosition);
   }
}
