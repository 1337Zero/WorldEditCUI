...
public class MinecraftClient extends ReentrantThreadExecutor<Runnable> implements SnooperListener, WindowEventHandler {
   ...
   private final DisableableProfiler profiler = new DisableableProfiler(() -> {
      return this.renderTickCounter.ticksThisFrame;
   });
   ...
   public MinecraftClient(RunArgs args) {
      ...
      if (textRenderer == null) {
         ...
         this.setOverlay(new SplashScreen(this, this.resourceManager.beginMonitoredReload(Util.getServerWorkerExecutor(), this, COMPLETED_UNIT_FUTURE, list), (optional) -> {
            Util.ifPresentOrElse(optional, this::handleResourceReloadExecption, () -> {
   ...
   private void handleResourceReloadExecption(Throwable throwable) {
      if (this.resourcePackManager.getEnabledProfiles().size() > 1) {
         ...
         this.reloadResources().thenRun(() -> {
            ...
            SystemToast.show(toastManager, SystemToast.Type.PACK_LOAD_FAILURE, new TranslatableText("resourcePack.load_fail", new Object[0]), text2);
         });
      } else {
   ...
   private void initializeSearchableContainers() {
      ...
      IdentifierSearchableContainer<ItemStack> identifierSearchableContainer = new IdentifierSearchableContainer((itemStack) -> {
         return ItemTags.getContainer().getTagsFor(itemStack.getItem()).stream();
      });
   ...
   private void render(boolean tick) {
      ...
      int j;
      if (tick) {
         for(j = 0; j < Math.min(10, this.renderTickCounter.ticksThisFrame); ++j) {
            this.tick();
         }
      }
   ...
   private void drawProfilerResults() {
      if (this.profiler.getController().isEnabled()) {
         ...
         int v;
         for(Iterator var11 = list.iterator(); var11.hasNext(); d += profilerTiming2.parentSectionUsagePercentage) {
            profilerTiming2 = (ProfilerTiming)var11.next();
         ...
         for(int w = 0; w < list.size(); ++w) {
            ...
            this.textRenderer.drawWithShadow(string3, (float)(j + 160 - 50 - this.textRenderer.getStringWidth(string3)), (float)(k + 80 + w * 8 + 20), profilerTiming3.getColor());
            string3 = decimalFormat.format(profilerTiming3.totalUsagePercentage) + "%";
            this.textRenderer.drawWithShadow(string3, (float)(j + 160 - this.textRenderer.getStringWidth(string3)), (float)(k + 80 + w * 8 + 20), profilerTiming3.getColor());
         }
   ...
   public void startIntegratedServer(String name, String displayName, @Nullable LevelInfo levelInfo) {
      this.disconnect();
      WorldSaveHandler worldSaveHandler = this.levelStorage.createSaveHandler(name, (MinecraftServer)null);
      ...
      try {
         ...
         crashReportSection.add("Level ID", (Object)name);
         crashReportSection.add("Level Name", (Object)displayName);
         throw new CrashException(crashReport);
      }
   ...
   public void addSnooperInfo(Snooper snooper) {
      ...
      while(var4.hasNext()) {
         ClientResourcePackProfile clientResourcePackProfile = (ClientResourcePackProfile)var4.next();
   ...
   public Function<Identifier, Sprite> getSpriteAtlas(Identifier identifier) {
      SpriteAtlasTexture var10000 = this.bakedModelManager.method_24153(identifier);
