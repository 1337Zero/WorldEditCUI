...
public class JsonGlProgram implements GlProgram, AutoCloseable {
   ...
   public void enable() {
      ...
      Iterator var4 = this.uniformData.iterator();

      while(var4.hasNext()) {
         GlUniform glUniform = (GlUniform)var4.next();
         glUniform.upload();
      }
   ...
   private void finalizeUniformsAndSamplers() {
      ...
      for(int j = 0; i < this.samplerNames.size(); ++j) {
         string2 = (String)this.samplerNames.get(i);
         l = GlUniform.getUniformLocation(this.programRef, string2);
         if (l == -1) {
            LOGGER.warn("Shader {}could not find sampler named {} in the specified shader program.", this.name, string2);
            this.samplerBinds.remove(string2);
            this.samplerNames.remove(j);
            --j;
         } else {
            this.samplerShaderLocs.add(l);
         }
