...
public class TextSearchableContainer<T> extends IdentifierSearchableContainer<T> {
   ...
   protected void index(T object) {
      super.index(object);
      ((Stream)this.textFinder.apply(object)).forEach((string) -> {
         this.byText.add(object, string.toLowerCase(Locale.ROOT));
      });
   }
   ...
   public List<T> findAll(String text) {
      ...
      if (i < 0) {
         ...
         return Lists.newArrayList(new IdentifierSearchableContainer.Iterator(list.iterator(), new TextSearchableContainer.Iterator(list2.iterator(), list3.iterator(), this::compare), this::compare));
      }
   }
