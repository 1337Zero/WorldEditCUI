...
public class ParticleManager implements ResourceReloadListener {
   ...
   public ParticleManager(World world, TextureManager textureManager) {
      this.particleAtlasTexture = new SpriteAtlasTexture(SpriteAtlasTexture.PARTICLE_ATLAS_TEX);
   ...
   private void registerDefaultFactories() {
      ...
      this.registerFactory(ParticleTypes.BUBBLE, (ParticleManager.SpriteAwareFactory)(BubbleColumnUpParticle.Factory::new));
      this.registerFactory(ParticleTypes.BUBBLE_COLUMN_UP, (ParticleManager.SpriteAwareFactory)(WaterBubbleParticle.Factory::new));
      ...
      this.registerFactory(ParticleTypes.LARGE_SMOKE, (ParticleManager.SpriteAwareFactory)(FireSmokeLargeParticle.Factory::new));
   ...
   public CompletableFuture<Void> reload(ResourceReloadListener.Synchronizer synchronizer, ResourceManager manager, Profiler prepareProfiler, Profiler applyProfiler, Executor prepareExecutor, Executor applyExecutor) {
      ...
      CompletableFuture var10000 = CompletableFuture.allOf(completableFutures).thenApplyAsync((var4) -> {
   ...
   public void addBlockBreakingParticles(BlockPos blockPos, Direction direction) {
      ...
      if (blockState.getRenderType() != BlockRenderType.INVISIBLE) {
         ...
         double d = (double)i + this.random.nextDouble() * (box.x2 - box.x1 - 0.20000000298023224D) + 0.10000000149011612D + box.x1;
         double e = (double)j + this.random.nextDouble() * (box.y2 - box.y1 - 0.20000000298023224D) + 0.10000000149011612D + box.y1;
         double g = (double)k + this.random.nextDouble() * (box.z2 - box.z1 - 0.20000000298023224D) + 0.10000000149011612D + box.z1;
         if (direction == Direction.DOWN) {
            e = (double)j + box.y1 - 0.10000000149011612D;
         }

         if (direction == Direction.UP) {
            e = (double)j + box.y2 + 0.10000000149011612D;
         }

         if (direction == Direction.NORTH) {
            g = (double)k + box.z1 - 0.10000000149011612D;
         }

         if (direction == Direction.SOUTH) {
            g = (double)k + box.z2 + 0.10000000149011612D;
         }

         if (direction == Direction.WEST) {
            d = (double)i + box.x1 - 0.10000000149011612D;
         }

         if (direction == Direction.EAST) {
            d = (double)i + box.x2 + 0.10000000149011612D;
         }

         this.addParticle((new BlockDustParticle(this.world, d, e, g, 0.0D, 0.0D, 0.0D, blockState)).setBlockPos(blockPos).move(0.2F).scale(0.6F));
      }
   }
