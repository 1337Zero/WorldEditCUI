...
public abstract class Particle {
   ...
   protected void setBoundingBoxSpacing(float spacingXZ, float spacingY) {
      if (spacingXZ != this.spacingXZ || spacingY != this.spacingY) {
         ...
         double d = (box.x1 + box.x2 - (double)spacingXZ) / 2.0D;
         double e = (box.z1 + box.z2 - (double)spacingXZ) / 2.0D;
   ...
   public void move(double dx, double dy, double dz) {
      if (!this.field_21507) {
         ...
         if (this.collidesWithWorld && (dx != 0.0D || dy != 0.0D || dz != 0.0D)) {
            Vec3d vec3d = Entity.adjustMovementForCollisions((Entity)null, new Vec3d(dx, dy, dz), this.getBoundingBox(), this.world, EntityContext.absent(), new ReusableStream(Stream.empty()));
            dx = vec3d.x;
            dy = vec3d.y;
            dz = vec3d.z;
         }
   ...
   protected void repositionFromBoundingBox() {
      Box box = this.getBoundingBox();
      this.x = (box.x1 + box.x2) / 2.0D;
      this.y = box.y1;
      this.z = (box.z1 + box.z2) / 2.0D;
   }
