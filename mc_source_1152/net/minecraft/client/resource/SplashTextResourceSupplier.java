...
public class SplashTextResourceSupplier extends SinglePreparationResourceReloadListener<List<String>> {
   ...
   protected List<String> prepare(ResourceManager resourceManager, Profiler profiler) {
      try {
         ...
         try {
            ...
            try {
               var7 = (List)bufferedReader.lines().map(String::trim).filter((string) -> {
   ...
   public String get() {
      ...
      if (calendar.get(2) + 1 == 12 && calendar.get(5) == 24) {
         ...
         return this.field_18934 != null && RANDOM.nextInt(this.splashTexts.size()) == 42 ? this.field_18934.getUsername().toUpperCase(Locale.ROOT) + " IS YOU" : (String)this.splashTexts.get(RANDOM.nextInt(this.splashTexts.size()));
      }
   }
}
