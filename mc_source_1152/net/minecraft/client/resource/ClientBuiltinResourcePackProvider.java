...
public class ClientBuiltinResourcePackProvider implements ResourcePackProvider {
   ...
   public <T extends ResourcePackProfile> void register(Map<String, T> registry, ResourcePackProfile.Factory<T> factory) {
      T resourcePackProfile = ResourcePackProfile.of("vanilla", true, () -> {
         return this.pack;
      ...
      if (file != null && file.isFile()) {
         T resourcePackProfile2 = ResourcePackProfile.of("programer_art", false, () -> {
   ...
   public CompletableFuture<?> download(String string, String string2) {
      ...
      try {
         ...
         this.downloadTask = completableFuture2.thenCompose((object) -> {
            return !this.verifyFile(string4, file) ? Util.completeExceptionally(new RuntimeException("Hash check failure for file " + file + ", see log")) : this.loadServerPack(file);
         }).whenComplete((var1, throwable) -> {
            if (throwable != null) {
   ...
   private boolean verifyFile(String expectedSha1, File rfile) {
      try {
         ...
         LOGGER.warn("File {} had wrong hash (expected {}, found {}).", rfile, expectedSha1, string2);
   ...
   private void deleteOldServerPack() {
      try {
         List<File> list = Lists.newArrayList(FileUtils.listFiles(this.serverPacksRoot, TrueFileFilter.TRUE, (IOFileFilter)null));
   ...
   public CompletableFuture<Void> loadServerPack(File packZip) {
      ...
      if (string != null) {
         ...
         this.serverContainer = new ClientResourcePackProfile("server", true, () -> {
            return new ZipResourcePack(packZip);
         }, new TranslatableText("resourcePack.server.name", new Object[0]), packResourceMetadata.getDescription(), ResourcePackCompatibility.from(packResourceMetadata.getPackFormat()), ResourcePackProfile.InsertionPosition.TOP, true, nativeImage);
