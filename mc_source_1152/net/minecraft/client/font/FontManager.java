   ...
   private final ResourceReloadListener resourceReloadListener = new SinglePreparationResourceReloadListener<Map<Identifier, List<Font>>>() {
      protected Map<Identifier, List<Font>> prepare(ResourceManager resourceManager, Profiler profiler) {
         ...
         while(var5.hasNext()) {
            ...
            try {
               for(Iterator var10 = resourceManager.getAllResources(identifier).iterator(); var10.hasNext(); profiler.pop()) {
                  ...
                  try {
                        ...
                        try {
                           ...
                           for(int i = jsonArray.size() - 1; i >= 0; --i) {
                              ...
                              try {
                                 ...
                                 FontManager.LOGGER.warn("Unable to read definition '{}' in fonts.json in resourcepack: '{}': {}", identifier2, resource.getResourcePackName(), var48.getMessage());
                              }
                           }
                     ...
                     FontManager.LOGGER.warn("Unable to load font '{}' in fonts.json in resourcepack: '{}': {}", identifier2, resource.getResourcePackName(), var53.getMessage());
                  }
               }
            ...
            for(char c = 0; c < '\uffff'; ++c) {
               if (c != ' ') {
                  Iterator var56 = Lists.reverse(list).iterator();

                  while(var56.hasNext()) {
                     Font font = (Font)var56.next();
      ...
      protected void apply(Map<Identifier, List<Font>> map, ResourceManager resourceManager, Profiler profiler) {
         ...
         Stream.concat(FontManager.this.textRenderers.keySet().stream(), map.keySet().stream()).distinct().forEach((identifier) -> {
            List<Font> list = (List)map.getOrDefault(identifier, Collections.emptyList());
            Collections.reverse(list);
            ((TextRenderer)FontManager.this.textRenderers.computeIfAbsent(identifier, (identifierx) -> {
               return new TextRenderer(FontManager.this.textureManager, new FontStorage(FontManager.this.textureManager, identifierx));
            })).setFonts(list);
         });
         profiler.pop();
         profiler.endTick();
