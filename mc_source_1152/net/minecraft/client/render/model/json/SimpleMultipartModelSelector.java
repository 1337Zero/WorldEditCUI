...
public class SimpleMultipartModelSelector implements MultipartModelSelector {
   ...
   public Predicate<BlockState> getPredicate(StateManager<Block, BlockState> stateFactory) {
      ...
      if (property == null) {
         throw new RuntimeException(String.format("Unknown property '%s' on '%s'", this.key, ((Block)stateFactory.getOwner()).toString()));
      } else {
         ...
         if (list.isEmpty()) {
            throw new RuntimeException(String.format("Empty value '%s' for property '%s' on '%s'", this.valueString, this.key, ((Block)stateFactory.getOwner()).toString()));
         } else {
            Predicate predicate2;
            if (list.size() == 1) {
               predicate2 = this.createPredicate(stateFactory, property, string);
            } else {
               List<Predicate<BlockState>> list2 = (List)list.stream().map((stringx) -> {
                  return this.createPredicate(stateFactory, property, stringx);
