...
public class ModelItemOverride {
   ...
   boolean matches(ItemStack stack, @Nullable World world, @Nullable LivingEntity entity) {
      ...
      Iterator var5 = this.minPropertyValues.entrySet().iterator();

      Entry entry;
      ItemPropertyGetter itemPropertyGetter;
      do {
         if (!var5.hasNext()) {
            return true;
         }
         ...
         itemPropertyGetter = item.getPropertyGetter((Identifier)entry.getKey());
      } while(itemPropertyGetter != null && itemPropertyGetter.call(stack, world, entity) >= (Float)entry.getValue());
