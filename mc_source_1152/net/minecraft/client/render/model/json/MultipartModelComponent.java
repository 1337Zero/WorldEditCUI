...
public class MultipartModelComponent {
   ...
   public static class Deserializer implements JsonDeserializer<MultipartModelComponent> {
      public MultipartModelComponent deserialize(JsonElement element, Type type, JsonDeserializationContext context) throws JsonParseException {
         ...
         return new MultipartModelComponent(this.deserializeSelectorOrDefault(jsonObject), (WeightedUnbakedModel)context.deserialize(jsonObject.get("apply"), WeightedUnbakedModel.class));
      }
      ...
      static MultipartModelSelector deserializeSelector(JsonObject object) {
         ...
         if (set.isEmpty()) {
            ...
            List list2;
            if (object.has("OR")) {
               list2 = (List)Streams.stream(JsonHelper.getArray(object, "OR")).map((jsonElement) -> {
                  return deserializeSelector(jsonElement.getAsJsonObject());
               }).collect(Collectors.toList());
               return new OrMultipartModelSelector(list2);
            } else if (object.has("AND")) {
               list2 = (List)Streams.stream(JsonHelper.getArray(object, "AND")).map((jsonElement) -> {
                  return deserializeSelector(jsonElement.getAsJsonObject());
               }).collect(Collectors.toList());
               return new AndMultipartModelSelector(list2);
            } else {
               return createStatePropertySelector((Entry)set.iterator().next());
            }
         } else {
            return new AndMultipartModelSelector((Iterable)set.stream().map(MultipartModelComponent.Deserializer::createStatePropertySelector).collect(Collectors.toList()));
         }
      }
