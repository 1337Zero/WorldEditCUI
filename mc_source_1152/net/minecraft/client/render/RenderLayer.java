...
public abstract class RenderLayer extends RenderPhase {
   ...
   private static RenderLayer.MultiPhaseParameters createTranslucentPhaseData() {
      return RenderLayer.MultiPhaseParameters.builder().shadeModel(SMOOTH_SHADE_MODEL).lightmap(ENABLE_LIGHTMAP).texture(MIPMAP_BLOCK_ATLAS_TEXTURE).transparency(TRANSLUCENT_TRANSPARENCY).build(true);
   }
   ...
   public static RenderLayer getOutline(Identifier texture) {
      return of("outline", VertexFormats.POSITION_COLOR_TEXTURE, 7, 256, RenderLayer.MultiPhaseParameters.builder().texture(new RenderPhase.Texture(texture, false, false)).cull(DISABLE_CULLING).depthTest(ALWAYS_DEPTH_TEST).alpha(ONE_TENTH_ALPHA).texturing(OUTLINE_TEXTURING).fog(NO_FOG).target(OUTLINE_TARGET).build(RenderLayer.OutlineMode.IS_OUTLINE));
   }
   ...
   public static RenderLayer getEndPortal(int layer) {
      ...
      if (layer <= 1) {
         ...
         texture2 = new RenderPhase.Texture(EndPortalBlockEntityRenderer.SKY_TEX, false, false);
      } else {
         transparency2 = ADDITIVE_TRANSPARENCY;
         texture2 = new RenderPhase.Texture(EndPortalBlockEntityRenderer.PORTAL_TEX, false, false);
      }
   ...
   static {
      ...
      LIGHTNING = of("lightning", VertexFormats.POSITION_COLOR, 7, 256, false, true, RenderLayer.MultiPhaseParameters.builder().writeMaskState(COLOR_MASK).transparency(LIGHTNING_TRANSPARENCY).shadeModel(SMOOTH_SHADE_MODEL).build(false));
      LINES = of("lines", VertexFormats.POSITION_COLOR, 1, 256, RenderLayer.MultiPhaseParameters.builder().lineWidth(new RenderPhase.LineWidth(OptionalDouble.empty())).layering(PROJECTION_LAYERING).transparency(TRANSLUCENT_TRANSPARENCY).writeMaskState(COLOR_MASK).build(false));
   }
   ...
   static final class MultiPhase extends RenderLayer {
      ...
      private MultiPhase(String name, VertexFormat vertexFormat, int drawMode, int expectedBufferSize, boolean hasCrumbling, boolean translucent, RenderLayer.MultiPhaseParameters phases) {
         ...
         this.phases = phases;
         this.affectedOutline = phases.outlineMode == RenderLayer.OutlineMode.AFFECTS_OUTLINE ? phases.texture.getId().map(RenderLayer::getOutline) : Optional.empty();
   ...
   public static final class MultiPhaseParameters {
      ...
      public static class Builder {
         ...
         private Builder() {
            ...
            this.lineWidth = RenderPhase.FULL_LINEWIDTH;
         }
