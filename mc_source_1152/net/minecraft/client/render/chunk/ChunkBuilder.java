...
public class ChunkBuilder {
   ...
   private void scheduleRunTasks() {
      if (!this.threadBuffers.isEmpty()) {
         ...
         if (task != null) {
            ...
            }, this.executor).thenCompose((var2) -> {
   ...
   public class BuiltChunk {
      ...
      public void setOrigin(int x, int y, int z) {
         if (x != this.origin.getX() || y != this.origin.getY() || z != this.origin.getZ()) {
            ...
            for(int var6 = 0; var6 < var5; ++var6) {
               ...
               this.neighborPositions[direction.ordinal()].set((Vec3i)this.origin).setOffset(direction, 16);
            }
      ...
      protected double getSquaredCameraDistance() {
         ...
         double d = this.boundingBox.x1 + 8.0D - camera.getPos().x;
         double e = this.boundingBox.y1 + 8.0D - camera.getPos().y;
         double f = this.boundingBox.z1 + 8.0D - camera.getPos().z;
         return d * d + e * e + f * f;
      }
            ...
            if (this.cancelled.get()) {
               ...
               if (state != null && this.data.nonEmptyLayers.contains(RenderLayer.getTranslucent())) {
                  ...
                  if (this.cancelled.get()) {
                     ...
                     CompletableFuture<ChunkBuilder.Result> completableFuture = ChunkBuilder.this.scheduleUpload(buffers.get(RenderLayer.getTranslucent()), BuiltChunk.this.getBuffer(RenderLayer.getTranslucent())).thenApply((var0) -> {
