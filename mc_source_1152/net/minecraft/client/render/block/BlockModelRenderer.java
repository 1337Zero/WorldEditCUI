...
public class BlockModelRenderer {
   ...
   public boolean render(BlockRenderView view, BakedModel model, BlockState state, BlockPos pos, MatrixStack matrix, VertexConsumer vertexConsumer, boolean cull, Random random, long seed, int overlay) {
      ...
      try {
         return bl ? this.renderSmooth(view, model, state, pos, matrix, vertexConsumer, cull, random, seed, overlay) : this.renderFlat(view, model, state, pos, matrix, vertexConsumer, cull, random, seed, overlay);
   ...
   private void renderQuadsFlat(BlockRenderView world, BlockState state, BlockPos pos, int light, int overlay, boolean useWorldLight, MatrixStack matrix, VertexConsumer vertexConsumer, List<BakedQuad> quads, BitSet flags) {
      BakedQuad bakedQuad;
      for(Iterator var11 = quads.iterator(); var11.hasNext(); this.renderQuad(world, state, pos, vertexConsumer, matrix.peek(), bakedQuad, 1.0F, 1.0F, 1.0F, 1.0F, light, light, light, light, overlay)) {
   ...
   class AmbientOcclusionCalculator {
      ...
      public void apply(BlockRenderView world, BlockState state, BlockPos pos, Direction direction, float[] box, BitSet flags) {
         ...
         mutable.set((Vec3i)blockPos).setOffset(neighborData.faces[2]);
         ...
         mutable.set((Vec3i)blockPos).setOffset(neighborData.faces[3]);
         ...
         mutable.set((Vec3i)blockPos).setOffset(neighborData.faces[0]).setOffset(direction);
         boolean bl = world.getBlockState(mutable).getOpacity(world, mutable) == 0;
         mutable.set((Vec3i)blockPos).setOffset(neighborData.faces[1]).setOffset(direction);
         boolean bl2 = world.getBlockState(mutable).getOpacity(world, mutable) == 0;
         mutable.set((Vec3i)blockPos).setOffset(neighborData.faces[2]).setOffset(direction);
         boolean bl3 = world.getBlockState(mutable).getOpacity(world, mutable) == 0;
         mutable.set((Vec3i)blockPos).setOffset(neighborData.faces[3]).setOffset(direction);
         ...
         if (!bl3 && !bl) {
            ...
            mutable.set((Vec3i)blockPos).setOffset(neighborData.faces[0]).setOffset(neighborData.faces[2]);
         ...
         if (!bl4 && !bl) {
            ...
            mutable.set((Vec3i)blockPos).setOffset(neighborData.faces[0]).setOffset(neighborData.faces[3]);
            blockState8 = world.getBlockState(mutable);
         ...
         if (!bl3 && !bl2) {
            ...
            mutable.set((Vec3i)blockPos).setOffset(neighborData.faces[1]).setOffset(neighborData.faces[2]);
            blockState8 = world.getBlockState(mutable);
         ...
         if (!bl4 && !bl2) {
            ...
            mutable.set((Vec3i)blockPos).setOffset(neighborData.faces[1]).setOffset(neighborData.faces[3]);
            blockState8 = world.getBlockState(mutable);
         ...
         if (flags.get(0) || !blockState9.isFullOpaque(world, mutable)) {
         ...
         if (flags.get(1) && neighborData.nonCubicWeight) {
            ...
            this.brightness[translation.firstCorner] = af * an + ag * ao + ah * ap + ai * aq;
            this.brightness[translation.secondCorner] = af * ar + ag * as + ah * at + ai * au;
            this.brightness[translation.thirdCorner] = af * av + ag * aw + ah * ax + ai * ay;
            this.brightness[translation.fourthCorner] = af * az + ag * ba + ah * bb + ai * bc;
   ...
   static class BrightnessCache {
      ...
      public int getInt(BlockState blockState, BlockRenderView blockRenderView, BlockPos pos) {
         ...
         j = WorldRenderer.getLightmapCoordinates(blockRenderView, blockState, pos);
         if (this.enabled) {
