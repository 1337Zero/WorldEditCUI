...
public class BlockRenderManager implements SynchronousResourceReloadListener {
   ...
   public boolean renderBlock(BlockState state, BlockPos pos, BlockRenderView world, MatrixStack matrix, VertexConsumer vertexConsumer, boolean cull, Random random) {
      try {
         ...
         return blockRenderType != BlockRenderType.MODEL ? false : this.blockModelRenderer.render(world, this.getModel(state), state, pos, matrix, vertexConsumer, cull, random, state.getRenderingSeed(pos), OverlayTexture.DEFAULT_UV);
   ...
   public boolean renderFluid(BlockPos blockPos, BlockRenderView blockRenderView, VertexConsumer vertexConsumer, FluidState fluidState) {
      try {
         ...
         CrashReportSection crashReportSection = crashReport.addElement("Block being tesselated");
         CrashReportSection.addBlockInfo(crashReportSection, blockPos, (BlockState)null);
         throw new CrashException(crashReport);
      }
   }
