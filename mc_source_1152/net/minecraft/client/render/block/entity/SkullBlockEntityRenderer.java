...
public class SkullBlockEntityRenderer extends BlockEntityRenderer<SkullBlockEntity> {
   private static final Map<SkullBlock.SkullType, SkullEntityModel> MODELS = (Map)Util.make(Maps.newHashMap(), (hashMap) -> {
   ...
   public static void render(@Nullable Direction direction, float f, SkullBlock.SkullType skullType, @Nullable GameProfile gameProfile, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
      ...
      if (direction == null) {
         ...
         switch(direction) {
         case NORTH:
            matrixStack.translate(0.5D, 0.25D, 0.7400000095367432D);
            break;
         case SOUTH:
            matrixStack.translate(0.5D, 0.25D, 0.25999999046325684D);
            break;
         case WEST:
            matrixStack.translate(0.7400000095367432D, 0.25D, 0.5D);
            break;
         case EAST:
         default:
            matrixStack.translate(0.25999999046325684D, 0.25D, 0.5D);
         }
      }
   ...
   private static RenderLayer method_3578(SkullBlock.SkullType skullType, @Nullable GameProfile gameProfile) {
      ...
      if (skullType == SkullBlock.Type.PLAYER && gameProfile != null) {
         ...
         return map.containsKey(Type.SKIN) ? RenderLayer.getEntityTranslucent(minecraftClient.getSkinProvider().loadSkin((MinecraftProfileTexture)map.get(Type.SKIN), Type.SKIN)) : RenderLayer.getEntityCutoutNoCull(DefaultSkinHelper.getTexture(PlayerEntity.getUuidFromProfile(gameProfile)));
      } else {
