...
public class GameRenderer implements AutoCloseable, SynchronousResourceReloadListener {
   ...
   private void bobViewWhenHurt(MatrixStack matrixStack, float f) {
      if (this.client.getCameraEntity() instanceof LivingEntity) {
         ...
         if (livingEntity.getHealth() <= 0.0F) {
            ...
            matrixStack.multiply(Vector3f.POSITIVE_Z.getDegreesQuaternion(40.0F - 8000.0F / (i + 200.0F)));
         }
   ...
   private void bobView(MatrixStack matrixStack, float f) {
      if (this.client.getCameraEntity() instanceof PlayerEntity) {
         ...
         matrixStack.multiply(Vector3f.POSITIVE_Z.getDegreesQuaternion(MathHelper.sin(h * 3.1415927F) * i * 3.0F));
         matrixStack.multiply(Vector3f.POSITIVE_X.getDegreesQuaternion(Math.abs(MathHelper.cos(h * 3.1415927F - 0.2F) * i) * 5.0F));
      }
   }
