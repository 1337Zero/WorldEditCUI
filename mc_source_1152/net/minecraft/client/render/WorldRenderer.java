...
public class WorldRenderer implements AutoCloseable, SynchronousResourceReloadListener {
   ...
   private final Vector4f[] capturedFrustrumOrientation;
   ...
   public WorldRenderer(MinecraftClient client, BufferBuilderStorage bufferBuilders) {
      ...
      this.capturedFrustrumOrientation = new Vector4f[8];
   ...
   private void setupTerrain(Camera camera, Frustum frustum, boolean hasForcedFrustum, int frame, boolean spectator) {
      ...
      if (!hasForcedFrustum && this.needsTerrainUpdate) {
         ...
         if (builtChunk != null) {
            boolean bl2 = false;
            WorldRenderer.ChunkInfo chunkInfo = new WorldRenderer.ChunkInfo(builtChunk, (Direction)null, 0);
   ...
   public void render(MatrixStack matrices, float tickDelta, long limitTime, boolean renderBlockOutline, Camera camera, GameRenderer gameRenderer, LightmapTextureManager lightmapTextureManager, Matrix4f matrix4f) {
      ...
      boolean bl2 = this.client.world.dimension.isFogThick(MathHelper.floor(d), MathHelper.floor(e)) || this.client.inGameHud.getBossBarHud().shouldThickenFog();
                     ...
                     while(true) {
                        ...
                        do {
                           if (!var52.hasNext()) {
                              ...
                              while(var53.hasNext()) {
                                 Entry<SortedSet<BlockBreakingInfo>> entry = (Entry)var53.next();
   ...
   private void method_22984(VertexConsumer vertexConsumer, int i) {
      vertexConsumer.vertex((double)this.capturedFrustrumOrientation[i].getX(), (double)this.capturedFrustrumOrientation[i].getY(), (double)this.capturedFrustrumOrientation[i].getZ()).next();
   }

   private void method_22985(VertexConsumer vertexConsumer, int i, int j, int k, int l, int m, int n, int o) {
      float f = 0.25F;
      vertexConsumer.vertex((double)this.capturedFrustrumOrientation[i].getX(), (double)this.capturedFrustrumOrientation[i].getY(), (double)this.capturedFrustrumOrientation[i].getZ()).color((float)m, (float)n, (float)o, 0.25F).next();
      vertexConsumer.vertex((double)this.capturedFrustrumOrientation[j].getX(), (double)this.capturedFrustrumOrientation[j].getY(), (double)this.capturedFrustrumOrientation[j].getZ()).color((float)m, (float)n, (float)o, 0.25F).next();
      vertexConsumer.vertex((double)this.capturedFrustrumOrientation[k].getX(), (double)this.capturedFrustrumOrientation[k].getY(), (double)this.capturedFrustrumOrientation[k].getZ()).color((float)m, (float)n, (float)o, 0.25F).next();
      vertexConsumer.vertex((double)this.capturedFrustrumOrientation[l].getX(), (double)this.capturedFrustrumOrientation[l].getY(), (double)this.capturedFrustrumOrientation[l].getZ()).color((float)m, (float)n, (float)o, 0.25F).next();
   }
   ...
   public void renderClouds(MatrixStack matrices, float tickDelta, double cameraX, double cameraY, double cameraZ) {
      if (this.client.world.dimension.hasVisibleSky()) {
         ...
         double h = (cameraX + e) / 12.0D;
         double i = (double)(this.world.dimension.getCloudHeight() - (float)cameraY + 0.33F);
         double j = cameraZ / 12.0D + 0.33000001311302185D;
         h -= (double)(MathHelper.floor(h / 2048.0D) * 2048);
         j -= (double)(MathHelper.floor(j / 2048.0D) * 2048);
         float k = (float)(h - (double)MathHelper.floor(h));
         float l = (float)(i / 4.0D - (double)MathHelper.floor(i / 4.0D)) * 4.0F;
         float m = (float)(j - (double)MathHelper.floor(j));
         Vec3d vec3d = this.world.getCloudsColor(tickDelta);
         int n = (int)Math.floor(h);
         int o = (int)Math.floor(i / 4.0D);
         int p = (int)Math.floor(j);
         if (n != this.lastCloudsBlockX || o != this.lastCloudsBlockY || p != this.lastCloudsBlockZ || this.client.options.getCloudRenderMode() != this.lastCloudsRenderMode || this.lastCloudsColor.squaredDistanceTo(vec3d) > 2.0E-4D) {
         ...
         matrices.translate((double)(-k), (double)l, (double)(-m));
         if (this.cloudsBuffer != null) {
            this.cloudsBuffer.bind();
            VertexFormats.POSITION_TEXTURE_COLOR_NORMAL.startDrawing(0L);
            int q = this.lastCloudsRenderMode == CloudRenderMode.FANCY ? 0 : 1;
   ...
   private void drawBlockOutline(MatrixStack matrixStack, VertexConsumer vertexConsumer, Entity entity, double d, double e, double f, BlockPos blockPos, BlockState blockState) {
      drawShapeOutline(matrixStack, vertexConsumer, blockState.getOutlineShape(this.world, blockPos, EntityContext.of(entity)), (double)blockPos.getX() - d, (double)blockPos.getY() - e, (double)blockPos.getZ() - f, 0.0F, 0.0F, 0.0F, 0.4F);
   }
   ...
   public void playSong(@Nullable SoundEvent song, BlockPos songPosition) {
      ...
      if (song != null) {
         ...
         SoundInstance soundInstance = PositionedSoundInstance.record(song, (float)songPosition.getX(), (float)songPosition.getY(), (float)songPosition.getZ());
   ...
   public void addParticle(ParticleEffect parameters, boolean shouldAlwaysSpawn, boolean isImportant, double x, double y, double z, double velocityX, double velocityY, double velocityY) {
      try {
   ...
   public void playLevelEvent(PlayerEntity source, int type, BlockPos pos, int data) {
      ...
      case 1501:
         ...
         for(i = 0; i < 8; ++i) {
            this.world.addParticle(ParticleTypes.LARGE_SMOKE, (double)pos.getX() + Math.random(), (double)pos.getY() + 1.2D, (double)pos.getZ() + Math.random(), 0.0D, 0.0D, 0.0D);
         }
      ...
      case 1502:
         ...
         for(i = 0; i < 5; ++i) {
            x = (double)pos.getX() + random.nextDouble() * 0.6D + 0.2D;
            y = (double)pos.getY() + random.nextDouble() * 0.6D + 0.2D;
            v = (double)pos.getZ() + random.nextDouble() * 0.6D + 0.2D;
            this.world.addParticle(ParticleTypes.SMOKE, x, y, v, 0.0D, 0.0D, 0.0D);
         }
      ...
      case 1503:
         ...
         for(i = 0; i < 16; ++i) {
            x = (double)((float)pos.getX() + (5.0F + random.nextFloat() * 6.0F) / 16.0F);
            y = (double)((float)pos.getY() + 0.8125F);
            v = (double)((float)pos.getZ() + (5.0F + random.nextFloat() * 6.0F) / 16.0F);
            f = 0.0D;
            double ay = 0.0D;
            double az = 0.0D;
            this.world.addParticle(ParticleTypes.SMOKE, x, y, v, 0.0D, 0.0D, 0.0D);
      ...
      case 2000:
         ...
         y = (double)pos.getX() + (double)i * 0.6D + 0.5D;
         v = (double)pos.getY() + (double)j * 0.6D + 0.5D;
         f = (double)pos.getZ() + (double)k * 0.6D + 0.5D;

         for(ad = 0; ad < 10; ++ad) {
            ae = random.nextDouble() * 0.2D + 0.01D;
            af = y + (double)i * 0.01D + (random.nextDouble() - 0.5D) * (double)k * 0.5D;
            ag = v + (double)j * 0.01D + (random.nextDouble() - 0.5D) * (double)j * 0.5D;
            ah = f + (double)k * 0.01D + (random.nextDouble() - 0.5D) * (double)i * 0.5D;
            ai = (double)i * ae + random.nextGaussian() * 0.01D;
            double p = (double)j * ae + random.nextGaussian() * 0.01D;
            double q = (double)k * ae + random.nextGaussian() * 0.01D;
            this.addParticle(ParticleTypes.SMOKE, af, ag, ah, ai, p, q);
         }
      ...
      case 2007:
         ...
         for(u = 0; u < 8; ++u) {
            this.addParticle(new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(Items.SPLASH_POTION)), w, x, y, random.nextGaussian() * 0.15D, random.nextDouble() * 0.2D, random.nextGaussian() * 0.15D);
         }

         float aa = (float)(data >> 16 & 255) / 255.0F;
         float ab = (float)(data >> 8 & 255) / 255.0F;
         float ac = (float)(data >> 0 & 255) / 255.0F;
         ParticleEffect particleEffect = type == 2007 ? ParticleTypes.INSTANT_EFFECT : ParticleTypes.EFFECT;

         for(ad = 0; ad < 100; ++ad) {
            ae = random.nextDouble() * 4.0D;
            af = random.nextDouble() * 3.141592653589793D * 2.0D;
            ag = Math.cos(af) * ae;
            ah = 0.01D + random.nextDouble() * 0.5D;
            ai = Math.sin(af) * ae;
            Particle particle = this.spawnParticle(particleEffect, particleEffect.getType().shouldAlwaysSpawn(), w + ag * 0.1D, x + 0.3D, y + ai * 0.1D, ag, ah, ai);
            if (particle != null) {
         ...
         break;
      case 2003:
         w = (double)pos.getX() + 0.5D;
         x = (double)pos.getY();
         y = (double)pos.getZ() + 0.5D;

         for(u = 0; u < 8; ++u) {
            this.addParticle(new ItemStackParticleEffect(ParticleTypes.ITEM, new ItemStack(Items.ENDER_EYE)), w, x, y, random.nextGaussian() * 0.15D, random.nextDouble() * 0.2D, random.nextGaussian() * 0.15D);
         }

         for(v = 0.0D; v < 6.283185307179586D; v += 0.15707963267948966D) {
            this.addParticle(ParticleTypes.PORTAL, w + Math.cos(v) * 5.0D, x - 0.4D, y + Math.sin(v) * 5.0D, Math.cos(v) * -5.0D, 0.0D, Math.sin(v) * -5.0D);
            this.addParticle(ParticleTypes.PORTAL, w + Math.cos(v) * 5.0D, x - 0.4D, y + Math.sin(v) * 5.0D, Math.cos(v) * -7.0D, 0.0D, Math.sin(v) * -7.0D);
         }

         return;
      case 2004:
         for(i = 0; i < 20; ++i) {
            x = (double)pos.getX() + 0.5D + ((double)this.world.random.nextFloat() - 0.5D) * 2.0D;
            y = (double)pos.getY() + 0.5D + ((double)this.world.random.nextFloat() - 0.5D) * 2.0D;
            v = (double)pos.getZ() + 0.5D + ((double)this.world.random.nextFloat() - 0.5D) * 2.0D;
            this.world.addParticle(ParticleTypes.SMOKE, x, y, v, 0.0D, 0.0D, 0.0D);
            this.world.addParticle(ParticleTypes.FLAME, x, y, v, 0.0D, 0.0D, 0.0D);
         }
      ...
      case 2006:
         for(i = 0; i < 200; ++i) {
            float bb = random.nextFloat() * 4.0F;
            float bc = random.nextFloat() * 6.2831855F;
            y = (double)(MathHelper.cos(bc) * bb);
            v = 0.01D + random.nextDouble() * 0.5D;
            f = (double)(MathHelper.sin(bc) * bb);
            Particle particle2 = this.spawnParticle(ParticleTypes.DRAGON_BREATH, false, (double)pos.getX() + y * 0.1D, (double)pos.getY() + 0.3D, (double)pos.getZ() + f * 0.1D, y, v, f);
            if (particle2 != null) {
               particle2.move(bb);
            }
         }
      ...
      case 2009:
         for(i = 0; i < 8; ++i) {
            this.world.addParticle(ParticleTypes.CLOUD, (double)pos.getX() + Math.random(), (double)pos.getY() + 1.2D, (double)pos.getZ() + Math.random(), 0.0D, 0.0D, 0.0D);
         }
