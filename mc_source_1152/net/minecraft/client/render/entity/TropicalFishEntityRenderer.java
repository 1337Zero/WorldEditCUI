...
public class TropicalFishEntityRenderer extends MobEntityRenderer<TropicalFishEntity, EntityModel<TropicalFishEntity>> {
   ...
   public void render(TropicalFishEntity tropicalFishEntity, float f, float g, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, int i) {
      TintableCompositeModel<TropicalFishEntity> tintableCompositeModel = tropicalFishEntity.getShape() == 0 ? this.field_4800 : this.field_4799;
      this.model = (EntityModel)tintableCompositeModel;
      float[] fs = tropicalFishEntity.getBaseColorComponents();
      ((TintableCompositeModel)tintableCompositeModel).setColorMultiplier(fs[0], fs[1], fs[2]);
      super.render((MobEntity)tropicalFishEntity, f, g, matrixStack, vertexConsumerProvider, i);
      ((TintableCompositeModel)tintableCompositeModel).setColorMultiplier(1.0F, 1.0F, 1.0F);
   }
