...
public class EntityRenderDispatcher {
   ...
   public <E extends Entity> void render(E entity, double x, double y, double z, float yaw, float tickDelta, MatrixStack matrices, VertexConsumerProvider vertexConsumers, int light) {
      ...
      try {
         ...
         if (this.gameOptions.entityShadows && this.renderShadows && entityRenderer.shadowSize > 0.0F && !entity.isInvisible()) {
            ...
            float h = (float)((1.0D - g / 256.0D) * (double)entityRenderer.shadowDarkness);
            if (h > 0.0F) {
   ...
   private void renderHitbox(MatrixStack matrixStack, VertexConsumer vertexConsumer, Entity entity, float f) {
      ...
      if (entity instanceof LivingEntity) {
         float l = 0.01F;
         WorldRenderer.drawBox(matrixStack, vertexConsumer, (double)(-g), (double)(entity.getStandingEyeHeight() - 0.01F), (double)(-g), (double)g, (double)(entity.getStandingEyeHeight() + 0.01F), (double)g, 1.0F, 0.0F, 0.0F, 1.0F);
      }
      ...
      vertexConsumer.vertex(matrix4f, (float)(vec3d.x * 2.0D), (float)((double)entity.getStandingEyeHeight() + vec3d.y * 2.0D), (float)(vec3d.z * 2.0D)).color(0, 0, 255, 255).next();
   }
   ...
   private void renderFire(MatrixStack matrix, VertexConsumerProvider vertexConsumerProvider, Entity entity) {
      ...
      matrix.translate(0.0D, 0.0D, (double)(-0.3F + (float)((int)i) * 0.02F));
      float k = 0.0F;
      int l = 0;
      ...
      if (blockState.getRenderType() != BlockRenderType.INVISIBLE && world.getLightLevel(pos) > 3) {
         if (blockState.isFullCube(world, blockPos)) {
            ...
            if (!voxelShape.isEmpty()) {
               float f = (float)(((double)darkness - (y - (double)pos.getY()) / 2.0D) * 0.5D * (double)world.getBrightness(pos));
               if (f >= 0.0F) {
                  if (f > 1.0F) {
                     f = 1.0F;
                  }
   ...
   private static void shadowVertex(MatrixStack.Entry matrix, VertexConsumer vertexConsumer, float alpha, float x, float y, float z, float u, float v) {
      vertexConsumer.vertex(matrix.getModel(), x, y, z).color(1.0F, 1.0F, 1.0F, alpha).texture(u, v).overlay(OverlayTexture.DEFAULT_UV).light(15728880).normal(matrix.getNormal(), 0.0F, 1.0F, 0.0F).next();
   }
