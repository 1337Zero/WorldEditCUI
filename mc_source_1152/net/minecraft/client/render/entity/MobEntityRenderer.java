...
public abstract class MobEntityRenderer<T extends MobEntity, M extends EntityModel<T>> extends LivingEntityRenderer<T, M> {
   ...
   protected boolean hasLabel(T mobEntity) {
      return super.hasLabel((LivingEntity)mobEntity) && (mobEntity.shouldRenderName() || mobEntity.hasCustomName() && mobEntity == this.renderManager.targetedEntity);
   }
   ...
   private <E extends Entity> void method_4073(T mobEntity, float f, MatrixStack matrixStack, VertexConsumerProvider vertexConsumerProvider, E entity) {
      ...
      g = Math.cos(n) * (double)mobEntity.getWidth() * 0.4D;
      h = Math.sin(n) * (double)mobEntity.getWidth() * 0.4D;
      ...
      matrixStack.translate(g, -(1.6D - (double)mobEntity.getHeight()) * 0.5D, h);
      float r = (float)(k - o);
      float s = (float)(l - p);
      float t = (float)(m - q);
      float u = 0.025F;
