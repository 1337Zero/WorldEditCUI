...
public class ShulkerEntityRenderer extends MobEntityRenderer<ShulkerEntity, ShulkerEntityModel<ShulkerEntity>> {
   ...
   public Vec3d getPositionOffset(ShulkerEntity shulkerEntity, float f) {
      ...
      if (i > 0 && shulkerEntity.method_7117()) {
         ...
         double d = (double)((float)i - f) / 6.0D;
         d *= d;
   ...
   public boolean shouldRender(ShulkerEntity shulkerEntity, Frustum frustum, double d, double e, double f) {
      if (super.shouldRender((MobEntity)shulkerEntity, frustum, d, e, f)) {
         return true;
      } else {
   ...
   static {
      SKIN = new Identifier("textures/" + TexturedRenderLayers.SHULKER_TEXTURE_ID.getTextureId().getPath() + ".png");
      SKIN_COLOR = (Identifier[])TexturedRenderLayers.COLORED_SHULKER_BOXES_TEXTURES.stream().map((spriteIdentifier) -> {
         return new Identifier("textures/" + spriteIdentifier.getTextureId().getPath() + ".png");
      }).toArray((i) -> {
