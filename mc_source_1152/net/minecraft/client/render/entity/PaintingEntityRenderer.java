...
public class PaintingEntityRenderer extends EntityRenderer<PaintingEntity> {
   ...
   private void method_4074(MatrixStack matrixStack, VertexConsumer vertexConsumer, PaintingEntity paintingEntity, int i, int j, Sprite sprite, Sprite sprite2) {
      ...
      float r = sprite2.getFrameV(1.0D);
      float s = sprite2.getMinU();
      float t = sprite2.getFrameU(1.0D);
   ...
   private void method_23188(Matrix4f matrix4f, Matrix3f matrix3f, VertexConsumer vertexConsumer, float f, float g, float h, float i, float j, int k, int l, int m, int n) {
      vertexConsumer.vertex(matrix4f, f, g, j).color(255, 255, 255, 255).texture(h, i).overlay(OverlayTexture.DEFAULT_UV).light(n).normal(matrix3f, (float)k, (float)l, (float)m).next();
   }
}
