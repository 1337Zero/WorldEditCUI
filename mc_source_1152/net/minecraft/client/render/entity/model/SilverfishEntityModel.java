...
public class SilverfishEntityModel<T extends Entity> extends CompositeEntityModel<T> {
   ...
   public SilverfishEntityModel() {
      ...
      for(int i = 0; i < this.field_3560.length; ++i) {
         ...
         this.field_3560[i].addCuboid((float)field_3558[i][0] * -0.5F, 0.0F, (float)field_3558[i][2] * -0.5F, (float)field_3558[i][0], (float)field_3558[i][1], (float)field_3558[i][2]);
   ...
   public void setAngles(T entity, float limbAngle, float limbDistance, float customAngle, float headYaw, float headPitch) {
      for(int i = 0; i < this.field_3560.length; ++i) {
         this.field_3560[i].yaw = MathHelper.cos(customAngle * 0.9F + (float)i * 0.15F * 3.1415927F) * 3.1415927F * 0.05F * (float)(1 + Math.abs(i - 2));
         this.field_3560[i].pivotX = MathHelper.sin(customAngle * 0.9F + (float)i * 0.15F * 3.1415927F) * 3.1415927F * 0.2F * (float)Math.abs(i - 2);
      }
