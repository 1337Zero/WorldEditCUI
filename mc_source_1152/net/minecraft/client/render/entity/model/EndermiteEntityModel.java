...
public class EndermiteEntityModel<T extends Entity> extends CompositeEntityModel<T> {
   ...
   public void setAngles(T entity, float limbAngle, float limbDistance, float customAngle, float headYaw, float headPitch) {
      for(int i = 0; i < this.field_3368.length; ++i) {
         this.field_3368[i].yaw = MathHelper.cos(customAngle * 0.9F + (float)i * 0.15F * 3.1415927F) * 3.1415927F * 0.01F * (float)(1 + Math.abs(i - 2));
         this.field_3368[i].pivotX = MathHelper.sin(customAngle * 0.9F + (float)i * 0.15F * 3.1415927F) * 3.1415927F * 0.1F * (float)Math.abs(i - 2);
      }
