...
public class IllagerEntityModel<T extends IllagerEntity> extends CompositeEntityModel<T> implements ModelWithArms, ModelWithHead {
   ...
   public void setAngles(T illagerEntity, float f, float g, float h, float i, float j) {
      ...
      if (state == IllagerEntity.State.ATTACKING) {
         m = MathHelper.sin(this.handSwingProgress * 3.1415927F);
         ...
         if (illagerEntity.getMainArm() == Arm.RIGHT) {
            ...
            var10000 = this.rightAttackingArm;
            var10000.pitch += m * 2.2F - l * 0.4F;
            var10000 = this.leftAttackingArm;
            var10000.pitch += m * 1.2F - l * 0.4F;
         } else {
         ...
         m = MathHelper.clamp(this.field_3424, 0.0F, 25.0F);
         this.leftAttackingArm.yaw = MathHelper.lerp(m / 25.0F, 0.4F, 0.85F);
         this.leftAttackingArm.pitch = MathHelper.lerp(m / 25.0F, this.leftAttackingArm.pitch, -1.5707964F);
