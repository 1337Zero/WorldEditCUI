...
public class DragonHeadEntityModel extends SkullEntityModel {
   ...
   public void render(float f, float g, float h) {
      this.jaw.pitch = (float)(Math.sin((double)(f * 3.1415927F * 0.2F)) + 1.0D) * 0.2F;
      this.head.yaw = g * 0.017453292F;
      this.head.pitch = h * 0.017453292F;
   }

   public void render(MatrixStack matrices, VertexConsumer vertexConsumer, int light, int overlay, float red, float green, float blue, float alpha) {
      matrices.push();
      matrices.translate(0.0D, -0.37437498569488525D, 0.0D);
