...
public class PhantomEntityModel<T extends Entity> extends CompositeEntityModel<T> {
   ...
   public void setAngles(T entity, float limbAngle, float limbDistance, float customAngle, float headYaw, float headPitch) {
      float f = ((float)(entity.getEntityId() * 3) + customAngle) * 0.13F;
      float g = 16.0F;
      this.leftWing.roll = MathHelper.cos(f) * 16.0F * 0.017453292F;
      this.leftWingTip.roll = MathHelper.cos(f) * 16.0F * 0.017453292F;
      this.rightWing.roll = -this.leftWing.roll;
      this.rightWingTip.roll = -this.leftWingTip.roll;
      this.tail.pitch = -(5.0F + MathHelper.cos(f * 2.0F) * 5.0F) * 0.017453292F;
      this.lowerTail.pitch = -(5.0F + MathHelper.cos(f * 2.0F) * 5.0F) * 0.017453292F;
   }
}
