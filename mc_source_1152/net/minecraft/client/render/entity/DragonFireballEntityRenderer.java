...
public class DragonFireballEntityRenderer extends EntityRenderer<DragonFireballEntity> {
   ...
   private static final RenderLayer field_21735;
   ...
   private static void method_23837(VertexConsumer vertexConsumer, Matrix4f matrix4f, Matrix3f matrix3f, int i, float f, int j, int k, int l) {
      vertexConsumer.vertex(matrix4f, f - 0.5F, (float)j - 0.25F, 0.0F).color(255, 255, 255, 255).texture((float)k, (float)l).overlay(OverlayTexture.DEFAULT_UV).light(i).normal(matrix3f, 0.0F, 1.0F, 0.0F).next();
   }
   ...
   static {
      field_21735 = RenderLayer.getEntityCutoutNoCull(SKIN);
   }
}
