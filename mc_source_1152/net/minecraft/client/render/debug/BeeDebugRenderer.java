...
public class BeeDebugRenderer implements DebugRenderer.Renderer {
   ...
   private void removeInvalidBees() {
      this.bees.entrySet().removeIf((entry) -> {
         return this.client.world.getEntityById(((BeeDebugRenderer.Bee)entry.getValue()).id) == null;
      });
   }
   ...
   private void drawFlowers() {
      ...
      this.bees.values().stream().filter(BeeDebugRenderer.Bee::hasFlower).forEach((bee) -> {
         Set<UUID> set = (Set)map.get(bee.flowerPos);
         if (set == null) {
            set = Sets.newHashSet();
            map.put(bee.flowerPos, set);
         }

         ((Set)set).add(bee.getUuid());
      });
   ...
   private void drawHiveInfo(BeeDebugRenderer.Hive hive, Collection<UUID> blacklistingBees) {
      ...
      drawString(hive.field_21544 + (hive.sedated ? " (sedated)" : ""), (BeeDebugRenderer.Hive)hive, i++, -1);
   }
   ...
   private void drawBee(BeeDebugRenderer.Bee bee) {
      ...
      if (bee.hivePos == null) {
         drawString(bee.pos, var6++, "No hive", -98404, 0.02F);
      } else {
         drawString(bee.pos, var6++, "Hive: " + this.getPositionString(bee, bee.hivePos), -256, 0.02F);
      }

      if (bee.flowerPos == null) {
         drawString(bee.pos, var6++, "No flower", -98404, 0.02F);
      } else {
         drawString(bee.pos, var6++, "Flower: " + this.getPositionString(bee, bee.flowerPos), -256, 0.02F);
      }

      Iterator var4 = bee.field_21542.iterator();

      while(var4.hasNext()) {
         String string = (String)var4.next();
         drawString(bee.pos, var6++, string, -16711936, 0.02F);
      }
      ...
      if (bee.travellingTicks > 0) {
         ...
         drawString(bee.pos, var6++, "Travelling: " + bee.travellingTicks + " ticks", j, 0.02F);
      }
   ...
   private Map<BlockPos, List<String>> getBeesByHive() {
      ...
      while(var2.hasNext()) {
         ...
         if (bee.hivePos != null && !this.hives.containsKey(bee.hivePos)) {
            List<String> list = (List)map.get(bee.hivePos);
            if (list == null) {
               list = Lists.newArrayList();
               map.put(bee.hivePos, list);
            }

            ((List)list).add(bee.getName());
         }
      }
