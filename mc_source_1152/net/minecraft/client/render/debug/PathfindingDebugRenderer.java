   ...
   private static void drawPathInternal(Path path, float nodeSize, boolean bl, boolean drawLabels, double cameraX, double cameraY, double cameraZ) {
      ...
      if (drawLabels) {
         for(j = 0; j < path.getLength(); ++j) {
            pathNode4 = path.getNode(j);
            if (getManhattanDistance(pathNode4.getPos(), cameraX, cameraY, cameraZ) <= 80.0F) {
               DebugRenderer.drawString(String.format("%s", pathNode4.type), (double)pathNode4.x + 0.5D, (double)pathNode4.y + 0.75D, (double)pathNode4.z + 0.5D, -1);
               DebugRenderer.drawString(String.format(Locale.ROOT, "%.2f", pathNode4.penalty), (double)pathNode4.x + 0.5D, (double)pathNode4.y + 0.25D, (double)pathNode4.z + 0.5D, -1);
            }
         }
      }
