...
public class SpectatorMenu {
   ...
   public SpectatorMenuCommand getCommand(int slot) {
      ...
      if (this.page > 0 && slot == 0) {
         ...
         return i >= 0 && i < this.currentGroup.getCommands().size() ? (SpectatorMenuCommand)MoreObjects.firstNonNull(this.currentGroup.getCommands().get(i), BLANK_COMMAND) : BLANK_COMMAND;
      }
   }
   ...
   static class ChangePageSpectatorMenuCommand implements SpectatorMenuCommand {
      ...
      public Text getName() {
         return this.direction < 0 ? new TranslatableText("spectatorMenu.previous_page", new Object[0]) : new TranslatableText("spectatorMenu.next_page", new Object[0]);
      }
