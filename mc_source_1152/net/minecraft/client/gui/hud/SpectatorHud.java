...
public class SpectatorHud extends DrawableHelper implements SpectatorMenuCloseCallback {
   ...
   private void renderSpectatorCommand(int slot, int x, float y, float alpha, SpectatorMenuCommand command) {
      ...
      if (command != SpectatorMenu.BLANK_COMMAND) {
         ...
         if (i > 3 && command.isEnabled()) {
            this.client.textRenderer.drawWithShadow(string, (float)(x + 19 - 2 - this.client.textRenderer.getStringWidth(string)), y + 6.0F + 3.0F, 16777215 + (i << 24));
         }
      }
   ...
   public void render() {
      ...
      if (i > 3 && this.spectatorMenu != null) {
         ...
         String string = spectatorMenuCommand == SpectatorMenu.BLANK_COMMAND ? this.spectatorMenu.getCurrentGroup().getPrompt().asFormattedString() : spectatorMenuCommand.getName().asFormattedString();
         if (string != null) {
   ...
   public void cycleSlot(double offset) {
      int i;
      for(i = this.spectatorMenu.getSelectedSlot() + (int)offset; i >= 0 && i <= 8 && (this.spectatorMenu.getCommand(i) == SpectatorMenu.BLANK_COMMAND || !this.spectatorMenu.getCommand(i).isEnabled()); i = (int)((double)i + offset)) {
      }
