...
public abstract class ListWidget extends AbstractParentElement implements Drawable {
   ...
   protected void renderHoleBackground(int i, int j, int k, int l) {
      ...
      float f = 32.0F;
      bufferBuilder.begin(7, VertexFormats.POSITION_TEXTURE_COLOR);
      bufferBuilder.vertex((double)this.left, (double)j, 0.0D).texture(0.0F, (float)j / 32.0F).color(64, 64, 64, l).next();
      bufferBuilder.vertex((double)(this.left + this.width), (double)j, 0.0D).texture((float)this.width / 32.0F, (float)j / 32.0F).color(64, 64, 64, l).next();
      bufferBuilder.vertex((double)(this.left + this.width), (double)i, 0.0D).texture((float)this.width / 32.0F, (float)i / 32.0F).color(64, 64, 64, k).next();
      bufferBuilder.vertex((double)this.left, (double)i, 0.0D).texture(0.0F, (float)i / 32.0F).color(64, 64, 64, k).next();
      tessellator.draw();
   }
