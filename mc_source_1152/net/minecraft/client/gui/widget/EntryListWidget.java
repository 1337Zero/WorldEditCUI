...
public abstract class EntryListWidget<E extends EntryListWidget.Entry<E>> extends AbstractParentElement implements Drawable {
   ...
   protected final E getEntryAtPosition(double x, double y) {
      ...
      return x < (double)this.getScrollbarPosition() && x >= (double)k && x <= (double)l && n >= 0 && m >= 0 && n < this.getItemCount() ? (EntryListWidget.Entry)this.children().get(n) : null;
   }
   ...
   public void render(int mouseX, int mouseY, float delta) {
      ...
      if (n > 0) {
         ...
         tessellator.draw();
         bufferBuilder.begin(7, VertexFormats.POSITION_TEXTURE_COLOR);
         bufferBuilder.vertex((double)i, (double)(p + o), 0.0D).texture(0.0F, 1.0F).color(128, 128, 128, 255).next();
         bufferBuilder.vertex((double)j, (double)(p + o), 0.0D).texture(1.0F, 1.0F).color(128, 128, 128, 255).next();
         ...
         tessellator.draw();
         bufferBuilder.begin(7, VertexFormats.POSITION_TEXTURE_COLOR);
         bufferBuilder.vertex((double)i, (double)(p + o - 1), 0.0D).texture(0.0F, 1.0F).color(192, 192, 192, 255).next();
         bufferBuilder.vertex((double)(j - 1), (double)(p + o - 1), 0.0D).texture(1.0F, 1.0F).color(192, 192, 192, 255).next();
         bufferBuilder.vertex((double)(j - 1), (double)p, 0.0D).texture(1.0F, 0.0F).color(192, 192, 192, 255).next();
   ...
   protected void moveSelection(int amount) {
      if (!this.children().isEmpty()) {
         ...
         E entry = (EntryListWidget.Entry)this.children().get(j);
   ...
   protected void renderList(int x, int y, int mouseX, int mouseY, float delta) {
      ...
      for(int j = 0; j < i; ++j) {
         ...
         if (l >= this.top && k <= this.bottom) {
            ...
            r = this.getRowLeft();
            entry.render(j, k, r, o, n, mouseX, mouseY, this.isMouseOver((double)mouseX, (double)mouseY) && Objects.equals(this.getEntryAtPosition((double)mouseX, (double)mouseY), entry), delta);
         }
      }
   ...
   protected void renderHoleBackground(int top, int bottom, int alphaTop, int alphaBottom) {
      ...
      float f = 32.0F;
      bufferBuilder.begin(7, VertexFormats.POSITION_TEXTURE_COLOR);
      bufferBuilder.vertex((double)this.left, (double)bottom, 0.0D).texture(0.0F, (float)bottom / 32.0F).color(64, 64, 64, alphaBottom).next();
      bufferBuilder.vertex((double)(this.left + this.width), (double)bottom, 0.0D).texture((float)this.width / 32.0F, (float)bottom / 32.0F).color(64, 64, 64, alphaBottom).next();
      bufferBuilder.vertex((double)(this.left + this.width), (double)top, 0.0D).texture((float)this.width / 32.0F, (float)top / 32.0F).color(64, 64, 64, alphaTop).next();
      bufferBuilder.vertex((double)this.left, (double)top, 0.0D).texture(0.0F, (float)top / 32.0F).color(64, 64, 64, alphaTop).next();
      tessellator.draw();
   }
