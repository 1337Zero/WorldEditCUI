...
public class BookEditScreen extends Screen {
   ...
   protected void init() {
      ...
      this.signButton = (ButtonWidget)this.addButton(new ButtonWidget(this.width / 2 - 100, 196, 98, 20, I18n.translate("book.signButton"), (buttonWidget) -> {
         this.signing = true;
         this.updateButtons();
      }));
      this.doneButton = (ButtonWidget)this.addButton(new ButtonWidget(this.width / 2 + 2, 196, 98, 20, I18n.translate("gui.done"), (buttonWidget) -> {
         this.minecraft.openScreen((Screen)null);
         this.finalizeBook(false);
      }));
      this.finalizeButton = (ButtonWidget)this.addButton(new ButtonWidget(this.width / 2 - 100, 196, 98, 20, I18n.translate("book.finalizeButton"), (buttonWidget) -> {
         if (this.signing) {
      ...
      this.cancelButton = (ButtonWidget)this.addButton(new ButtonWidget(this.width / 2 + 2, 196, 98, 20, I18n.translate("gui.cancel"), (buttonWidget) -> {
         if (this.signing) {
            this.signing = false;
         }
   ...
   private void applyUpArrowKey(String content) {
      if (!content.isEmpty()) {
         ...
         if (position.y == 0) {
            ...
            int var10005 = position.x + this.getCharWidthInString(content, this.cursorIndex) / 3;
            int var10006 = position.y;
            this.font.getClass();
            int i = this.getCharacterCountInFrontOfCursor(content, new BookEditScreen.Position(var10005, var10006 - 9));
            if (i >= 0) {
               this.cursorIndex = i;
               if (!Screen.hasShiftDown()) {
                  this.highlightTo = this.cursorIndex;
   ...
   private void applyDownArrowKey(String content) {
      if (!content.isEmpty()) {
         ...
         if (var10000 + 9 == i) {
            ...
            int var10006 = position.y;
            this.font.getClass();
            int j = this.getCharacterCountInFrontOfCursor(content, new BookEditScreen.Position(var10005, var10006 + 9));
            if (j >= 0) {
               this.cursorIndex = j;
               if (!Screen.hasShiftDown()) {
                  this.highlightTo = this.cursorIndex;
   ...
   private void moveCursorToBottom(String content) {
      this.cursorIndex = this.getCharacterCountInFrontOfCursor(content, new BookEditScreen.Position(113, this.getCursorPositionForIndex(content, this.cursorIndex).y));
      if (!Screen.hasShiftDown()) {
         this.highlightTo = this.cursorIndex;
      }
   ...
   public void render(int mouseX, int mouseY, float delta) {
      ...
      if (this.signing) {
         ...
         string6 = I18n.translate("book.editTitle");
         k = this.getStringWidth(string6);
         this.font.draw(string6, (float)(i + 36 + (114 - k) / 2), 34.0F, 0);
         ...
         string = I18n.translate("book.pageIndicator", this.currentPage + 1, this.countPages());
         string6 = this.getCurrentPageContent();
         k = this.getStringWidth(string);
         this.font.draw(string, (float)(i - k + 192 - 44), 18.0F, 0);
         this.font.drawTrimmed(string6, i + 36, 32, 114, 0);
         ...
         if (this.tickCounter / 6 % 2 == 0) {
            ...
            if (this.cursorIndex < string6.length()) {
               int var10000 = position.x;
               int var10001 = position.y - 1;
               int var10002 = position.x + 1;
               int var10003 = position.y;
               this.font.getClass();
               DrawableHelper.fill(var10000, var10001, var10002, var10003 + 9, -16777216);
            } else {
   ...
   private void drawHighlight(String content) {
      if (this.highlightTo != this.cursorIndex) {
         ...
         int var10003 = position.x;
         int var10004 = position.y;
         this.font.getClass();
         BookEditScreen.Position position2 = new BookEditScreen.Position(var10003, var10004 + 9);
         ...
         while(!string.isEmpty()) {
            ...
            this.drawHighlightRect(position, position2);
            position.x = 0;
            int var10001 = position.y;
            this.font.getClass();
            position.y = var10001 + 9;
            var10001 = position2.y;
            this.font.getClass();
            position2.y = var10001 + 9;
         }
   ...
   private void drawHighlightRect(BookEditScreen.Position position1, BookEditScreen.Position position2) {
      ...
      BookEditScreen.Position position2 = new BookEditScreen.Position(position2.x, position2.y);
      if (this.font.isRightToLeft()) {
         this.localizePosition(position);
         this.localizePosition(position2);
         int i = position2.x;
      ...
      this.translateRelativePositionToGlPosition(position);
      this.translateRelativePositionToGlPosition(position2);
      ...
      bufferBuilder.vertex((double)position.x, (double)position2.y, 0.0D).next();
      bufferBuilder.vertex((double)position2.x, (double)position2.y, 0.0D).next();
      bufferBuilder.vertex((double)position2.x, (double)position.y, 0.0D).next();
      bufferBuilder.vertex((double)position.x, (double)position.y, 0.0D).next();
      tessellator.draw();
   ...
   private BookEditScreen.Position getCursorPositionForIndex(String content, int index) {
      ...
      for(String string = content; !string.isEmpty(); j = i) {
         ...
         String string3;
         if (string.length() <= k) {
            string3 = string.substring(0, Math.min(Math.max(index - j, 0), string.length()));
            position.x = position.x + this.getStringWidth(string3);
            break;
         }

         string3 = string.substring(0, k);
         char c = string.charAt(k);
         boolean bl = c == ' ' || c == '\n';
         string = Formatting.getFormatAtEnd(string3) + string.substring(k + (bl ? 1 : 0));
         i += string3.length() + (bl ? 1 : 0);
         if (i - 1 >= index) {
            String string4 = string3.substring(0, Math.min(Math.max(index - j, 0), string3.length()));
