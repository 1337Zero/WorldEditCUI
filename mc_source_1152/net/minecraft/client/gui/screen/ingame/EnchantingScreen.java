...
public class EnchantingScreen extends AbstractContainerScreen<EnchantingTableContainer> {
   ...
   public boolean mouseClicked(double mouseX, double mouseY, int button) {
      ...
      for(int k = 0; k < 3; ++k) {
         ...
         if (d >= 0.0D && e >= 0.0D && d < 108.0D && e < 19.0D && ((EnchantingTableContainer)this.container).onButtonClick(this.minecraft.player, k)) {
   ...
   protected void drawBackground(float delta, int mouseX, int mouseY) {
      ...
      RenderSystem.color4f(1.0F, 1.0F, 1.0F, 1.0F);
      EnchantingPhrases.getInstance().setSeed((long)((EnchantingTableContainer)this.container).getSeed());
         ...
         if (this.isPointWithinBounds(60, 14 + 19 * j, 108, 17, (double)mouseX, (double)mouseY) && k > 0 && l >= 0 && enchantment != null) {
            ...
            if (!bl) {
               ...
               if (this.minecraft.player.experienceLevel < k) {
                  list.add(Formatting.RED + I18n.translate("container.enchant.level.requirement", ((EnchantingTableContainer)this.container).enchantmentPower[j]));
               } else {
                  String string2;
                  if (m == 1) {
                  ...
                  list.add(Formatting.GRAY + "" + string2);
               }
            }
   ...
   public void doTick() {
      ...
      for(int i = 0; i < 3; ++i) {
         if (((EnchantingTableContainer)this.container).enchantmentPower[i] != 0) {
            bl = true;
         }
      }
