...
public abstract class AbstractContainerScreen<T extends Container> extends Screen implements ContainerProvider<T> {
   ...
   public AbstractContainerScreen(T container, PlayerInventory playerInventory, Text name) {
      super(name);
   ...
   public void render(int mouseX, int mouseY, float delta) {
      ...
      int r;
      for(int m = 0; m < this.container.slotList.size(); ++m) {
         Slot slot = (Slot)this.container.slotList.get(m);
         ...
         if (this.isPointOverSlot(slot, (double)mouseX, (double)mouseY) && slot.doDrawHoveringEffect()) {
            ...
            this.fillGradient(n, r, n + 16, r + 16, -2130706433, -2130706433);
      ...
      if (!this.touchDropReturningStack.isEmpty()) {
         ...
         r = this.touchDropOriginSlot.xPosition - this.touchDropX;
         int s = this.touchDropOriginSlot.yPosition - this.touchDropY;
   ...
   public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
      ...
      } else if (this.isCursorDragging && slot != null && !itemStack.isEmpty() && (itemStack.getCount() > this.cursorDragSlots.size() || this.heldButtonType == 2) && Container.canInsertItemIntoSlot(slot, itemStack, true) && slot.canInsert(itemStack) && this.container.canInsertIntoSlot(slot)) {
      ...
      if (this.isDoubleClicking && slot != null && button == 0 && this.container.canInsertIntoSlot(ItemStack.EMPTY, slot)) {
         if (hasShiftDown()) {
            if (!this.quickMovingStack.isEmpty()) {
               var13 = this.container.slotList.iterator();

               while(var13.hasNext()) {
                  slot3 = (Slot)var13.next();
                  if (slot3 != null && slot3.canTakeItems(this.minecraft.player) && slot3.hasStack() && slot3.inventory == slot.inventory && Container.canInsertItemIntoSlot(slot3, this.quickMovingStack, true)) {
         ...
         if (this.touchDragSlotStart != null && this.minecraft.options.touchscreen) {
            if (button == 0 || button == 1) {
               ...
               bl3 = Container.canInsertItemIntoSlot(slot, this.touchDragStack, false);
            ...
            this.onMouseClick((Slot)null, -999, Container.packClickData(0, this.heldButtonType), SlotActionType.QUICK_CRAFT);
            var13 = this.cursorDragSlots.iterator();

            while(var13.hasNext()) {
               slot3 = (Slot)var13.next();
               this.onMouseClick(slot3, slot3.id, Container.packClickData(1, this.heldButtonType), SlotActionType.QUICK_CRAFT);
            }
