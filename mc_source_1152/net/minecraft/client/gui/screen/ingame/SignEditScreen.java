...
public class SignEditScreen extends Screen {
   ...
   public void render(int mouseX, int mouseY, float delta) {
      ...
      SignBlockEntityRenderer.SignModel var10002 = this.field_21525;
      var10002.getClass();
      ...
      for(s = 0; s < strings.length; ++s) {
         ...
         if (string2 != null) {
            ...
            this.minecraft.textRenderer.draw(string2, p, (float)(s * 10 - this.sign.text.length * 5), i, false, matrix4f, immediate, false, 0, 15728880);
            if (s == this.currentRow && k >= 0 && bl2) {
               u = this.minecraft.textRenderer.getStringWidth(string2.substring(0, Math.max(Math.min(k, string2.length()), 0)));
               v = (u - this.minecraft.textRenderer.getStringWidth(string2) / 2) * m;
               if (k >= string2.length()) {
                  this.minecraft.textRenderer.draw("_", (float)v, (float)n, i, false, matrix4f, immediate, false, 0, 15728880);
               }
            }
         }
      }
      ...
      for(s = 0; s < strings.length; ++s) {
         ...
         if (string2 != null && s == this.currentRow && k >= 0) {
            int t = this.minecraft.textRenderer.getStringWidth(string2.substring(0, Math.max(Math.min(k, string2.length()), 0)));
            ...
            if (l != k) {
               v = Math.min(k, l);
               int w = Math.max(k, l);
               int x = (this.minecraft.textRenderer.getStringWidth(string2.substring(0, v)) - this.minecraft.textRenderer.getStringWidth(string2) / 2) * m;
               int y = (this.minecraft.textRenderer.getStringWidth(string2.substring(0, w)) - this.minecraft.textRenderer.getStringWidth(string2) / 2) * m;
               int z = Math.min(x, y);
               int aa = Math.max(x, y);
