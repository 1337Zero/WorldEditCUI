...
public class CommandSuggestor {
   ...
   private void showUsages(Formatting formatting) {
      ...
      while(var7.hasNext()) {
         Entry<CommandNode<CommandSource>, String> entry = (Entry)var7.next();
   ...
   public static String highlight(ParseResults<CommandSource> parse, String original, int firstCharacterIndex) {
      ...
      Iterator var9 = commandContextBuilder.getArguments().values().iterator();

      while(var9.hasNext()) {
         ParsedArgument<CommandSource, ?> parsedArgument = (ParsedArgument)var9.next();
         ++j;
   ...
   public class SuggestionWindow {
      ...
      public boolean mouseScrolled(double amount) {
         ...
         int j = (int)(CommandSuggestor.this.client.mouse.getY() * (double)CommandSuggestor.this.client.getWindow().getScaledHeight() / (double)CommandSuggestor.this.client.getWindow().getHeight());
