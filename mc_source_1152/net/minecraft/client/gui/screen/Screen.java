...
public abstract class Screen extends AbstractParentElement implements Drawable {
         ...
         if (hoverEvent.getAction() == HoverEvent.Action.SHOW_ITEM) {
            ...
            if (this.minecraft.options.advancedItemTooltips) {
               try {
                  ...
                  this.renderTooltip((List)list, x, y);
   ...
   public void renderDirtBackground(int alpha) {
      ...
      bufferBuilder.vertex(0.0D, (double)this.height, 0.0D).texture(0.0F, (float)this.height / 32.0F + (float)alpha).color(64, 64, 64, 255).next();
      bufferBuilder.vertex((double)this.width, (double)this.height, 0.0D).texture((float)this.width / 32.0F, (float)this.height / 32.0F + (float)alpha).color(64, 64, 64, 255).next();
      bufferBuilder.vertex((double)this.width, 0.0D, 0.0D).texture((float)this.width / 32.0F, (float)alpha).color(64, 64, 64, 255).next();
      bufferBuilder.vertex(0.0D, 0.0D, 0.0D).texture(0.0F, (float)alpha).color(64, 64, 64, 255).next();
      tessellator.draw();
   }
