...
public class RecipeBookWidget extends DrawableHelper implements Drawable, Element, RecipeDisplayListener, RecipeGridAligner<Ingredient> {
   ...
   public void reset(boolean isNarrow) {
      ...
      this.searchField = new TextFieldWidget(var10003, var10004, var10005, 80, 9 + 5, I18n.translate("itemGroup.search"));
      ...
      Iterator var5 = ClientRecipeBook.getGroupsForContainer(this.craftingContainer).iterator();

      while(var5.hasNext()) {
         RecipeBookGroup recipeBookGroup = (RecipeBookGroup)var5.next();
         this.tabButtons.add(new RecipeGroupButtonWidget(recipeBookGroup));
      }

      if (this.currentTab != null) {
         this.currentTab = (RecipeGroupButtonWidget)this.tabButtons.stream().filter((recipeGroupButtonWidget) -> {
   ...
   public int findLeftEdge(boolean narrow, int width, int containerWidth) {
      ...
      if (this.isOpen() && !narrow) {
         j = 177 + (width - containerWidth - 200) / 2;
      } else {
         j = (width - containerWidth) / 2;
      }
   ...
   private void refreshResults(boolean resetCurrentPage) {
      ...
      list.forEach((recipeResultCollection) -> {
         recipeResultCollection.computeCraftables(this.recipeFinder, this.craftingContainer.getCraftingWidth(), this.craftingContainer.getCraftingHeight(), this.recipeBook);
      });
      List<RecipeResultCollection> list2 = Lists.newArrayList(list);
      list2.removeIf((recipeResultCollection) -> {
         return !recipeResultCollection.isInitialized();
      });
      list2.removeIf((recipeResultCollection) -> {
         return !recipeResultCollection.hasFittingRecipes();
      });
      String string = this.searchField.getText();
      if (!string.isEmpty()) {
         ObjectSet<RecipeResultCollection> objectSet = new ObjectLinkedOpenHashSet(this.client.getSearchableContainer(SearchManager.RECIPE_OUTPUT).findAll(string.toLowerCase(Locale.ROOT)));
         list2.removeIf((recipeResultCollection) -> {
            return !objectSet.contains(recipeResultCollection);
         });
      }

      if (this.recipeBook.isFilteringCraftable(this.craftingContainer)) {
         list2.removeIf((recipeResultCollection) -> {
            return !recipeResultCollection.hasCraftableRecipes();
         });
      }
   ...
   private void refreshTabButtons() {
      ...
      while(true) {
         while(var5.hasNext()) {
            RecipeGroupButtonWidget recipeGroupButtonWidget = (RecipeGroupButtonWidget)var5.next();
      ...
      if (this.isOpen() && !this.client.player.isSpectator()) {
         if (this.recipesArea.mouseClicked(mouseX, mouseY, button, (this.parentWidth - 147) / 2 - this.leftOffset, (this.parentHeight - 166) / 2, 147, 166)) {
            ...
            do {
               ...
               recipeGroupButtonWidget = (RecipeGroupButtonWidget)var6.next();
