...
public abstract class ResourcePackListWidget extends AlwaysSelectedEntryListWidget<ResourcePackListWidget.ResourcePackEntry> {
   ...
   protected void renderHeader(int i, int j, Tessellator tessellator) {
      ...
      this.client.textRenderer.draw(text.asFormattedString(), (float)(i + this.width / 2 - this.client.textRenderer.getStringWidth(text.asFormattedString()) / 2), (float)Math.min(this.top + 3, j), 16777215);
   }
   ...
   public static class ResourcePackEntry extends AlwaysSelectedEntryListWidget.Entry<ResourcePackListWidget.ResourcePackEntry> {
      ...
      public void render(int i, int j, int k, int l, int m, int n, int o, boolean bl, float f) {
         ...
         p = this.client.textRenderer.getStringWidth(string);
         if (p > 157) {
      ...
      public boolean mouseClicked(double mouseX, double mouseY, int button) {
         ...
         if (this.isMoveable() && d <= 32.0D) {
            ...
            List list2;
            int j;
            if (d > 16.0D && e < 16.0D && this.canMoveUp()) {
               list2 = this.resourcePackList.children();
               j = list2.indexOf(this);
               list2.remove(j);
               list2.add(j - 1, this);
               this.getScreen().markDirty();
               return true;
            ...
            if (d > 16.0D && e > 16.0D && this.canMoveDown()) {
               list2 = this.resourcePackList.children();
               j = list2.indexOf(this);
               list2.remove(j);
               list2.add(j + 1, this);
