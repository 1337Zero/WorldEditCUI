...
public class Keyboard {
   ...
   private void debugWarn(String string, Object... objects) {
      this.client.inGameHud.getChatHud().addMessage((new LiteralText("")).append((new TranslatableText("debug.prefix", new Object[0])).formatted(new Formatting[]{Formatting.YELLOW, Formatting.BOLD})).append(" ").append((Text)(new TranslatableText(string, objects))));
   }

   private void debugError(String string, Object... objects) {
      this.client.inGameHud.getChatHud().addMessage((new LiteralText("")).append((new TranslatableText("debug.prefix", new Object[0])).formatted(new Formatting[]{Formatting.RED, Formatting.BOLD})).append(" ").append((Text)(new TranslatableText(string, objects))));
   }
   ...
   private boolean processF3(int key) {
      if (this.debugCrashStartTime > 0L && this.debugCrashStartTime < Util.getMeasuringTimeMs() - 100L) {
         ...
         case 67:
            ...
            this.setClipboard(String.format(Locale.ROOT, "/execute in %s run tp @s %.2f %.2f %.2f %.2f %.2f", DimensionType.getId(this.client.player.world.dimension.getType()), this.client.player.getX(), this.client.player.getY(), this.client.player.getZ(), this.client.player.yaw, this.client.player.pitch));
            return true;
         case 68:
      ...
      if (hitResult != null) {
         ...
         case BLOCK:
            ...
            if (bl) {
               ...
               this.copyBlock(blockState, blockPos, (CompoundTag)null);
               this.debugWarn("debug.inspect.client.block");
            }
            break;
         case ENTITY:
            ...
            if (bl) {
               ...
               this.copyEntity(identifier, entity.getPos(), (CompoundTag)null);
               this.debugWarn("debug.inspect.client.entity");
            }
         }
   ...
   private void copyEntity(Identifier identifier, Vec3d vec3d, @Nullable CompoundTag compoundTag) {
      ...
      if (compoundTag != null) {
         ...
         string3 = String.format(Locale.ROOT, "/summon %s %.2f %.2f %.2f %s", identifier.toString(), vec3d.x, vec3d.y, vec3d.z, string);
      } else {
         string3 = String.format(Locale.ROOT, "/summon %s %.2f %.2f %.2f", identifier.toString(), vec3d.x, vec3d.y, vec3d.z);
      }
   ...
   public void setup(long l) {
      InputUtil.setKeyboardCallbacks(l, (lx, i, j, k, m) -> {
         this.client.execute(() -> {
            this.onKey(lx, i, j, k, m);
         });
      }, (lx, i, j) -> {
         this.client.execute(() -> {
            this.onChar(lx, i, j);
