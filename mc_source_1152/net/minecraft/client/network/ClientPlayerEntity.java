...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
   ...
   private void sendMovementPackets() {
      ...
      if (this.isCamera()) {
         ...
         ++this.field_3923;
         boolean bl3 = d * d + e * e + f * f > 9.0E-4D || this.field_3923 >= 20;
   ...
   protected void pushOutOfBlocks(double x, double y, double z) {
      ...
      if (this.cannotFitAt(blockPos)) {
         ...
         if (!this.cannotFitAt(blockPos.east()) && 1.0D - d < f) {
            f = 1.0D - d;
         ...
         if (!this.cannotFitAt(blockPos.south()) && 1.0D - e < f) {
            f = 1.0D - e;
         ...
         if (direction != null) {
            ...
            switch(direction) {
            case WEST:
               this.setVelocity(-0.1D, vec3d.y, vec3d.z);
               break;
            case EAST:
               this.setVelocity(0.1D, vec3d.y, vec3d.z);
               break;
            case NORTH:
               this.setVelocity(vec3d.x, vec3d.y, -0.1D);
               break;
            case SOUTH:
               this.setVelocity(vec3d.x, vec3d.y, 0.1D);
            }
   ...
   public void tickMovement() {
      ...
      if (this.isInFluid(FluidTags.WATER)) {
         ...
         this.field_3917 = MathHelper.clamp(this.field_3917 + j, 0, 600);
   ...
   protected void method_3148(float f, float g) {
      if (this.method_22119()) {
         ...
         Vec3d vec3d2 = vec3d.add((double)f, 0.0D, (double)g);
         Vec3d vec3d3 = new Vec3d((double)f, 0.0D, (double)g);
         ...
         if (i <= 0.001F) {
            ...
            float j = h * vec2f.x;
            float k = h * vec2f.y;
            o = MathHelper.sin(this.yaw * 0.017453292F);
            float m = MathHelper.cos(this.yaw * 0.017453292F);
            vec3d3 = new Vec3d((double)(j * m - k * o), vec3d3.y, (double)(k * m + j * o));
         ...
         o = (float)(vec3d5.x * vec3d4.x + vec3d5.z * vec3d4.z);
         if (o >= -0.15F) {
            ...
            if (blockState.getCollisionShape(this.world, blockPos, entityContext).isEmpty()) {
               ...
               if (blockState2.getCollisionShape(this.world, blockPos, entityContext).isEmpty()) {
                  ...
                  Box box = (new Box(vec3d, vec3d7.add(0.0D, (double)t, 0.0D))).expand((double)s, 0.0D, (double)s);
                  ...
                  Iterator<Box> iterator = this.world.getCollisions(this, box, Collections.emptySet()).flatMap((voxelShapex) -> {
   ...
   private boolean method_22119() {
      return this.getLastAutoJump() && this.field_3934 <= 0 && this.onGround && !this.clipAtLedge() && !this.hasVehicle() && this.hasMovementInput() && (double)this.getJumpVelocityMultiplier() >= 1.0D;
   }
