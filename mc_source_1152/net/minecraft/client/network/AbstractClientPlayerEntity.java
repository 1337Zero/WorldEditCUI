...
public abstract class AbstractClientPlayerEntity extends PlayerEntity {
   ...
   public static PlayerSkinTexture loadSkin(Identifier id, String playerName) {
      ...
      if (abstractTexture == null) {
         abstractTexture = new PlayerSkinTexture((File)null, String.format("http://skins.minecraft.net/MinecraftSkins/%s.png", ChatUtil.stripTextFormat(playerName)), DefaultSkinHelper.getTexture(getOfflinePlayerUuid(playerName)), true, (Runnable)null);
   ...
   public float getSpeed() {
      ...
      f = (float)((double)f * ((entityAttributeInstance.getValue() / (double)this.abilities.getWalkSpeed() + 1.0D) / 2.0D));
