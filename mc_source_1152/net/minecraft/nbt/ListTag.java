...
public class ListTag extends AbstractListTag<Tag> {
   ...
   private static final ByteSet field_21461 = new ByteOpenHashSet(Arrays.asList(1, 2, 3, 4, 5, 6));
      ...
      if (this.isEmpty()) {
         ...
         if (field_21461.contains(this.type) && this.size() <= 8) {
            ...
            for(j = 0; j < this.value.size(); ++j) {
               ...
               text3.append(((Tag)this.value.get(j)).toText(indent, depth + 1));
               if (j != this.value.size() - 1) {
                  text3.append(string2).append(indent.isEmpty() ? " " : "\n");
               }

               text2.append((Text)text3);
            }
