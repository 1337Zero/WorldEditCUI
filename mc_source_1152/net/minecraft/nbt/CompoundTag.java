...
public class CompoundTag implements Tag {
   ...
   public Text toText(String indent, int depth) {
      if (this.tags.isEmpty()) {
         ...
         Text text2;
         for(Iterator iterator = ((Collection)collection).iterator(); iterator.hasNext(); text.append(text2)) {
            String string = (String)iterator.next();
            text2 = (new LiteralText(Strings.repeat(indent, depth + 1))).append(prettyPrintTagKey(string)).append(String.valueOf(':')).append(" ").append(((Tag)this.tags.get(string)).toText(indent, depth + 1));
            if (iterator.hasNext()) {
