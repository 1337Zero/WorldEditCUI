...
public class ServerAdvancementLoader extends JsonDataLoader {
   ...
   }).registerTypeAdapter(AdvancementRewards.class, new AdvancementRewards.Deserializer()).registerTypeHierarchyAdapter(Text.class, new Text.Serializer()).registerTypeHierarchyAdapter(Style.class, new Style.Serializer()).registerTypeAdapterFactory(new LowercaseEnumTypeAdapterFactory()).create();
   ...
   protected void apply(Map<Identifier, JsonObject> map, ResourceManager resourceManager, Profiler profiler) {
      ...
      map.forEach((identifier, jsonObject) -> {
         try {
            ...
            LOGGER.error("Parsing error loading custom advancement {}: {}", identifier, var4.getMessage());
         }
