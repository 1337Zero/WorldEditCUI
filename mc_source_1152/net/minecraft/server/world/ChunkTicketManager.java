...
public abstract class ChunkTicketManager {
   ...
   private static int getLevel(SortedArraySet<ChunkTicket<?>> sortedArraySet) {
      return !sortedArraySet.isEmpty() ? ((ChunkTicket)sortedArraySet.first()).getLevel() : ThreadedAnvilChunkStorage.MAX_LEVEL + 1;
   }
   ...
   private void addTicket(long position, ChunkTicket<?> chunkTicket) {
      ...
      ChunkTicket<?> chunkTicket2 = (ChunkTicket)sortedArraySet.addAndGet(chunkTicket);
   ...
   private SortedArraySet<ChunkTicket<?>> getTicketSet(long position) {
      return (SortedArraySet)this.ticketsByPosition.computeIfAbsent(position, (l) -> {
   ...
   public void handleChunkEnter(ChunkSectionPos pos, ServerPlayerEntity serverPlayerEntity) {
      ...
      ((ObjectSet)this.playersByChunkPos.computeIfAbsent(l, (lx) -> {
   ...
   public void handleChunkLeave(ChunkSectionPos pos, ServerPlayerEntity player) {
      long l = pos.toChunkPos().toLong();
      ObjectSet<ServerPlayerEntity> objectSet = (ObjectSet)this.playersByChunkPos.get(l);
   ...
   protected String method_21623(long l) {
      ...
      if (sortedArraySet != null && !sortedArraySet.isEmpty()) {
         string2 = ((ChunkTicket)sortedArraySet.first()).toString();
      } else {
         string2 = "no_ticket";
      }
   ...
   class TicketDistanceLevelPropagator extends ChunkPosDistanceLevelPropagator {
      ...
      protected int getInitialLevel(long id) {
         ...
         if (sortedArraySet == null) {
            ...
            return sortedArraySet.isEmpty() ? Integer.MAX_VALUE : ((ChunkTicket)sortedArraySet.first()).getLevel();
         }
      }
   ...
   class DistanceFromNearestPlayerTracker extends ChunkPosDistanceLevelPropagator {
      ...
      protected void setLevel(long id, int level) {
         ...
         if (level > this.maxDistance) {
            c = this.distanceFromNearestPlayer.remove(id);
         } else {
      ...
      private boolean isPlayerInChunk(long chunkPos) {
         ObjectSet<ServerPlayerEntity> objectSet = (ObjectSet)ChunkTicketManager.this.playersByChunkPos.get(chunkPos);
