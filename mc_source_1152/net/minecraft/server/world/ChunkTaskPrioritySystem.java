...
public class ChunkTaskPrioritySystem implements AutoCloseable, ChunkHolder.LevelUpdateListener {
   ...
   public ChunkTaskPrioritySystem(List<MessageListener<?>> actors, Executor executor, int maxQueues) {
      this.queues = (Map)actors.stream().collect(Collectors.toMap(Function.identity(), (messageListener) -> {
         return new LevelPrioritizedQueue(messageListener.getName() + "_queue", maxQueues);
      }));
   ...
   public static ChunkTaskPrioritySystem.Task<Runnable> createMessage(ChunkHolder holder, Runnable runnable) {
      ...
      holder.getClass();
      return createMessage(runnable, var10001, holder::getCompletedLevel);
   }
   ...
   public <T> MessageListener<ChunkTaskPrioritySystem.Task<T>> createExecutor(MessageListener<T> executor, boolean bl) {
      return (MessageListener)this.sorter.ask((messageListener2) -> {
         return new TaskQueue.PrioritizedTask(0, () -> {
            this.getQueue(executor);
            messageListener2.send(MessageListener.create("chunk priority sorter around " + executor.getName(), (task) -> {
               this.execute(executor, task.function, task.pos, task.lastLevelUpdatedToProvider, bl);
            }));
         });
      }).join();
   }
   ...
   public MessageListener<ChunkTaskPrioritySystem.SorterMessage> createSorterExecutor(MessageListener<Runnable> executor) {
      return (MessageListener)this.sorter.ask((messageListener2) -> {
         return new TaskQueue.PrioritizedTask(0, () -> {
            messageListener2.send(MessageListener.create("chunk priority sorter around " + executor.getName(), (sorterMessage) -> {
               this.sort(executor, sorterMessage.pos, sorterMessage.runnable, sorterMessage.field_17451);
            }));
         });
      }).join();
   }
   ...
   public void updateLevel(ChunkPos pos, IntSupplier levelGetter, int targetLevel, IntConsumer levelSetter) {
      this.sorter.send(new TaskQueue.PrioritizedTask(0, () -> {
         ...
         this.queues.values().forEach((levelPrioritizedQueue) -> {
            levelPrioritizedQueue.updateLevel(j, pos, targetLevel);
         });
   ...
   private <T> void method_17630(LevelPrioritizedQueue<Function<MessageListener<Unit>, T>> levelPrioritizedQueue, MessageListener<T> actor) {
      this.sorter.send(new TaskQueue.PrioritizedTask(3, () -> {
         ...
         if (stream == null) {
            ...
            Util.combine((List)stream.map((either) -> {
               return (CompletableFuture)either.map(actor::ask, (runnable) -> {
                  runnable.run();
   ...
   public String method_21680() {
      return (String)this.queues.entrySet().stream().map((entry) -> {
         return ((MessageListener)entry.getKey()).getName() + "=[" + (String)((LevelPrioritizedQueue)entry.getValue()).method_21679().stream().map((var0) -> {
            return var0 + ":" + new ChunkPos(var0);
         }).collect(Collectors.joining(",")) + "]";
      }).collect(Collectors.joining(",")) + ", s=" + this.actors.size();
   }
