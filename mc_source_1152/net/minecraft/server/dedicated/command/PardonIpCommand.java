...
public class PardonIpCommand {
   ...
   public static void register(CommandDispatcher<ServerCommandSource> commandDispatcher) {
      ...
      })).then(CommandManager.argument("target", StringArgumentType.word()).suggests((commandContext, suggestionsBuilder) -> {
         return CommandSource.suggestMatching(((ServerCommandSource)commandContext.getSource()).getMinecraftServer().getPlayerManager().getIpBanList().getNames(), suggestionsBuilder);
      }).executes((commandContext) -> {
         return pardonIp((ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "target"));
      })));
   }
   ...
   private static int pardonIp(ServerCommandSource serverCommandSource, String string) throws CommandSyntaxException {
      ...
      if (!matcher.matches()) {
         ...
         if (!bannedIpList.isBanned(string)) {
            ...
            serverCommandSource.sendFeedback(new TranslatableText("commands.pardonip.success", new Object[]{string}), true);
            return 1;
         }
      }
   }
