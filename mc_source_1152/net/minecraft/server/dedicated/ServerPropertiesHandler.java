...
public class ServerPropertiesHandler extends AbstractPropertiesHandler<ServerPropertiesHandler> {
   ...
   public ServerPropertiesHandler(Properties properties) {
      super(properties);
      this.difficulty = (Difficulty)this.get("difficulty", wrapIntParsingFunction(Difficulty::byOrdinal, Difficulty::byName), Difficulty::getName, Difficulty.EASY);
      this.gameMode = (GameMode)this.get("gamemode", wrapIntParsingFunction(GameMode::byId, GameMode::byName), GameMode::getName, GameMode.SURVIVAL);
      this.levelName = this.getString("level-name", "world");
      this.levelSeed = this.getString("level-seed", "");
      this.levelType = (LevelGeneratorType)this.get("level-type", LevelGeneratorType::getTypeFromName, LevelGeneratorType::getName, LevelGeneratorType.DEFAULT);
      this.generatorSettings = this.getString("generator-settings", "");
      this.serverPort = this.getInt("server-port", 25565);
      this.maxBuildHeight = this.parseIntWithOperation("max-build-height", (integer) -> {
      ...
      this.maxWorldSize = this.parseIntWithOperation("max-world-size", (integer) -> {
