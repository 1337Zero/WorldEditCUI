...
public class ForceLoadCommand {
   private static final Dynamic2CommandExceptionType TOOBIG_EXCEPTION = new Dynamic2CommandExceptionType((object, object2) -> {
   ...
   private static int executeQuery(ServerCommandSource source, ColumnPos pos) throws CommandSyntaxException {
      ...
      if (bl) {
         source.sendFeedback(new TranslatableText("commands.forceload.query.success", new Object[]{chunkPos, dimensionType}), false);
         return 1;
      } else {
   ...
   private static int executeQuery(ServerCommandSource source) {
      ...
      if (i > 0) {
         ...
         if (i == 1) {
            source.sendFeedback(new TranslatableText("commands.forceload.list.single", new Object[]{dimensionType, string}), false);
         } else {
            source.sendFeedback(new TranslatableText("commands.forceload.list.multiple", new Object[]{i, dimensionType, string}), false);
         }
      } else {
         source.sendError(new TranslatableText("commands.forceload.added.none", new Object[]{dimensionType}));
      }
   ...
   private static int executeRemoveAll(ServerCommandSource source) {
      ...
      source.sendFeedback(new TranslatableText("commands.forceload.removed.all", new Object[]{dimensionType}), true);
      return 0;
   }
      ...
      if (i >= -30000000 && j >= -30000000 && k < 30000000 && l < 30000000) {
         ...
         if (q > 256L) {
            ...
            if (r == 0) {
               ...
               if (r == 1) {
                  source.sendFeedback(new TranslatableText("commands.forceload." + (forceLoaded ? "added" : "removed") + ".single", new Object[]{chunkPos, dimensionType}), true);
               } else {
                  ChunkPos chunkPos2 = new ChunkPos(m, n);
                  ChunkPos chunkPos3 = new ChunkPos(o, p);
                  source.sendFeedback(new TranslatableText("commands.forceload." + (forceLoaded ? "added" : "removed") + ".multiple", new Object[]{r, dimensionType, chunkPos2, chunkPos3}), true);
               }
