...
public class TestCommand {
   public static void register(CommandDispatcher<ServerCommandSource> dispatcher) {
      dispatcher.register((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)CommandManager.literal("test").then(CommandManager.literal("runthis").executes((commandContext) -> {
         return executeRunThis((ServerCommandSource)commandContext.getSource());
      }))).then(CommandManager.literal("runthese").executes((commandContext) -> {
         return executeRunThese((ServerCommandSource)commandContext.getSource());
      }))).then(CommandManager.literal("run").then(CommandManager.argument("testName", TestFunctionArgumentType.testFunction()).executes((commandContext) -> {
         return executeRun((ServerCommandSource)commandContext.getSource(), TestFunctionArgumentType.getFunction(commandContext, "testName"));
      })))).then(((LiteralArgumentBuilder)CommandManager.literal("runall").executes((commandContext) -> {
         return executeRunAll((ServerCommandSource)commandContext.getSource());
      })).then(CommandManager.argument("testClassName", TestClassArgumentType.method_22370()).executes((commandContext) -> {
         return executeRunAll((ServerCommandSource)commandContext.getSource(), TestClassArgumentType.getTestClass(commandContext, "testClassName"));
      })))).then(CommandManager.literal("export").then(CommandManager.argument("testName", StringArgumentType.word()).executes((commandContext) -> {
         return executeExport((ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "testName"));
      })))).then(CommandManager.literal("import").then(CommandManager.argument("testName", StringArgumentType.word()).executes((commandContext) -> {
         return executeImport((ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "testName"));
      })))).then(((LiteralArgumentBuilder)CommandManager.literal("pos").executes((commandContext) -> {
         return executePos((ServerCommandSource)commandContext.getSource(), "pos");
      })).then(CommandManager.argument("var", StringArgumentType.word()).executes((commandContext) -> {
         return executePos((ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "var"));
      })))).then(CommandManager.literal("create").then(((RequiredArgumentBuilder)CommandManager.argument("testName", StringArgumentType.word()).executes((commandContext) -> {
         return executeCreate((ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "testName"), 5, 5, 5);
      })).then(((RequiredArgumentBuilder)CommandManager.argument("width", IntegerArgumentType.integer()).executes((commandContext) -> {
         return executeCreate((ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "testName"), IntegerArgumentType.getInteger(commandContext, "width"), IntegerArgumentType.getInteger(commandContext, "width"), IntegerArgumentType.getInteger(commandContext, "width"));
      })).then(CommandManager.argument("height", IntegerArgumentType.integer()).then(CommandManager.argument("depth", IntegerArgumentType.integer()).executes((commandContext) -> {
         return executeCreate((ServerCommandSource)commandContext.getSource(), StringArgumentType.getString(commandContext, "testName"), IntegerArgumentType.getInteger(commandContext, "width"), IntegerArgumentType.getInteger(commandContext, "height"), IntegerArgumentType.getInteger(commandContext, "depth"));
      }))))))).then(((LiteralArgumentBuilder)CommandManager.literal("clearall").executes((commandContext) -> {
         return executeClearAll((ServerCommandSource)commandContext.getSource(), 200);
      })).then(CommandManager.argument("radius", IntegerArgumentType.integer()).executes((commandContext) -> {
         return executeClearAll((ServerCommandSource)commandContext.getSource(), IntegerArgumentType.getInteger(commandContext, "radius"));
      }))));
   }
      ...
      if (x <= 32 && y <= 32 && z <= 32) {
         ...
         for(int i = 0; i < x; ++i) {
            for(int j = 0; j < z; ++j) {
               ...
               BlockStateArgument blockStateArgument = new BlockStateArgument(block.getDefaultState(), Collections.EMPTY_SET, (CompoundTag)null);
   ...
   private static int executePos(ServerCommandSource source, String string) throws CommandSyntaxException {
      BlockHitResult blockHitResult = (BlockHitResult)source.getPlayer().rayTrace(10.0D, 1.0F, false);
      ...
      if (!optional.isPresent()) {
         ...
         Text text = (new LiteralText(string2)).setStyle((new Style()).setBold(true).setColor(Formatting.GREEN).setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new LiteralText("Click to copy to clipboard"))).setClickEvent(new ClickEvent(ClickEvent.Action.COPY_TO_CLIPBOARD, "final BlockPos " + string + " = new BlockPos(" + string2 + ");")));
   ...
   private static int executeRunThis(ServerCommandSource source) {
      ...
      if (blockPos2 == null) {
         ...
         run(serverWorld, blockPos2, (TestSet)null);
         return 1;
      }
   }
   ...
   private static int executeRunThese(ServerCommandSource source) {
      ...
      if (collection.isEmpty()) {
         ...
         collection.forEach((blockPosx) -> {
            run(serverWorld, blockPosx, testSet);
         });
         return 1;
      }
   }
   ...
   private static void sendMessage(ServerWorld world, String message, Formatting formatting) {
      world.getPlayers((serverPlayerEntity) -> {
         return true;
      }).forEach((serverPlayerEntity) -> {
         serverPlayerEntity.sendMessage(new LiteralText(formatting + message));
      });
   }
