...
public class TitleCommand {
   ...
   private static int executeClear(ServerCommandSource serverCommandSource, Collection<ServerPlayerEntity> collection) {
      ...
      if (collection.size() == 1) {
         serverCommandSource.sendFeedback(new TranslatableText("commands.title.cleared.single", new Object[]{((ServerPlayerEntity)collection.iterator().next()).getDisplayName()}), true);
      } else {
         serverCommandSource.sendFeedback(new TranslatableText("commands.title.cleared.multiple", new Object[]{collection.size()}), true);
      }
