...
public class CommandManager {
   ...
   public CommandManager(boolean isDedicatedServer) {
      ...
      this.dispatcher.setConsumer((commandContext, bl, i) -> {
         ((ServerCommandSource)commandContext.getSource()).onCommandComplete(commandContext, bl, i);
      });
   }
   ...
   public int execute(ServerCommandSource commandSource, String command) {
      ...
      try {
         byte var18;
         try {
            int var4 = this.dispatcher.execute(stringReader, commandSource);
            return var4;
            ...
            LiteralText var10000 = new LiteralText;
            ...
            commandSource.sendError((new TranslatableText("command.failed", new Object[0])).styled((style) -> {
   ...
   private void makeTreeForSource(CommandNode<ServerCommandSource> tree, CommandNode<CommandSource> result, ServerCommandSource source, Map<CommandNode<ServerCommandSource>, CommandNode<CommandSource>> resultNodes) {
      ...
      while(var5.hasNext()) {
         CommandNode<ServerCommandSource> commandNode = (CommandNode)var5.next();
