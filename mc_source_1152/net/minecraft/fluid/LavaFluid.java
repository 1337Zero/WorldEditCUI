...
public abstract class LavaFluid extends BaseFluid {
   ...
   public void randomDisplayTick(World world, BlockPos blockPos, FluidState fluidState, Random random) {
      ...
      if (world.getBlockState(blockPos2).isAir() && !world.getBlockState(blockPos2).isFullOpaque(world, blockPos2)) {
         ...
         if (random.nextInt(200) == 0) {
            world.playSound((double)blockPos.getX(), (double)blockPos.getY(), (double)blockPos.getZ(), SoundEvents.BLOCK_LAVA_AMBIENT, SoundCategory.BLOCKS, 0.2F + random.nextFloat() * 0.2F, 0.9F + random.nextFloat() * 0.15F, false);
         }
      }
   ...
   private boolean method_15817(WorldView worldView, BlockPos blockPos) {
      return blockPos.getY() >= 0 && blockPos.getY() < 256 && !worldView.isChunkLoaded(blockPos) ? false : worldView.getBlockState(blockPos).getMaterial().isBurnable();
   }
   ...
   public BlockState toBlockState(FluidState fluidState) {
      return (BlockState)Blocks.LAVA.getDefaultState().with(FluidBlock.LEVEL, method_15741(fluidState));
   }
