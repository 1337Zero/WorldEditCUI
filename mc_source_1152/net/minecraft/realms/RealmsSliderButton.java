...
public abstract class RealmsSliderButton extends AbstractRealmsButton<RealmsSliderButtonProxy> {
   ...
   public double toPct(double d) {
      return MathHelper.clamp((this.clamp(d) - this.minValue) / (this.maxValue - this.minValue), 0.0D, 1.0D);
   }

   public double toValue(double d) {
      return this.clamp(MathHelper.lerp(MathHelper.clamp(d, 0.0D, 1.0D), this.minValue, this.maxValue));
   }
