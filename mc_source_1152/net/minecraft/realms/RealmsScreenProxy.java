...
public class RealmsScreenProxy extends Screen {
   ...
   public void narrateLabels() {
      List<String> list = (List)this.children.stream().filter((element) -> {
         return element instanceof RealmsLabelProxy;
      }).map((element) -> {
         return ((RealmsLabelProxy)element).getLabel().getText();
   ...
   public List<AbstractRealmsButton<?>> buttons() {
      ...
      while(var2.hasNext()) {
         AbstractButtonWidget abstractButtonWidget = (AbstractButtonWidget)var2.next();
